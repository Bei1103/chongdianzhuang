...
<div class="boxs-lixa">
    ...
    <ul class="xsz-boxs">
        <?php  if(is_array($ports)) { foreach($ports as $index => $port) { ?>
            <li  data-na="<?php  echo sprintf("%02d", $index+1); ?>" data-type="<?php  echo $port;?>" class="<?php  if($port) { ?>has-pos<?php  } else { ?>se-por<?php  } ?>">
            ...
            if (pay_type == 0) {
                ...
                mui.confirm("是否确定余额扣款" + fee + "元？", "余额扣款", btnArray, function (e) {
                    if (e.index == 1) {
                        ...
                        //     curl = "jiaheAjax.php"
                        // }
                        var inurl='<?php  echo $post_port;?>';
                        var inurl1='<?php  echo $post_money;?>';
                        var inurl2='<?php  echo $post_paytype;?>';
                        $.post(curl, {
                            ax: "pays",
                            port: kms,
                            fee: money,
