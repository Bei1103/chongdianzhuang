...
class Log_Page extends WebPage {
	...
	function main() {
		...
		$list = pdo_fetchall("SELECT  log.* ,u.username FROM " . tablename('vending_machine_perm_log') . " log  "
		...
		$total = pdo_fetchcolumn("SELECT count(*) FROM " . tablename('vending_machine_perm_log') . " log  "
			. " left join " . tablename('users') . " u on log.uid = u.uid  "
	...
	public function merch()
    {
        ...
        $sql = "select log.id,user.merchname as username,log.name,log.op,log.ip,log.createtime from" . tablename('vending_machine_merch_perm_log') . " log "
                ." left join " . tablename('vending_machine_merch_user') . " user"
        ...
        $total = pdo_fetchcolumn("select count(log.id) from" . tablename('vending_machine_merch_perm_log') . " log "
            ." left join " . tablename('vending_machine_merch_user') . " user"
            ." on log.merchid = user.id"
            ." where 1 {$condition}",$params);
