...
class User_Page extends WebPage {
	...
	function main() {
		...
		$list = pdo_fetchall("SELECT u.*,r.rolename FROM " . tablename('vending_machine_perm_user') . " u  "
			. " left join " . tablename('vending_machine_perm_role') . " r on u.roleid =r.id  "
			. " WHERE 1 {$condition} ORDER BY id desc LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);
		$total = pdo_fetchcolumn("SELECT count(*) FROM " . tablename('vending_machine_perm_user') . " u  "
			. " left join " . tablename('vending_machine_perm_role') . " r on u.roleid =r.id  "
			. " WHERE 1 {$condition} ", $params);
		$pager = pagination2($total, $pindex, $psize);
		$roles = pdo_fetchall('select id,rolename from ' . tablename('vending_machine_perm_role') . ' where uniacid=:uniacid and deleted=0', array(':uniacid' => $_W['uniacid']));
	...
	protected function post() {
		...
		//公众号权限
        $permset = intval(m('cache')->getString('permset', 'global'));
        $accounts =  pdo_fetch('SELECT * FROM '.tablename('vending_machine_perm_plugin').' WHERE acid = :uniacid',array(':uniacid'=>$_W['acid']));
		...
		if($_W['role'] == 'operator'){
            $operator = pdo_fetch('SELECT * FROM '.tablename('vending_machine_perm_user').' WHERE uid = :uid AND uniacid = :uniacid ',array(':uid'=>$_W['user']['uid'],':uniacid'=>$_W['uniacid']));
            $operator_prems = explode(',',$operator['perms2']);
        }

		$item = pdo_fetch("SELECT * FROM " . tablename('vending_machine_perm_user') . " WHERE id =:id and deleted=0 and uniacid=:uniacid limit 1", array(':uniacid' => $_W['uniacid'], ':id' => $id));
		...
		if ($_W['ispost']) {
			...
			if (!empty($item['id'])) {
				...
				pdo_update('vending_machine_perm_user', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
				...
				pdo_insert('vending_machine_perm_user', $data);
				$id = pdo_insertid();
	...
	function delete() {
		...
		$items = pdo_fetchall("SELECT id,username FROM " . tablename('vending_machine_perm_user') . " WHERE id in( $id ) AND uniacid=" . $_W['uniacid']);
		foreach ($items as $item) {
			pdo_delete('vending_machine_perm_user', array('id' => $item['id']));
	...
	function status() {
		...
		$items = pdo_fetchall("SELECT id,username FROM " . tablename('vending_machine_perm_user') . " WHERE id in( $id ) AND uniacid=" . $_W['uniacid']);
		foreach ($items as $item) {
			pdo_update('vending_machine_perm_user', array('status' => $status), array('id' => $item['id']));
