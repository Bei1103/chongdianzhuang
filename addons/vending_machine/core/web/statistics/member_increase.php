...
class Member_increase_Page extends WebPage {
...
//时间字段
		...
		if (!empty($days)) {
			...
			for ($i = $days; $i >= 0; $i--) {
				...
				$datas[] = array(
					'date' => $time,
					'mcount' => pdo_fetchcolumn("select count(*) from " . tablename('vending_machine_member') . " where isagent=0  {$condition}", $params),
					'acount' => pdo_fetchcolumn("select count(*) from " . tablename('vending_machine_member') . " where isagent=1  {$condition}", $params)
				);
			}
