...
class Member_cost_Page extends WebPage {
	...
	function main() {
		...
		$sql  = "SELECT m.realname, m.mobile,m.avatar,m.nickname,m.openid,l.levelname,"
		         . "(select ifnull( count(o.id) ,0) from  " . tablename('vending_machine_order') . " o where o.openid=m.openid and o.status>=1 {$condition})  as ordercount,"
		         . "(select ifnull(sum(o.price),0) from  " . tablename('vending_machine_order') . " o where o.openid=m.openid  and o.status>=1 {$condition})  as ordermoney"
		...
		$total = pdo_fetchcolumn("select  count(1) from " . tablename('vending_machine_member') . ' m '
