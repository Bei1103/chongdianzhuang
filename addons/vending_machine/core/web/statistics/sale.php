...
class Sale_Page extends WebPage {
		...
		if (!empty($year) && !empty($month) && !empty($day)) {
		    ...
		    for ($hour = 0; $hour < 24; $hour++) {
		        ...
		        $dr = array(
		            ...
		            'count' => pdo_fetchcolumn("SELECT ifnull({$countfield},0) as cnt FROM " . tablename('vending_machine_order') . " WHERE uniacid=:uniacid and status>=1 and isparent=0 and createtime >=:starttime and createtime <=:endtime", array(
		                ':uniacid' => $_W['uniacid'],
