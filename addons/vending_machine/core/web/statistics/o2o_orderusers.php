...
class O2o_orderusers_Page extends WebPage {
    ...
    function main() {
        ...
        $total = pdo_fetchcolumn("select  count(1) from " . tablename('vending_machine_store') ." where uniacid=:uniacid  {$condition2}", $params2);

        $sql ="select *  from  " . tablename('vending_machine_store') ." where uniacid=:uniacid {$condition2} ";
        ...
        {
            $sql  = "select count(a.openid) from (SELECT  openid   from  " . tablename('vending_machine_order') ."    where uniacid=:uniacid and storeid = :storeid  {$condition}  and isnewstore=1 GROUP BY openid) a";
            ...
            $sql  = "SELECT  SUM(price)   from     " . tablename('vending_machine_order') ."  where   uniacid=:uniacid and storeid = :storeid {$condition}  and isnewstore=1 ";
