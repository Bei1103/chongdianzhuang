...
class O2o_monthorders_Page extends WebPage {
    ...
    function main() {
        ...
        $sql  = "SELECT  s.id, s.storename,ifnull(a.num,0) as num ,ifnull(a.num2,0) as num2,ifnull(a.num3,0) as num3 from  " . tablename('vending_machine_store') . "   s left join
        (SELECT  storeid,count(*) as num,TRUNCATE(avg(price-dispatchprice),2) as num2,sum(price) as num3  from    " . tablename('vending_machine_order') . "  where uniacid=:uniacid  {$condition}  and isnewstore=1 GROUP BY storeid) a on s.id =a.storeid
