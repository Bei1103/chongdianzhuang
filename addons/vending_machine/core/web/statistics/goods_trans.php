...
class Goods_trans_Page extends WebPage {
	...
	function main() {
		...
		$sql = "SELECT g.id,g.title,g.thumb,g.viewcount,"
		         . "(select sum(og.total) from  " . tablename('vending_machine_order_goods') . " og left join ".tablename('vending_machine_order')." o on og.orderid=o.id  where o.status>=1 and og.goodsid=g.id {$condition})  as buycount"
		...
		$list = pdo_fetchall($sql, $params1);
		 ...
		 unset($row);
		$total = pdo_fetchcolumn("select  count(*) from " . tablename('vending_machine_goods') . ' g '
