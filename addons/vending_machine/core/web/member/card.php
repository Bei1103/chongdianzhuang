...
class Card_Page extends WebPage {
    ...
    function activationset()
    {
        ...
        {
            $coupon = pdo_fetch("SELECT id,couponname as title , thumb  FROM ".tablename('vending_machine_coupon')." WHERE uniacid = ".$_W['uniacid']." and id =".$item['couponid']);
        }

        $levels  = pdo_fetchall("SELECT * FROM " . tablename('vending_machine_member_level') . " WHERE uniacid = '{$_W['uniacid']}' ORDER BY level asc");
