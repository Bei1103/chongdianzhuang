...
class List_Page extends WebPage {
    ...
    function main($index=1) {
        ...
        if ($_GPC['groupid'] != '') {
            ...
            $join .= " left join (select * from " . tablename('vending_machine_member_group_log') . " order by log_id desc limit 1 ) glog on (glog.openid = dm.openid) and glog.group_id = ".(int)$_GPC['groupid'];
        }
        if ($_GPC['followed'] != '') {
            // 0: 未关注;1: 已关注;2:取消关注
            if ($_GPC['followed'] == 2) {
        ...
        $sql = "select * ,dm.openid from " . tablename('vending_machine_member') . " dm {$join} where 1 {$condition}  ORDER BY dm.id DESC,dm.createtime DESC ";
...
//        exit();
        ...
        if (!empty($list_group)){
            $res_group = pdo_fetchall("select id,groupname from " . tablename('vending_machine_member_group') . " where id in (".implode(',',$list_group).")",array(),'id');
        }
        isset($list_level) && $list_level = array_values(array_filter($list_level));
        if (!empty($list_level)){
            $res_level = pdo_fetchall("select id,levelname from " . tablename('vending_machine_member_level') . " where id in (".implode(',',$list_level).")",array(),'id');
        }
        isset($list_agent) && $list_agent = array_values(array_filter($list_agent));
        if (!empty($list_agent)){
            $res_agent = pdo_fetchall("select id,nickname as agentnickname,avatar as agentavatar from " . tablename('vending_machine_member') . " where id in (".implode(',',$list_agent).")",array(),'id');
        }
        ...
        foreach ($list as &$row) {
            ...
            $row['ordercount'] = pdo_fetchcolumn('select count(*) from ' . tablename('vending_machine_order') . ' where uniacid=:uniacid and openid=:openid and status=3', array(':uniacid' => $_W['uniacid'], ':openid' => $row['openid']));
            $row['ordermoney'] = pdo_fetchcolumn('select sum(price) from ' . tablename('vending_machine_order') . ' where uniacid=:uniacid and openid=:openid and status=3', array(':uniacid' => $_W['uniacid'], ':openid' => $row['openid']));
...
//导出Excel
        if ($_GPC['export'] == '1') {
            ...
            if ($index == 1) {
                ...
                $savepath = VENDING_MACHINE_DATA.'member/'.$filename;
            ...
            m('excel')->exportCSV($list, array(
                "title" => "会员数据",
                "columns" => $columns
            ),VENDING_MACHINE_DATA.'member/',$index,$exflag);
            unset($list);
            if(!$exflag){
                $pindex ++;
                $this->main($pindex);
...
//            ));
        ...
        if($join == "" && $condition == " and dm.uniacid=:uniacid"){
            if($open_redis) {
                $redis_key = "vending_{$_W['uniacid']}_member_list";
                $total = m('member')->memberRadisCount($redis_key,false);
                if(!$total){
                    $total = pdo_fetchcolumn("select count(*) from" . tablename('vending_machine_member') . " dm {$join} where 1 {$condition} ", $params);
...
//是否开启分销
    ...
    function detail() {
        ...
        $member['self_ordercount'] = pdo_fetchcolumn('select count(*) from ' . tablename('vending_machine_order') . ' where uniacid=:uniacid and openid=:openid and status=3', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
        $member['self_ordermoney'] = pdo_fetchcolumn('select sum(price) from ' . tablename('vending_machine_order') . ' where uniacid=:uniacid and openid=:openid and status=3', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));

        if (!empty($member['agentid'])) {
            $parentagent = m('member')->getMember($member['agentid']);
        }

        $order = pdo_fetch('select finishtime from ' . tablename('vending_machine_order') . ' where uniacid=:uniacid and openid=:openid and status>=1 Limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
        ...
        if ($plugin_sns) {
            ...
            $sns_member = pdo_fetch('select * from '.tablename('vending_machine_sns_member')." where openid=:openid and uniacid=:uniacid limit 1",array(':openid'=>$member['openid'],':uniacid'=>$_W['uniacid']));

            $sns_member['postcount'] = pdo_fetchcolumn('select count(*) from '.tablename('vending_machine_sns_post').' where uniacid=:uniacid and openid=:openid and pid=0 and deleted = 0 and checked=1',array(':uniacid'=>$_W['uniacid'],':openid'=>$member['openid']));
            $sns_member['replycount'] = pdo_fetchcolumn('select count(*) from '.tablename('vending_machine_sns_post').' where uniacid=:uniacid and openid=:openid and pid>0 and deleted = 0 and checked=1',array(':uniacid'=>$_W['uniacid'],':openid'=>$member['openid']));
        ...
        if ($_W['ispost']) {
            ...
            if($openbind){
                ...
                if(!empty($data['mobileverify'])){
                    ...
                    $m = pdo_fetch('select id from ' . tablename('vending_machine_member') . ' where mobile=:mobile and mobileverify=1 and uniacid=:uniaicd limit 1 ', array(':mobile'=>$data['mobile'], ':uniaicd'=>$_W['uniacid']));
                    if(!empty($m) && $m['id']!=$id){
            ...
            pdo_update('vending_machine_member', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
            ...
            if ($hascommission) {
                if (cv('commission.agent.edit')) {
                    ...
                    if (!empty($adata)) {
                        ...
                        pdo_update('vending_machine_member', $adata, array('id' => $id, 'uniacid' => $_W['uniacid']));
                        if($adata['agentid']!=$member['agentid']){
//                            p('commission') -> modify($member['id'], $adata['agentid']);
                            if(p('dividend')){
                                $dividend = pdo_fetch('select id,isheads,headsid,headsstatus from '.tablename('vending_machine_member').' where id = :id',array(':id'=>$adata['agentid']));
                                $dividend_init = pdo_fetch('select * from '.tablename('vending_machine_dividend_init').' where headsid = :headsid',array(':headsid'=>$adata['agentid']));
                                if(!empty($dividend['isheads']) && !empty($dividend['headsstatus']) && !empty($dividend_init['status'])){
                                    pdo_update('vending_machine_member',array('headsid'=>$adata['agentid']),array('id'=>$member['id'],'uniacid'=>$_W['uniacid']));
                                    $data = pdo_fetchall('select id from '.tablename('vending_machine_commission_relation').' where pid = :pid',array(':pid'=>$member['id']));
                                    if(!empty($data)){
                                        $ids = array();
                                        foreach($data as $k => $v){
                                            $ids[] = $v['id'];
                                        }
                                        pdo_update('vending_machine_member', array("headsid"=>$adata['agentid']), array('id' =>$ids));
                                    }
                                }else if(empty($dividend['isheads']) && !empty($dividend['headsid'])){
                                    pdo_update('vending_machine_member',array('headsid'=>$dividend['headsid']),array('id'=>$member['id'],'uniacid'=>$_W['uniacid']));
                                    $data = pdo_fetchall('select id from '.tablename('vending_machine_commission_relation').' where pid = :pid',array(':pid'=>$member['id']));
                                    if(!empty($data)){
                                        $ids = array();
                                        foreach($data as $k => $v){
                                        ...
                                        pdo_update('vending_machine_member', array("headsid"=>$dividend['headsid']), array('id' =>$ids));
                                    }
                                }else{
                                    pdo_update('vending_machine_member',array('headsid'=>0),array('id'=>$member['id'],'uniacid'=>$_W['uniacid']));
                                    $data = pdo_fetchall('select id from '.tablename('vending_machine_commission_relation').' where pid = :pid',array(':pid'=>$member['id']));
                                    if(!empty($data)){
                                        $ids = array();
                                        foreach($data as $k => $v){
                                        ...
                                        pdo_update('vending_machine_member', array("headsid"=>0), array('id' =>$ids));
                                    }
                                }
                            }
                        }
            ...
            if($hasglobonus){
                if (cv('globonus.partner.check')) {
                    ...
                    if (!empty($gdata)) {
                        ...
                        pdo_update('vending_machine_member', $gdata, array('id' => $id, 'uniacid' => $_W['uniacid']));
                    }
                }
            }
            ...
            if($hasauthor){
                if (cv('author.partner.check')) {
                    ...
                    if (!empty($author_data)) {
                        ...
                        pdo_update('vending_machine_member', $author_data, array('id' => $id, 'uniacid' => $_W['uniacid']));
                    }
                }
            }
            ...
            if($hasabonus){
                if (cv('abonus.agent.check')) {
                    ...
                    if (!empty($aadata)) {
                        ...
                        pdo_update('vending_machine_member', $aadata, array('id' => $id, 'uniacid' => $_W['uniacid']));
                    }
                }
            }
            ...
            if($hassns){
                if (cv('sns.member.edit')) {
                    ...
                    if (!empty($snsdata)) {
                        ...
                        pdo_update('vending_machine_sns_member', $snsdata, array('id' => $sns_member['id'], 'uniacid' => $_W['uniacid']));
                    }
                }
            }
        ...
        $order = pdo_fetch('select finishtime from ' . tablename('vending_machine_order') . ' where uniacid=:uniacid and openid=:openid and status=3 order by id desc limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid']));
        $member['last_ordertime'] = $order['finishtime'];
    ...
    function delete() {
        ...
        foreach ($members as $member) {
            //pdo_update('vending_machine_member',array('agentid'=>0),array('agentid'=>$member['id']));
            pdo_delete('vending_machine_member', array('id' => $member['id']));
    ...
    function setblack() {
        ...
        foreach($members as $member) {
            if (!empty($black)) {
                pdo_update('vending_machine_member', array('isblack' => 1), array('id' => $member['id']));
                plog('member.list.edit', "设置黑名单 <br/>用户信息:  ID: {$member['id']} /  {$member['openid']}/{$member['nickname']}/{$member['realname']}/{$member['mobile']}");
            } else {
                pdo_update('vending_machine_member', array('isblack' => 0), array('id' => $member['id']));
    ...
    function changelevel() {
        ...
        if($_W['ispost']){
            ...
            if($toggle=='group'){
                if(!empty($levelid)){
                    ...
                    if(!empty($levelid_arr)){
                        foreach ($levelid_arr as $id){
                            $group = pdo_fetch('select * from ' . tablename('vending_machine_member_group') . ' where id = :id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
                            if(empty($group)){
                                show_json(0, "此分组不存在");
                            }
                        }
                ...
                if(!empty($levelid)) {
                    $level = pdo_fetch('select * from ' . tablename('vending_machine_member_level') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $levelid, ':uniacid' => $_W['uniacid']));
                    if (empty($level)) {
                        show_json(0, "此等级不存在");
                    }
                }else{
            ...
            $members = pdo_fetchall("select id,openid,nickname,realname,mobile from " . tablename('vending_machine_member')  . " WHERE id in( $idsstr ) AND uniacid=" . $_W['uniacid']);
            if(!empty($members)){
                foreach ($members as $member) {
                    if ($toggle=='group'){
                        m('member')->setGroups($member['id'],$levelid,'管理员设置批量分组');
                    }else{
                        pdo_update('vending_machine_member', $arr, array('id' => $member['id']));
                        $changeids[] = $member['id'];
                    }
                }
            }
