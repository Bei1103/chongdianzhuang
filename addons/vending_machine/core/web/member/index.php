...
class Index_Page extends WebPage
{
	...
	protected function selectMemberCreate($day = 0)
	{
		...
		$sql = 'select count(*) from ' . tablename('vending_machine_member') . ' where uniacid = :uniacid and createtime between :createtime1 and :createtime2';
		$param = array(
			':uniacid' => $_W['uniacid'],
	...
	function query()
	{
        ...
        $ds = pdo_fetchall('SELECT * FROM ' . tablename('vending_machine_member') . " WHERE 1 {$condition} order by id asc", $params);
        foreach ($ds as &$value){
	...
	protected function ajaxnewmember($day=0)
	{
		...
		$member_count = pdo_fetchcolumn('select count(*) from ' . tablename('vending_machine_member') . ' where uniacid=:uniacid', $param);
	...
	protected function ajaxmembergender()
	{
		...
		$sql_member = 'select gender,count(gender) as gender_num from ' . tablename('vending_machine_member') . ' where uniacid = :uniacid group by gender';
		$param_member = array(
			':uniacid' => $_W['uniacid']
		);
	...
	{
		global $_W;
		$levels = pdo_fetchall('select * from ' . tablename('vending_machine_member_level') . ' where uniacid=:uniacid order by level asc', array(':uniacid' => $_W['uniacid']), 'id');
		$levelname = array();
		foreach($levels as $l){
		...
		$sql_level = 'select level,count(level) as level_num from ' . tablename('vending_machine_member') . ' where uniacid = :uniacid group by level';
		$param_level = array(
			':uniacid' => $_W['uniacid']
		);
	...
	{
		global $_W;
		$province = pdo_fetchall('select province,count(province) as province_num from ' . tablename('vending_machine_member') . ' where uniacid = :uniacid group by province', array(':uniacid' => $_W['uniacid']));
		$result = array();
