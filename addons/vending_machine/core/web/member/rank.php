...
class Rank_Page extends WebPage
{
	...
	function main()
	{
		...
		if ($_W['ispost']){
            ...
            $result = pdo_fetchall("SELECT sm.id,sm.uid,m.credit1,sm.nickname,sm.avatar,sm.openid FROM ".tablename('vending_machine_member')." sm RIGHT JOIN ".tablename('mc_members')." m ON m.uid=sm.uid WHERE sm.uniacid = :uniacid ORDER BY m.credit1 DESC LIMIT {$rank['num']}",array(':uniacid'=>$_W['uniacid']));
            $result1 = pdo_fetchall("SELECT id,uid,credit1,nickname,avatar,openid FROM ".tablename('vending_machine_member')." WHERE uniacid = :uniacid AND uid=0 ORDER BY credit1 DESC LIMIT {$rank['num']}",array(':uniacid'=>$_W['uniacid']));
