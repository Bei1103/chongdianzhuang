...
class Open_Page extends WebPage
{
    ...
    function main()
    {
        ...
        $key = pdo_fetch("SELECT * FROM ".tablename('vending_machine_open_plugin')." WHERE plugin = :plugin", array(':plugin' => $plugin));
        if(empty($plugin)){
        ...
        if($_W['ispost']){
            ...
            if($key){
                ...
                $num = pdo_get('vending_machine_open_plugin',array('plugin'=>$plugin),'status');
                if(!empty($num)){
                    pdo_update('vending_machine_open_plugin',array('status'=>1,'expirtime'=>time()+172800),array('plugin'=>$plugin));
                ...
                pdo_insert('vending_machine_open_plugin', $data);
                $id = pdo_insertid();
                if($id){
                    show_json(1,array('url'=>webUrl($plugin)));
                }
    ...
    private  function checkOpen($key='',$plugin='',$domain=''){
        ...
        $resp = ihttp_post(VENDING_MACHINE_AUTH_URL . "/grant", $data);
