...
class Goods_selector_Page extends WebPage
{
    /**
     * 商品选择器
     */
    public function main($page = 0)
    {
        ...
        if (trim($_GPC['creditshop']) == 1) {
            ...
            $tablename = tablename('vending_machine_creditshop_goods');
        ...
        elseif (trim($_GPC['group']) == 1) {
            ...
            $tablename = tablename('vending_machine_groups_goods');
        }
...
//        处理自定义项目
        ...
        $id = intval($_GPC['id']);
//        商品
        $sql = "select * from " . tablename('vending_machine_goods') . " where id = {$id}";
        $goods = pdo_fetch($sql);
        if (empty($goods)) {
        ...
        if (empty($_GPC['nooption'])) {
            //规格
            $sql = "select * from " . tablename('vending_machine_goods_option') . " where goodsid = {$id}";
