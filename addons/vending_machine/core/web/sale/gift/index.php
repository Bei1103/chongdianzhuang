...
class Index_Page extends WebPage {
    ...
    function main() {
        ...
        $gifts = pdo_fetchall("SELECT * FROM ".tablename('vending_machine_gift')."
                    WHERE 1 ".$condition." ORDER BY displayorder DESC,id DESC LIMIT ". ($pindex - 1) * $psize . ',' . $psize,$params);
        $total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vending_machine_gift') . " WHERE 1 ".$condition." ", $params);
    ...
    protected function post() {
        ...
        if ($_W['ispost']){
            ...
            if( !empty($data['goodsid']) ){
                ...
                foreach ( $goodsArr as $k => $v ){
                    $temp = pdo_fetch("select isverify from ".tablename('vending_machine_goods')." where uniacid = ".$uniacid." and status = 1 and id = ".$v." and deleted = 0 ");
                    if( $temp['isverify'] == 2 ){
            ...
            if(empty($data['thumb'])){
                foreach($_GPC['giftgoodsid'] as $key => $value){
                    $giftgood = pdo_fetch("select id,title,thumb from ".tablename('vending_machine_goods')." where uniacid = ".$uniacid." and status = 2 and deleted = 0 limit 1 ");
                }
            ...
            if (!empty($id)) {
                pdo_update('vending_machine_gift', $data, array('id' => $id));
                plog('sale.gift.edit', "编辑赠品 ID: {$id} <br/>赠品名称: {$data['title']}");
            } else {
                pdo_insert('vending_machine_gift', $data);
                $id = pdo_insertid();
        ...
        $item = pdo_fetch("SELECT * FROM ".tablename('vending_machine_gift')." WHERE uniacid = ".$uniacid." and id = ".$id." ");
        if(!empty($item['thumb'])){
        ...
        if(!empty($item['goodsid'])){
            ...
            if($goodsid){
                foreach($goodsid as $key => $value){
                    $goods[$key] = pdo_fetch("select id,title,thumb from ".tablename('vending_machine_goods')." where uniacid = ".$uniacid." and status = 1 and id = ".$value." and deleted = 0 ");
                }
            }
        }
        //赠品
        if(!empty($item['giftgoodsid'])){
            ...
            if($giftid){
                foreach($giftid as $key => $value){
                    $gift[$key] = pdo_fetch("select id,title,thumb from ".tablename('vending_machine_goods')." where uniacid = ".$uniacid." and status = 2 and id = ".$value." and deleted = 0 and total > 0 ");
                }
            }
    ...
    function delete() {
        ...
        $items = pdo_fetchall("SELECT id,title FROM " . tablename('vending_machine_gift') . " WHERE id in( $id ) AND uniacid=" . $_W['uniacid']);
        foreach ($items as $item) {
            pdo_update('vending_machine_gift', array('deleted' => 1,'status'=>0), array('id' => $item['id']));
    ...
    function status() {
        ...
        foreach ($items as $item) {
            pdo_update('vending_machine_gift', array('status' => intval($_GPC['status'])), array('id' => $item['id']));
    ...
    function delete1() {
        ...
        foreach ($items as $item) {
            pdo_delete('vending_machine_gift', array('id' => $item['id']));
    ...
    function restore() {
        ...
        foreach ($items as $item) {
            pdo_update('vending_machine_gift', array('deleted' => 0), array('id' => $item['id']));
    ...
    function change() {
        ...
        $gift = pdo_fetch('select id from ' . tablename('vending_machine_gift') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
        if (empty($gift)) {
            show_json(0, array('message' => '参数错误'));
        }

        pdo_update('vending_machine_gift', array($type => $value), array('id' => $id));
        show_json(1);
    }
