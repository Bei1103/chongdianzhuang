...
class Index_Page extends WebPage {
    ...
    function main() {
        ...
        $gifts = pdo_fetchall("SELECT * FROM ".tablename('vending_machine_fullback_goods')."
        ...
        $total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vending_machine_fullback_goods') . " WHERE 1 ".$condition." ", $params);
    ...
    protected function post() {
        ...
        if ($_W['ispost']){
            ...
            if(empty($id)){
                ...
                $item = pdo_fetch("SELECT goodsid FROM ".tablename('vending_machine_fullback_goods')." WHERE uniacid = ".$uniacid." and id = ".$id." ");
            ...
            $fullbackgoodsnum = pdo_fetchcolumn("select count(1) from ".tablename('vending_machine_fullback_goods')." where goodsid = :goodsid and uniacid = :uniacid ",array(":goodsid"=>$data['goodsid'],":uniacid"=>$uniacid));
            ...
            $good_data = pdo_fetch("select title,thumb,marketprice,goodssn,productsn,hasoption
                            from ".tablename('vending_machine_goods')." where id = ".$data['goodsid']." and uniacid = ".$uniacid." ");
            if(empty($data['thumb'])){
            ...
            if(!empty($good_data['option'])){
                ...
                pdo_update('vending_machine_goods_option',array('isfullback'=>0),array('uniacid'=>$uniacid,'goodsid'=>$data['goodsid']));
                foreach($fullbackOption as $val){
                    ...
                    pdo_update('vending_machine_goods_option',$optionData,array('uniacid'=>$uniacid,'id'=>intval($val)));
                }
            }

            //show_json(0,$data);
            if (!empty($id)) {
                pdo_update('vending_machine_fullback_goods', $data, array('id' => $id));
                plog('sale.fullback.edit', "编辑全返 ID: {$id} <br/>全返名称: {$data['titles']}");
            } else {
                pdo_insert('vending_machine_fullback_goods', $data);
                $id = pdo_insertid();
                plog('sale.fullback.add', "添加全返 ID: {$id}  <br/>全返名称: {$data['titles']}");
            }
            //更新最低价和最高价
            $sql = "update ".tablename('vending_machine_fullback_goods')." g set
                g.minallfullbackallprice = (select min(allfullbackprice) from ".tablename('vending_machine_goods_option')." where goodsid = ".$data['goodsid']."),
                g.maxallfullbackallprice = (select max(allfullbackprice) from ".tablename('vending_machine_goods_option')." where goodsid = ".$data['goodsid']."),
                g.minallfullbackallratio = (select min(allfullbackratio) from ".tablename('vending_machine_goods_option')." where goodsid = ".$data['goodsid']."),
                g.maxallfullbackallratio = (select max(allfullbackratio) from ".tablename('vending_machine_goods_option')." where goodsid = ".$data['goodsid'].")
                where g.goodsid = ".$data['goodsid']." and g.hasoption=1 and g.uniacid = ".$uniacid." and g.id = ".$id." ";
            pdo_query($sql);
            if($data['status']>0){
                pdo_update('vending_machine_goods', array('isfullback'=>$id), array('id' => $data['goodsid']));
            }else{
                pdo_update('vending_machine_goods', array('isfullback'=>0), array('id' => $data['goodsid']));
            }
            show_json(1, array('url' => webUrl('sale/fullback/edit',array('id'=>$id))));
        }
        if(!empty($id)){
            $item = pdo_fetch("SELECT * FROM ".tablename('vending_machine_fullback_goods')." WHERE uniacid = ".$uniacid." and id = ".$id." ");
            $good_data = pdo_fetch("select title,thumb,hasoption
                            from ".tablename('vending_machine_goods')." where id = ".$item['goodsid']." and uniacid = ".$uniacid." ");
            if(empty($good_data)){
            ...
            if($item['hasoption']>0){
                ...
                FROM ".tablename('vending_machine_goods_option')."
    ...
    function status() {
        ...
        $items = pdo_fetchall("SELECT id,titles FROM " . tablename('vending_machine_fullback_goods') . " WHERE id in( $id ) AND uniacid=" . $_W['uniacid']);

        foreach ($items as $item) {
            pdo_update('vending_machine_fullback_goods', array('status' => intval($_GPC['status'])), array('id' => $item['id']));
            pdo_update('vending_machine_goods', array('isfullback'=>intval($item['id'])), array('id' => $items['goodsid']));
    ...
    function delete1() {
        ...
        $items = pdo_fetchall("SELECT id,titles,goodsid FROM " . tablename('vending_machine_fullback_goods') . " WHERE id in( $id ) AND uniacid=" . $_W['uniacid']);

        foreach ($items as $item) {
            pdo_delete('vending_machine_fullback_goods', array('id' => $item['id']));
            pdo_update('vending_machine_goods', array('isfullback'=>0), array('id' => $item['goodsid']));
    ...
    function change() {
        ...
        $gift = pdo_fetch('select id from ' . tablename('vending_machine_fullback_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
        if (empty($gift)) {
            show_json(0, array('message' => '参数错误'));
        }

        pdo_update('vending_machine_fullback_goods', array($type => $value), array('id' => $id));
        show_json(1);
    }
    ...
    function query(){
        ...
        $goods = pdo_fetchall('SELECT id,title,thumb,marketprice,total
            FROM ' . tablename('vending_machine_goods') . "
            WHERE 1 {$condition} ORDER BY displayorder DESC,id DESC LIMIT ". ($pindex - 1) * $psize . ',' . $psize, $params);

        $total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vending_machine_goods') . " WHERE 1 ".$condition." ", $params);
    ...
    function hasoption(){
        ...
        //是否有规格
        $goods = pdo_fetch("select id,title,marketprice,hasoption,isfullback from ".tablename('vending_machine_goods')." where uniacid = :uniacid and id = :goodsid ",$params);
        if(!empty($id)){
            $fullback = pdo_fetch("select * from ".tablename('vending_machine_fullback_goods')."
    ...
    function option(){
        ...
        $option = pdo_fetch("SELECT id,title FROM ".tablename('vending_machine_goods_option')."
    ...
    private function deal_fullback_goods($goods = array()){
        ...
        foreach ($goods as &$item){
            if($item['hasoption']){
                ...
                $options = pdo_fetchall("SELECT id,title,marketprice,`day`,allfullbackprice,fullbackprice FROM ".tablename('vending_machine_goods_option')."
            ...
            $item['order_count'] = pdo_fetchcolumn('SELECT COUNT(DISTINCT(orderid)) FROM ' . tablename('vending_machine_order_goods') . " WHERE uniacid = ".$_W['uniacid']." and fullbackid = ".$item['id']." and  goodsid = ".$item['goodsid']);
        }unset($item);
        return $goods;
    }
    ...
    public function info()
    {
        ...
        $sql = "select * from " . tablename('vending_machine_fullback_log') . " where orderid = :orderid and optionid = :optionid and goodsid = :goodsid ";
        ...
        $sql = "select * from " . tablename('vending_machine_fullback_log_map') . " where logid = :logid order by fullback_time desc LIMIT ". ($pindex - 1) * $psize . ',' . $psize;

        $fullbackLogs = pdo_fetchall($sql, array(':logid' => $fullbackInfo['id']));

        $total = pdo_fetchcolumn("select count(*) from " .tablename('vending_machine_fullback_log_map') . " where logid = :logid", array(':logid' => $fullbackInfo['id']));
        $pager = pagination2($total, $pindex, $psize,'',array('before' => 5, 'after' => 4, 'ajaxcallback'=>'select_page', 'callbackfuncname'=>'select_page'));
