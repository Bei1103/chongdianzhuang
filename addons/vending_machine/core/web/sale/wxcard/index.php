...
class Index_Page extends ComWebPage {
    ...
    function main() {
        ...
        $sql = 'SELECT * FROM ' . tablename('vending_machine_wxcard') . " "
            . " where  1 and {$condition} ORDER BY id DESC,id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize;

        $list = pdo_fetchall($sql, $data);

        $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('vending_machine_wxcard') . " where 1 and {$condition}", $data);
    ...
    protected function post() {
        ...
        if(!empty($id))
        {
            $item = pdo_fetch("SELECT * FROM " . tablename('vending_machine_wxcard') . " WHERE id =:id and uniacid=:uniacid  limit 1", array(':uniacid' => $_W['uniacid'], ':id' => $id));
        }
        ...
        if ($_W['ispost']) {
            ...
            if (!empty($item)) {
                ...
                //保存到本地
                pdo_update('vending_machine_wxcard', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
               ...
               /* $imagesize =@getimagesize($imgurl);
                ...
                //保存到本地
                pdo_insert('vending_machine_wxcard', $data);
                $id = pdo_insertid();
        ...
        }else
        {
            ...
            if($item['limitgoodtype']==1||$item['limitgoodtype']==2)
            {
                if($item['limitgoodids']){
                    ...
                    $goods = pdo_fetchall("SELECT id,title,thumb FROM ".tablename('vending_machine_goods')." WHERE uniacid = :uniacid and id in ({$limitgoodids}) ",array(':uniacid'=>$_W['uniacid']));
                }
            }
    ...
    function delete() {
        ...
        $items = pdo_fetchall("SELECT id,card_id,title FROM " . tablename('vending_machine_wxcard') . " WHERE id in( $id ) and merchid=0 AND uniacid=" . $_W['uniacid']);
        foreach ($items as $item) {
            ...
            pdo_delete('vending_machine_wxcard', array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
    ...
    function stock() {
        ...
        $sql ="select id,uniacid,logo_url, card_id,title,total_quantity,quantity from " . tablename('vending_machine_wxcard') ;
