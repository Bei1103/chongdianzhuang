...
class Index_Page extends ComWebPage {
    ...
    function enoughfree() {
        ...
        if (!empty($data['goodsids'])){
            $goods = pdo_fetchall("SELECT id,uniacid,title,thumb FROM ".tablename('vending_machine_goods')." WHERE uniacid=:uniacid AND id IN (".implode(',',$data['goodsids']).")",array(':uniacid'=>$_W['uniacid']));
        }
