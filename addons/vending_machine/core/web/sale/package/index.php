...
class Index_Page extends WebPage {
    ...
    function main() {
        ...
        $packages = pdo_fetchall("SELECT * FROM ".tablename('vending_machine_package')."
        ...
        $total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vending_machine_package') . " WHERE 1 ".$condition." ", $params);
    ...
    protected function post() {
        ...
        if ($_W['ispost']){
            ...
            foreach($goodsid as $key => $value){
                $good_data = pdo_fetch("select title,thumb,marketprice,goodssn,productsn,hasoption,merchid
                            from ".tablename('vending_machine_goods')." where id = ".$value." and uniacid = ".$uniacid." ");
            ...
            if (!empty($id)) {
                foreach($goodsid as $key => $value) {
                    $packagenum = pdo_fetchcolumn("select count(1) from " . tablename('vending_machine_package_goods') . " where goodsid = :goodsid and uniacid = :uniacid",array(':goodsid'=>$value,':uniacid'=>$value));
                    $thisgoods = pdo_fetch("select id from ".tablename('vending_machine_package_goods')." where pid = :pid and goodsid = :goodsid and uniacid = :uniacid ",array(':pid'=>$id,'goodsid'=>$value,':uniacid'=>$uniacid));
                    if(!$thisgoods && $packagenum >= 3){
                        show_json(0, '同一件商品最多参与三个套餐活动!');
                    }
                }

                $package_update = pdo_update('vending_machine_package', $data, array('id' => $id, 'uniacid' => $uniacid));
                $package_goods_del = pdo_delete('vending_machine_package_goods',array('pid'=>$id,'uniacid' => $uniacid));
                $package_goods_option_del = pdo_delete('vending_machine_package_goods_option',array('pid'=>$id,'goodsid'=>$value,'uniacid' => $uniacid));
                foreach($goodsid as $key => $value){
                    ...
                    $package_goods_insert = pdo_insert('vending_machine_package_goods', $good_data);

                    //添加规格表数据
                    if(!empty($good_data['option'])){
                        $packageGoodsOption = array_filter(explode(",",$good_data['option']));
                        foreach($packageGoodsOption as $k => $val){
                            $op = pdo_fetch("SELECT id,title,marketprice,goodssn,productsn FROM ".tablename('vending_machine_goods_option')."
                            ...
                            $package_goods_option_insert = pdo_insert('vending_machine_package_goods_option', $optionData);
                ...
                foreach($goodsid as $key => $value) {
                    $packagenum = pdo_fetchcolumn("select count(1) from " . tablename('vending_machine_package_goods') . " where uniacid = " . $uniacid . " and goodsid = " . $value . " ");
                    if ($packagenum >= 3) {
                        show_json(0, '同一件商品最多参与三个套餐活动!');
                    }
                }
                $package_insert = pdo_insert('vending_machine_package', $data);
                if(!$package_insert){
                    show_json(0,'套餐添加失败！');
                }
                $id = pdo_insertid();
        ...
        $item = pdo_fetch("SELECT * FROM ".tablename('vending_machine_package')." WHERE uniacid = ".$uniacid." and id = $id ");
        if($item){
            ...
            $package_goods = pdo_fetchall("select id,pid,title,thumb,packageprice,hasoption,goodsid,`option`,commission1,commission2,commission3
                            from ".tablename('vending_machine_package_goods')." where pid = ".$id." and uniacid = ".$uniacid." ");
            ...
            foreach($package_goods as $key => $value){
                if($value['hasoption']){
                    $package_goods[$key]['optiontitle'] = pdo_fetchall("select id,goodsid,optionid,pid,packageprice,title,marketprice,commission1,commission2,commission3
                            from ".tablename('vending_machine_package_goods_option')." where pid = ".$id." and goodsid = ".$value['goodsid']." and uniacid = ".$uniacid." ");
                }
            }
        }
    ...
    function delete() {
        ...
        $items = pdo_fetchall("SELECT id,title FROM " . tablename('vending_machine_package') . " WHERE id in( $id ) AND uniacid=" . $_W['uniacid']);
        foreach ($items as $item) {
            pdo_update('vending_machine_package', array('deleted' => 1,'status'=>0), array('id' => $item['id']));
    ...
    function status() {
        ...
        foreach ($items as $item) {
            pdo_update('vending_machine_package', array('status' => intval($_GPC['status'])), array('id' => $item['id']));
    ...
    function delete1() {
        ...
        foreach ($items as $item) {
            pdo_delete('vending_machine_package', array('id' => $item['id']));
            pdo_delete('vending_machine_package_goods', array('pid' => $item['id']));
            pdo_delete('vending_machine_package_goods_option', array('pid' => $item['id']));
    ...
    function restore() {
        ...
        foreach ($items as $item) {
            pdo_update('vending_machine_package', array('deleted' => 0), array('id' => $item['id']));
    ...
    function change() {
        ...
        $package = pdo_fetch('select id from ' . tablename('vending_machine_package') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
        if (empty($package)) {
            show_json(0, array('message' => '参数错误'));
        }

        pdo_update('vending_machine_package', array($type => $value), array('id' => $id));
        show_json(1);
    }
    ...
    function query(){
        ...
        foreach ($ds as $key => $row){
            if($row['merchid']>0){
                $merch = pdo_fetch("select merchname from ".tablename('vending_machine_merch_user')." where id = :merchid and uniacid = :uniacid ",array(':merchid'=>$row['merchid'],":uniacid"=>$uniacid));
    ...
    function hasoption(){
        ...
        //是否有规格
        $goods = pdo_fetch("select id,title,marketprice,hasoption,nocommission from ".tablename('vending_machine_goods')." where uniacid = :uniacid and id = :goodsid ",$params);
        if(!empty($pid)){
            $packgoods = pdo_fetch("select id,title,packageprice,commission1,commission2,commission3,`option`,goodsid from ".tablename('vending_machine_package_goods')."
        ...
        if($goods['hasoption']){
            $hasoption = 1;
            $option = array();
            $option = pdo_fetchall("SELECT id,title,marketprice,specs,displayorder FROM ".tablename('vending_machine_goods_option')."
            WHERE uniacid = :uniacid and goodsid = :goodsid  ORDER BY displayorder DESC,id DESC ",$params);
            $package_option = pdo_fetchall("SELECT id,uniacid,goodsid,optionid,pid,title,marketprice,packageprice,commission1,commission2,commission3 FROM ".tablename('vending_machine_package_goods_option')."
