...
class Setticket_Page extends WebPage {
			...
			if(empty($id)){
				if(is_array($_GPC['couponid'])){
					...
					foreach ($_GPC['couponid'] as $cpk => $cpv) {
						...
						$sql = 'SELECT * FROM '.tablename('vending_machine_coupon').$where;
...
//						} else
			...
			if (empty($id)) {
				$result = pdo_insert('vending_machine_sendticket',$data);
				plog('sendticket.set', "设置发券内容");
			} else {
				$params = array('id' => $id);
				$result = pdo_update('vending_machine_sendticket',$data,$params);
			}
		...
		$sql = 'SELECT * FROM '.tablename('vending_machine_sendticket').' WHERE uniacid = '.intval($_W['uniacid']);
		$item = pdo_fetch($sql);
		if (!empty($item)) {
			...
			foreach ($cpids as $cpidk => $cpidv) {
				$cpsql = 'SELECT * FROM '.tablename('vending_machine_coupon').' WHERE uniacid = '.intval($_W['uniacid']).' AND id = '.intval($cpidv);
