...
class Shareticket_Page extends WebPage {
    ...
    function main() {
        ...
        $gifts = pdo_fetchall("SELECT * FROM ".tablename('vending_machine_sendticket_share')."
        ...
        $total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vending_machine_sendticket_share') . " WHERE 1 ".$condition." ", $params);
    ...
    protected function post() {
        ...
        if ($_W['ispost']){
            ...
            if (empty($id))  {
                $rathersql = 'SELECT * FROM '.tablename('vending_machine_sendticket_share').' WHERE uniacid = '.intval($_W['uniacid']).' AND enough = "'.$_GPC['enough'].'"';
                ...
                $rathersql = 'SELECT * FROM '.tablename('vending_machine_sendticket_share').' WHERE uniacid = '.intval($_W['uniacid']).' AND enough = "'.$_GPC['enough'].'" AND id <> '.intval($id);
                $ratherlist = pdo_fetchall($rathersql);
                if(!empty($ratherlist)){
                    show_json(0,'满额发放的金额已存在，请填写其他金额！');
                }
            ...
            if ($_GPC['issync'] == 0) {
                    ...
                    if (!empty($_GPC['couponid']) && is_array($_GPC['couponid'])) {
                        ...
                        foreach ($_GPC['couponid'] as $ck => $cv) {
                            ...
                            $csql = 'SELECT * FROM '.tablename('vending_machine_coupon').' WHERE uniacid = '.intval($_W['uniacid']).' AND id = '.$cv;
...
//            dump($data);exit;
            ...
            if (!empty($id)) {
                pdo_update('vending_machine_sendticket_share', $data, array('id' => $id));
                plog('sale.sendticket.share.edit', "编辑分享活动 ID: {$id} <br/>分享名称: {$data['sharetitle']}");
            } else {
                pdo_insert('vending_machine_sendticket_share', $data);
                $id = pdo_insertid();
        ...
        $item = pdo_fetch("SELECT * FROM ".tablename('vending_machine_sendticket_share')." WHERE uniacid = ".$uniacid." and id = ".$id." ");
        if(!empty($item['thumb'])){
    ...
    function status() {
        ...
        $items = pdo_fetchall("SELECT id,sharetitle FROM " . tablename('vending_machine_sendticket_share') . " WHERE id in( $id ) AND uniacid=" . $_W['uniacid']);
        foreach ($items as $item) {
            pdo_update('vending_machine_sendticket_share', array('status' => intval($_GPC['status'])), array('id' => $item['id']));
    ...
    function delete1() {
        ...
        foreach ($items as $item) {
            pdo_delete('vending_machine_sendticket_share', array('id' => $item['id']));
    ...
    function change() {
        ...
        $gift = pdo_fetch('select id from ' . tablename('vending_machine_sendticket_share') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
        if (empty($gift)) {
            show_json(0, array('message' => '参数错误'));
        }

        pdo_update('vending_machine_sendticket_share', array($type => $value), array('id' => $id));
        show_json(1);
    }
    ...
    function querycoupon($couponid){
        ...
        foreach ($couponid as $ck => $cv) {
            ...
            $cpsql = 'SELECT * FROM '.tablename('vending_machine_coupon').$where;
