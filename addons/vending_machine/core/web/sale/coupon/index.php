...
//        ini_set('error_reporting', E_ALL);
        ...
        $sql = 'SELECT * FROM ' . tablename('vending_machine_coupon') . " "
            . " where  1 and {$condition} ORDER BY displayorder DESC,id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize;

        $list = pdo_fetchall($sql, $params);
        foreach ($list as &$row) {
            $row['gettotal'] = pdo_fetchcolumn('select count(*) from ' . tablename('vending_machine_coupon_data') . ' where couponid=:couponid and uniacid=:uniacid limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid']));
            $row['usetotal'] = pdo_fetchcolumn('select count(*) from ' . tablename('vending_machine_coupon_data') . ' where used = 1 and couponid=:couponid and uniacid=:uniacid limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid']));
            $row['pwdjoins'] = pdo_fetchcolumn('select count(*) from ' . tablename('vending_machine_coupon_guess') . ' where couponid=:couponid and uniacid=:uniacid limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid']));
            $row['pwdoks'] = pdo_fetchcolumn('select count(*) from ' . tablename('vending_machine_coupon_guess') . ' where couponid=:couponid and uniacid=:uniacid and ok=1 limit 1', array(':couponid' => $row['id'], ':uniacid' => $_W['uniacid']));
        ...
        unset($row);
        $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('vending_machine_coupon') . " where 1 and {$condition}", $params);
        $pager = pagination2($total, $pindex, $psize);
        $category = pdo_fetchall('select * from ' . tablename('vending_machine_coupon_category') . ' where uniacid=:uniacid and merchid=0 order by id desc', array(':uniacid' => $_W['uniacid']), 'id');
    ...
    protected function post() {
        ...
        if ($_W['ispost']) {
            ...
            if (!empty($id)) {
                ...
                if (!empty($data['pwdkey2'])) {
                    $pwdkey2 = pdo_fetch("SELECT pwdkey2 FROM " . tablename('vending_machine_coupon') . " WHERE id=:id and uniacid=:uniacid limit 1 ", array(':id' => $id, ':uniacid' => $_W['uniacid']));
                ...
                pdo_update('vending_machine_coupon', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
                ...
                pdo_insert('vending_machine_coupon', $data);
                $id = pdo_insertid();
                plog('sale.coupon.add', "添加优惠券 ID: {$id}  <br/>优惠券名称: {$data['couponname']}");
            }

            $key = "vending_machine:com:coupon:" . $id;
            $rule = pdo_fetch("select * from " . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'vending_machine', ':name' => $key));
            if (!empty($data['pwdkey2'])) {
                //创建回复关键词
                if (empty($rule)) {
                    $rule_data = array(
        ...
        $item = pdo_fetch("SELECT * FROM " . tablename('vending_machine_coupon') . " WHERE id =:id and uniacid=:uniacid and merchid=0 limit 1", array(':uniacid' => $_W['uniacid'], ':id' => $id));
        if (empty($item)) {
            $starttime = time();
            ...
            if($item['limitgoodtype']==1||$item['limitgoodtype']==2)
            {
                if($item['limitgoodids']){
                    $goods = pdo_fetchall("SELECT id,title,thumb FROM ".tablename('vending_machine_goods')." WHERE uniacid = :uniacid and id in ({$item['limitgoodids']}) ",array(':uniacid'=>$_W['uniacid']));
                }
            }
    ...
    function delete() {
        ...
        $items = pdo_fetchall("SELECT id,couponname FROM " . tablename('vending_machine_coupon') . " WHERE id in( $id ) and merchid=0 AND uniacid=" . $_W['uniacid']);
        ...
        foreach ($items as $item) {
            pdo_delete('vending_machine_coupon', array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
            pdo_delete('vending_machine_coupon_data', array('couponid' => $item['id'], 'uniacid' => $_W['uniacid']));
    ...
    function displayorder() {
        ...
        $items = pdo_fetchall("SELECT id,couponname FROM " . tablename('vending_machine_coupon') . " WHERE id in( $id ) and merchid=0 AND uniacid=" . $_W['uniacid']);
        foreach ($items as $item) {
            pdo_update('vending_machine_coupon', array('displayorder' => $displayorder), array('id' => $id));
    ...
    function query() {
        ...
        $time = time();
        if($merch){
            $ds = pdo_fetchall("SELECT c.*,m.merchname FROM " . tablename('vending_machine_coupon') . " as c
                 left join ".tablename('vending_machine_merch_user')." as m on m.id = c.merchid
                 WHERE 1 {$condition} ORDER BY c.id asc", $params);
        }else {
            $ds = pdo_fetchall("SELECT * FROM " . tablename('vending_machine_coupon') . "  WHERE 1 {$condition} ORDER BY id asc", $params);
        }
        if(!empty($ds)){
            foreach ($ds as &$d) {
    ...
    function querycoupons(){
        ...
        $ds = pdo_fetchall('SELECT id,couponname as title , thumb FROM ' . tablename('vending_machine_coupon') . " WHERE 1 {$condition} order by createtime desc", $params);
    ...
    function send() {
        ...
        $coupon = pdo_fetch('SELECT * FROM ' . tablename('vending_machine_coupon') . ' WHERE id=:id and uniacid=:uniacid and merchid=0', array(':id' => $couponid, ':uniacid' => $_W['uniacid']));
        ...
        if ($_W['ispost']) {
            ...
            if ($class1 == 1) {
                ...
                $members = pdo_fetchall("SELECT openid FROM " . tablename('vending_machine_member') . " WHERE uniacid = '{$_W['uniacid']}'" . $where, array(), 'openid');
                if (!empty($_GPC['send_level'])) {
                    $levelname = pdo_fetchcolumn('select levelname from ' . tablename('vending_machine_member_level') . ' where id=:id limit 1', array(':id' => $_GPC['send_level']));
                } else {
                    $levelname = "全部";
                }
                ...
                $members = pdo_fetchall("SELECT openid FROM " . tablename('vending_machine_member') . " WHERE uniacid = '{$_W['uniacid']}'" . $where, array(), 'openid');
                if (!empty($_GPC['send_group'])) {
                    $groupname = pdo_fetchcolumn('select groupname from ' . tablename('vending_machine_member_group') . ' where id=:id limit 1', array(':id' => $_GPC['send_group']));
                } else {
                    $groupname = "全部分组";
                }
                $openids = array_keys($members);
                ...
                $members = pdo_fetchall("SELECT openid FROM " . tablename('vending_machine_member') . " WHERE uniacid = '{$_W['uniacid']}' and isagent=1 and status=1 " . $where, array(), 'openid');
                if ($_GPC['send_agentlevel'] != '') {
                    $levelname = pdo_fetchcolumn('select levelname from ' . tablename('vending_machine_commission_level') . ' where id=:id limit 1', array(':id' => $_GPC['send_agentlevel']));
                } else {
                    $levelname = "全部";
                }
                ...
                $members = pdo_fetchall("SELECT openid FROM " . tablename('vending_machine_member') . " WHERE uniacid = '{$_W['uniacid']}' and ispartner=1 and partnerstatus=1 " . $where, array(), 'openid');
                if ($_GPC['send_partnerlevels'] != '') {
                    $levelname = pdo_fetchcolumn('select levelname from ' . tablename('vending_machine_globonus_level') . ' where id=:id limit 1', array(':id' => $_GPC['send_partnerlevels']));
                } else {
                    $levelname = "全部";
                }
                ...
                $members = pdo_fetchall("SELECT openid FROM " . tablename('vending_machine_member') . " WHERE uniacid = '{$_W['uniacid']}' and isaagent=1 and aagentstatus=1 " . $where, array(), 'openid');
                if ($_GPC['send_aagentlevels'] != '') {
                    $levelname = pdo_fetchcolumn('select levelname from ' . tablename('vending_machine_abonus_level') . ' where id=:id limit 1', array(':id' => $_GPC['send_aagentlevels']));
                } else {
                    $levelname = "全部";
                }
            ...
            $members = pdo_fetchall('select id,openid,nickname from ' . tablename('vending_machine_member') . ' where openid in (' . implode(',', $mopenids) . ") and uniacid={$_W['uniacid']}");
            if (empty($members)) {
                show_json(0, '未找到发送的会员!');
            }
            ...
            pdo_update('vending_machine_coupon', $upgrade, array('id' => $coupon['id']));
        ...
        $coupons = pdo_fetchall("SELECT * FROM " . tablename('vending_machine_coupon') . " WHERE uniacid = '{$_W['uniacid']}' ORDER BY id asc");
