...
class Pluginsetting_Page extends SystemPage {

	function main() {
		global $_W, $_GPC;
        $data = pdo_fetch("select * from ".tablename('vending_machine_system_plugingrant_setting')." where 1 = 1 limit 1 ");
        load()->model('user');
        ...
        if ($_W['ispost']) {
            ...
            if(!empty($data)){
                pdo_update("vending_machine_system_plugingrant_setting",$data_set,array('id'=>$data['id']));
                plog('system.plugin.pluginsetting.edit', '修改授权管理基本设置');
            }else{
                //show_json(0, pdo_insert("vending_machine_system_plugingrant_setting",$data_set));
                pdo_insert("vending_machine_system_plugingrant_setting",$data_set);
