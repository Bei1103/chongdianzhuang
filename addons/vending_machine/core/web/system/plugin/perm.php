...
class Perm_Page extends SystemPage {
	...
	function main() {
		...
		if ($permset) {
			...
			$list = pdo_fetchall("SELECT p.*,ac.name FROM " . tablename('vending_machine_perm_plugin') . " p  "
			...
			foreach ($list as &$row) {
				...
				if (!empty($aplugins)) {
					$row['plugins'] = pdo_fetchall('select name from ' . tablename('vending_machine_plugin') . ' where identity in (' . implode(',', $aplugins) . ') and status=1');
				} else {
					$row['plugins'] = array();
				}
				...
				if (!empty($acoms)) {
					$row['coms'] = pdo_fetchall('select name from ' . tablename('vending_machine_plugin') . ' where identity in (' . implode(',', $acoms) . ') and status=1');
				} else { 
					$row['coms'] = array();
				}
			...
			$total = pdo_fetchcolumn("SELECT count(*) FROM " . tablename('vending_machine_perm_plugin') . " p  "
	...
	protected function post() {
		...
		$id = intval($_GPC['id']);
		$item = pdo_fetch("SELECT * FROM " . tablename('vending_machine_perm_plugin') . " WHERE id =:id limit 1", array(':id' => $id));
		$item_plugins = array();
		$item_coms = array();
		if (!empty($item)) {
		...
		if ($_W['ispost']) {
			...
			if (!empty($id)) {
				pdo_update('vending_machine_perm_plugin', $data, array('id' => $id));
			} else {

				$wechatcount = pdo_fetchcolumn('select count(*) from ' . tablename('vending_machine_perm_plugin') . ' where acid=:acid limit 1', array(':acid' => $data['acid']));
				if ($wechatcount > 0) {
					show_json(0, '此公众号的插件权限已经存在!', '', 'error');
				}

				pdo_insert('vending_machine_perm_plugin', $data);
				$id = pdo_insertid();
			}
	...
	function delete() {
		global $_W, $_GPC;
		$id = intval($_GPC['id']);
		$item = pdo_fetch("SELECT id FROM " . tablename('vending_machine_perm_plugin') . " WHERE id = '$id'");
		if (empty($item)) {
			message('抱歉，权限设置不存在或是已经被删除！', webUrl('perm/plugins', array('op' => 'display')), 'error');
		}
		pdo_delete('vending_machine_perm_plugin', array('id' => $id));
		show_json(1, array('url' => webUrl('system/plugin/perm')));
	}
