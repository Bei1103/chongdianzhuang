...
class Export_Page extends WebPage {
    ...
    public function main() {
        ...
        if ($_GPC['export'] == 1) {
            ...
            if ($status != '') {
                ...
                $sql = "select o.* , a.realname as arealname,m.level,a.mobile as amobile,a.province as aprovince ,a.city as acity,a.area as aarea, a.street as astreet,a.address as aaddress,d.dispatchname,m.nickname,m.id as mid,m.realname as mrealname,m.mobile as mmobile,m.groupid,m.uid,sm.id as salerid,sm.nickname as salernickname,s.salername from " . tablename('vending_machine_order') . " o"
                    ...
                    . " left join " . tablename('vending_machine_member') . " sm on sm.openid = o.verifyopenid and sm.uniacid=o.uniacid"
            ...
            //查询是否有用户会员等级的设置。
            $userlevel = pdo_fetchall("SELECT * FROM ".tablename('vending_machine_member_level')." WHERE uniacid=:uniacid",array(':uniacid'=>$_W['uniacid']));
            if(!empty($userlevel)){
            ...
            $list_group = pdo_fetchall("SELECT * FROM " . tablename('vending_machine_member_group') . " WHERE uniacid=:uniacid",array(':uniacid'=>$_W['uniacid']),'id');
            ...
            foreach ($list as &$value) {
                ...
                if (!empty($value['storeid'])) {
                    if ($value['merchid'] > 0) {
                        $value['pickname'] = pdo_fetchcolumn('select storename from ' . tablename('vending_machine_merch_store') . ' where id=:storeid limit 1 ', array(':storeid' => $value['storeid']));
                    } else {
                        $value['pickname'] = pdo_fetchcolumn('select storename from ' . tablename('vending_machine_store') . ' where id=:storeid limit 1 ', array(':storeid' => $value['storeid']));
                    }
                }
                if (!empty($value['verifystoreid'])) {
                    if ($value['merchid'] > 0) {
                        $value['storeinfo'] = pdo_fetchcolumn('select storename from ' . tablename('vending_machine_merch_store') . ' where id=:storeid limit 1 ', array(':storeid' => $value['verifystoreid']));
                    } else {
                        $value['storeinfo'] = pdo_fetchcolumn('select storename from ' . tablename('vending_machine_store') . ' where id=:storeid limit 1 ', array(':storeid' => $value['verifystoreid']));
                    }
                }
        ...
        $stores = pdo_fetchall('select id,storename from ' . tablename('vending_machine_store') . ' where uniacid=:uniacid ', array(':uniacid' => $_W['uniacid']));
