...
class Area_Page extends WebPage
{
...
//        print_r($data);exit;
        ...
        foreach($data as $k => $v) {
            if (!empty($v)) {
                pdo_insert("vending_machine_city2", $v);
            }
        }
    ...
    function get_area($code)
    {
        ...
        $sql = "select * from " . tablename('vending_machine_city') . " where  code=:code and status=1 Limit 1";
        ...
        if (!empty($data)) {
            foreach($data as $k => $v) {
                if (!empty($v)) {
                    ...
                    if (empty($item2)) {
                        pdo_insert("vending_machine_city", $v);
                    }
                }
            }
...
//            print_r($l3 ."\n");
    ...
    function count()
    {
        ...
        foreach($list as $k => $v) {
            ...
            $sql = "select count(1) from " . tablename('vending_machine_city') . " where  code=:code";
...
//        $sql = "select * from " . tablename('vending_machine_city') . " where  level=:level and status=1 and noarea=1 order by id";
//        $sql .= " Limit 3";
        $sql = "select * from " . tablename('vending_machine_city') . " where  level=:level and status=1";
    ...
    function get_child_list($pcode)
    {
        ...
        $sql = "select * from " . tablename('vending_machine_city') . " where  pcode=:pcode and status=1 order by id";
        $list = pdo_fetchall($sql, $params);
...
//            print_r($xml);exit;
            ...
            $styles = array();
            $dir = IA_ROOT . "/addons/vending_machine/static/js/dist/area/list/" .$left."/";
...
//                pdo_update('vending_machine_city', array('noarea' => 1), array('id' => $v2['id']));
