...
class Detail_Page extends WebPage {
    ...
    function main() {
        ...
        if($item['sendtype']>0){
            ...
            foreach($order_goods as $key => $value){
                $order_goods[$key]['goods'] = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell,og.realprice from ' . tablename('vending_machine_order_goods') . ' og '
        ...
        if (empty($item['addressid'])) {
            ...
            if($item['storeid']<>0 && $item['ismerch'] == 0){
                $user['storename'] = pdo_fetch("SELECT storename FROM " . tablename('vending_machine_store') . " WHERE id = :id and uniacid=:uniacid and status=1", array(':id' => $item['storeid'], ':uniacid' => $_W['uniacid']));
            }
        } else {
        ...
        $goods = pdo_fetchall("SELECT op.id as option_id, o.fullbackid,op.specs,g.*,o.title as gtitle, o.goodssn as option_goodssn, o.productsn as option_productsn,o.total,g.type,o.optionname,o.optionid,o.price as orderprice,o.realprice,o.changeprice,o.oldprice,o.commission1,o.commission2,o.commission3,o.commissions,o.seckill,o.seckill_taskid,o.seckill_roomid,o.nocommission,o.single_refundstate{$diyformfields} FROM " . tablename('vending_machine_order_goods') .
            ...
            ." left join " . tablename('vending_machine_goods_option') . " op on o.optionid=op.id "
...
//                $option_title = pdo_fetch('SELECT title FROM'.tablename('vending_machine_goods_spec_item').' WHERE uniacid = '.intval($_W['uniacid']).' AND id = '.intval($r['specs']));
//                $r['optionname'] = $option_title['title'];
        ...
        if(!empty($fullbackid)){
            $info = pdo_fetchall("SELECT * FROM". tablename("vending_machine_fullback_log")."where orderid=:orderid and uniacid=:uniacid", array('orderid'=>$id,'uniacid'=>$_W['uniacid']));
            $allday = array();
            $fullbackday = array();
            $createtime = array();
        ...
        if (com('verify')) {
            ...
            if (!empty($item['verifyopenid'])) {
                ...
                if (empty($item['merchid'])) {
                    ...
                    $saler['salername'] = pdo_fetchcolumn('select salername from ' . tablename('vending_machine_merch_saler') . ' where openid=:openid and merchid=:merchid and uniacid=:uniacid limit 1 ', array(':uniacid' => $_W['uniacid'], ':openid' => $item['verifyopenid'], ':merchid' => $item['merchid']));
                }
            }
            if (!empty($item['verifystoreid'])) {
                if (empty($item['merchid'])) {
                    ...
                    $store = pdo_fetch('select * from ' . tablename('vending_machine_merch_store') . ' where id=:storeid limit 1 ', array(':storeid' => $item['verifystoreid']));
                }
            }
            if(!empty($item['storeid'])){
                if (empty($item['merchid'])) {
                    $goodsstore = pdo_fetch('select * from ' . tablename('vending_machine_store') . ' where id=:storeid limit 1 ', array(':storeid' => $item['storeid']));
                } else {
                    $goodsstore = pdo_fetch('select * from ' . tablename('vending_machine_merch_store') . ' where id=:storeid limit 1 ', array(':storeid' => $item['storeid']));
                }
            }
                        ...
                        foreach ($verifyinfo as &$v) {
                            if ($v['verified'] || $item['verifytype']==1) {
                                if (empty($item['merchid'])) {
                                    ...
                                    $v['storename'] = pdo_fetchcolumn('select storename from ' . tablename('vending_machine_merch_store') . " where id=:id limit 1", array(':id' => $v['verifystoreid']));
                                }
                                ...
                                if (empty($item['merchid'])) {
                                    ...
                                    $v['salername'] = pdo_fetchcolumn('select salername from ' . tablename('vending_machine_merch_saler') . " where openid=:openid and merchid=:merchid and uniacid=:uniacid limit 1", array(':openid' => $v['verifyopenid'], ':uniacid' => $_W['uniacid'], ':merchid' => $item['merchid']));
                                }
        ...
        if($isonlyverifygoods){
            $sql = 'select vg.*,vgl.verifydate,vgl.verifynum,s.storename,sa.salername,vgl.remarks  from ' . tablename('vending_machine_verifygoods_log') . '   vgl
            ...
            $verifygood =   pdo_fetch('select *  from ' . tablename('vending_machine_verifygoods') . '    where orderid =:orderid limit 1  ', array(':orderid' => $item['id']));
        ...
        if(!empty($item['headsid']) && !empty($item['dividend'])){
            ...
            $heads=pdo_fetch('select * from ' . tablename('vending_machine_member') . " where id=:id and uniacid=:uniacid limit 1", array(':id' => $item['headsid'], ':uniacid' => $_W['uniacid']));
        }
        //判断是否使用会员卡
    ...
    public function option_title($spec){
        ...
        $specs =  ($spec);
//        //查出spec_item的标题
//        $sql = "select o.specs from " . tablename('vending_machine_order_goods')
//            . " g join " . tablename('vending_machine_goods_option')
...
//        $specs =  pdo_fetchcolumn($sql, array(':orderid'=>$orderid));
        ...
        $table = tablename('vending_machine_goods_spec_item');
