...
class Op_Page extends WebPage {
    ...
    function delete() {
        ...
        pdo_update('vending_machine_order', array('deleted' => 1), array('id' => $orderid, 'uniacid' => $_W['uniacid']));
    ...
    protected function opData() {
        ...
        global $_W, $_GPC;
        $id = intval($_GPC['id']);
        $item = pdo_fetch("SELECT * FROM " . tablename('vending_machine_order') . " WHERE id = :id and uniacid=:uniacid", array(':id' => $id, ':uniacid' => $_W['uniacid']));
        if (empty($item)) {
            if ($_W['isajax']) {
                show_json(0, "未找到订单!");
            }
        ...
        $order_goods = pdo_fetchall('select single_refundstate from ' . tablename('vending_machine_order_goods'). ' where orderid=:orderid and uniacid=:uniacid', array(':uniacid' => $_W['uniacid'], ':orderid' => $item['id']));
    ...
    function changeprice() {
        ...
        if ($_W['ispost']) {
            ...
            foreach ($changegoodsprice as $ogid => $change) {
                $og = pdo_fetch('select price,realprice from ' . tablename('vending_machine_order_goods') . ' where id=:ogid and uniacid=:uniacid limit 1', array(':ogid' => $ogid, ':uniacid' => $_W['uniacid']));
                if (!empty($og)) {
            ...
            if (!empty($orderupdate)) {
                pdo_update('vending_machine_order', $orderupdate, array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
            }
            ...
            //修改商品价
            foreach ($changegoodsprice as $ogid => $change) {
                $og = pdo_fetch('select price,realprice,changeprice from ' . tablename('vending_machine_order_goods') . ' where id=:ogid and uniacid=:uniacid limit 1', array(':ogid' => $ogid, ':uniacid' => $_W['uniacid']));
                if (!empty($og)) {
                    $realprice = $og['realprice'] + $change; //这次的变化
                    $changeprice = $og['changeprice'] + $change; //累计的变化
                    pdo_update('vending_machine_order_goods', array('realprice' => $realprice, 'changeprice' => $changeprice), array('id' => $ogid));
                }
            }
        ...
        if (empty($item['addressid'])) {
            ...
            if (!is_array($user)) {
                $user = pdo_fetch("SELECT * FROM " . tablename('vending_machine_member_address') . " WHERE id = :id and uniacid=:uniacid", array(':id' => $item['addressid'], ':uniacid' => $_W['uniacid']));
            }
    ...
    function pay($a = array(), $b = array()) {
        ...
        if (!empty($item['virtual']) && com('virtual')) {
            ...
            //确认付款先改状态，再设置库存
            pdo_update('vending_machine_order', $updateorder, array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
    ...
    function close() {
        ...
        if ($_W['ispost']) {
            if(!empty($item['payscore_order_id'])){//取消支付分订单
                $res= p('payscore')->serviceorder_cancel(array('out_order_no'=>$item['ordersn'],'reason'=>'平台后台关闭'));
                if(empty($res)){
                    show_json(0, '支付分订单取消失败!');
                }
            }
            ...
            if ($item['refundstate'] > 0 && !empty($item['refundid'])) {
                ...
                pdo_update('vending_machine_order_refund', $change_refund, array('id' => $item['refundid'], 'uniacid' => $_W['uniacid']));
            }
            ...
            pdo_update('vending_machine_order', array('status' => -1, 'refundstate' => 0, 'canceltime' => $time, 'remarkclose' => $_GPC['remark']), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));

            if (!empty($item['virtual']) && $item['virtual'] != 0) {

                $goodsid = pdo_fetch('SELECT goodsid FROM '.tablename('vending_machine_order_goods').' WHERE uniacid = '.$_W['uniacid'].' AND orderid = '.$item['id']);
                ...
                //未付款卡密变为未使用
                pdo_query("update " . tablename('vending_machine_virtual_data') . ' set openid="",usetime=0,orderid=0,ordersn="",price=0,merchid='.$item['merchid'].' where typeid=' . intval($typeid).' and orderid = '.$item["id"]);

                //模板减少使用数据
                pdo_query("update " . tablename('vending_machine_virtual_type') . " set usedata=usedata-".$vkeynum." where id=" .intval($typeid));
        ...
        if(!empty($item['payscore_order_id'])){//取消支付分订单
            if($item['price']<=0) {
                ...
                if (is_error($res)||empty($res['order_id'])) {
                    show_json(0, '支付分订单取消失败!'.$res['message']);
                }
            }else{
                $order_goods = pdo_fetchall("SELECT * FROM " . tablename('vending_machine_order_goods') . " WHERE uniacid = :uniacid and orderid=:orderid and totalprice>0", array(':orderid' => $item['id'], ':uniacid' => $_W['uniacid']));
                $post_payments = array();
                foreach ($order_goods as $goods) {
                    $goods['title'] = substr($goods['id'] . '-' . $goods['title'], 0, 20);
                ...
                //完结并扣款支付分订单
                $res = p('payscore')->serviceorder_complete(array('out_order_no' => $item['ordersn'], 'post_payments' => $post_payments, 'total_amount' => round($item['price'] * 100)));
                if (is_error($res)||empty($res['order_id'])||$res['state'] != 'DONE') {
                    show_json(0, '支付分订单扣款失败!'.$res['message']);
                }
            }
        }
        show_json(0, '支付分订单扣款失败!'.$res['message']);
        ...
        if (!empty($item['wxapp_allow_subscribe'])) {
            ...
            if (in_array('receive', $template)) {
                ...
                if (strexists('sns_wa_', $item['openid'])) {
                    ...
                    $openid = pdo_fetchcolumn("select openid_wa from ".tablename('vending_machine_member')." where openid='{$item['openid']}'");
                }
    ...
    function sendcancel() {
        ...
        if ($_W['ispost']) {
            ...
            if($item['sendtype']>0){
                ...
                $data['sendtype'] = 0;
                pdo_update('vending_machine_order_goods', $data, array('orderid' => $item['id'],'sendtype'=>$sendtype, 'uniacid' => $_W['uniacid']));
                $order = pdo_fetch("select sendtype from ".tablename('vending_machine_order')." where id = ".$item['id']." and uniacid = ".$_W['uniacid']." ");
                pdo_update('vending_machine_order', array('sendtype'=>$order['sendtype']-1,'status'=>1), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
            }else{
                $data['status'] = 1;
                pdo_update('vending_machine_order', $data, array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
            }
        ...
        $bundles = array();
        if($sendtype>0){
            $sendgoods = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('vending_machine_order_goods') . ' og '
                . ' left join ' . tablename('vending_machine_goods') . ' g on g.id=og.goodsid '
            ...
            if($item['sendtype']>0){
                for($i=1;$i<=intval($item['sendtype']);$i++){
                    $bundles[$i]['goods'] = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('vending_machine_order_goods') . ' og '
                        . ' left join ' . tablename('vending_machine_goods') . ' g on g.id=og.goodsid '
    ...
    function fetch() {
        ...
        pdo_update(
            'vending_machine_order', $d, array('id' => $item['id'], 'uniacid' => $_W['uniacid'])
        );
        ...
        if (!empty($item['refundid'])) {
            $refund = pdo_fetch('select * from ' . tablename('vending_machine_order_refund') . ' where id=:id limit 1', array(':id' => $item['refundid']));
            if (!empty($refund)) {
                pdo_update('vending_machine_order_refund', array('status' => -1), array('id' => $item['refundid']));
                pdo_update('vending_machine_order', array('refundstate' => 0), array('id' => $item['id']));
            }
        }
    ...
    function send() {
        ...
        if ($_W['ispost']) {
            ...
            if($item['city_express_state']==0){
                    ...
                    if(intval($_GPC['sendtype'])==1 || $item['sendtype'] > 0){
                        ...
                        $ogoods = array();
                        $ogoods = pdo_fetchall("select sendtype from ".tablename('vending_machine_order_goods')."
                        ...
                        foreach($goodsid as $key => $value){
                            pdo_update('vending_machine_order_goods', $data, array('id'=>$value, 'uniacid' => $_W['uniacid']));
                        }
                        //查询是否有未发货包裹
                        $send_goods = pdo_fetch("select * from ".tablename('vending_machine_order_goods')."
                        ...
                        $senddata['refundid'] =0 ;
                        pdo_update('vending_machine_order', $senddata, array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
                    }else{
                        $data['status'] = 2;
                        $data['refundid'] = 0;
                        pdo_update('vending_machine_order', $data, array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
            ...
            if (!empty($item['refundid'])) {
                $refund = pdo_fetch('select * from ' . tablename('vending_machine_order_refund') . ' where id=:id limit 1', array(':id' => $item['refundid']));
                if (!empty($refund)) {
                    pdo_update('vending_machine_order_refund', array('status' => -1, 'endtime' => $time), array('id' => $item['refundid']));
                    pdo_update('vending_machine_order', array('refundstate' => 0), array('id' => $item['id']));
                }
            }
        ...
        if($item['sendtype']>0){
            //未发货商品
            $noshipped = pdo_fetchall('select og.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('vending_machine_order_goods') . ' og '
                . ' left join ' . tablename('vending_machine_goods') . ' g on g.id=og.goodsid '
            ...
            for($i=1;$i<=$item['sendtype'];$i++){
                ...
                $shipped[$i]['goods'] = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('vending_machine_order_goods') . ' og '
                    . ' left join ' . tablename('vending_machine_goods') . ' g on g.id=og.goodsid '
        ...
        $order_goods = pdo_fetchall('select og.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('vending_machine_order_goods') . ' og '
            . ' left join ' . tablename('vending_machine_goods') . ' g on g.id=og.goodsid '
        ...
        if (!is_array($address)) {
            $address = pdo_fetch("SELECT * FROM " . tablename('vending_machine_member_address') . " WHERE id = :id and uniacid=:uniacid", array(':id' => $item['addressid'], ':uniacid' => $_W['uniacid']));
        }
    ...
    function remarksaler() {
        ...
        if ($_W['ispost']) {
            pdo_update('vending_machine_order', array('remarksaler' => $_GPC['remark']), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
        ...
        if ($_W['ispost']) {
            ...
            if (!empty($expresssn)) {
                ...
                if($item['sendtype']>0){
                    ...
                    pdo_update('vending_machine_order_goods', $change_data, array('orderid' => $id,'sendtype'=>$sendtype, 'uniacid' => $_W['uniacid']));
                }else{
                    pdo_update('vending_machine_order', $change_data, array('id' => $id, 'uniacid' => $_W['uniacid']));
                }
    ...
    function changeaddress() {
        ...
        if ($_W['ispost']) {
            ...
            if (!empty($id)) {
                ...
                $item = pdo_fetch("SELECT id, ordersn, address,openid FROM " . tablename('vending_machine_order') . " WHERE id = :id and uniacid=:uniacid", array(':id' => $id, ':uniacid' => $_W['uniacid']));
                ...
                pdo_update('vending_machine_order', array('address' => $address_array), array('id' => $id, 'uniacid' => $_W['uniacid']));
    ...
    function upload_invoice(){
        ...
        if (!$_W['ispost']){
            $invoice = pdo_fetch('select invoicename,invoice_img from '.tablename('vending_machine_order').' where id = :order_id and uniacid = :uniacid limit 1',array(':order_id'=>$order_id, ':uniacid'=>$_W['uniacid']));
        ...
        $update_ret = pdo_update('vending_machine_order', array('invoice_img'=>$invoice_img), array('id'=>$order_id));
    ...
    function peerpay() {
        ...
        $sql = "SELECT * FROM ".tablename('vending_machine_order_peerpay_payinfo')." where pid=:pid";
    ...
    function payscore_complete()
    {
        global $_W, $_GPC;
        $opdata = $this->opData();
        extract($opdata);

        if(!empty($item['payscore_order_id'])||1){//取消支付分订单
            if($item['price']<=0) {
                $res = p('payscore')->serviceorder_cancel(array('out_order_no' => $item['ordersn'], 'reason' => '平台后台关闭'));
                if (empty($res)) {
                    show_json(0, '支付分订单取消失败!');
