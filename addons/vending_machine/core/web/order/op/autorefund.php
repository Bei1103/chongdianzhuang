...
class Autorefund_Page extends WebPage
{
    protected function globalData()
    {
        ...
        $order = pdo_fetch("select id,status,price,refundid,goodsprice,dispatchprice,deductprice,deductcredit2,finishtime,isverify,`virtual`,refundstate,merchid,random_code,iscycelbuy,paytype from " . tablename('vending_machine_order') . ' where id=:id and uniacid=:uniacid  limit 1'
            , array(':id' => $orderid, ':uniacid' => $uniacid,));
        $orderprice = $order['price'];

        if ($order['iscycelbuy'] == 1) {
            //查询分期订单下面是否存在有开始的周期商品
            $order_goods = pdo_fetch("select * from " . tablename('vending_machine_cycelbuy_periods') . "where orderid = {$order['id']} and status != 0");
            if (!empty($order_goods)) {
                show_json(0, '订单已经开始，无法进行退款');
            }
        }
        $refund = pdo_fetch("select id,orderid from " . tablename('vending_machine_order_refund') . " where uniacid = :uniacid and orderid = :orderid and status > 0 limit 1 ", array(':uniacid' => $uniacid, ':orderid' => $orderid));
        if (!empty($refund)) {
        ...
        if ($order['status'] >= 1) {
            $goods = pdo_fetchall("select og.goodsid, og.price, og.total, og.optionname, g.cannotrefund,g.refund,g.returngoods,g.exchange,g.type, g.thumb, g.title,g.isfullback from" . tablename("vending_machine_order_goods") . " og left join " . tablename("vending_machine_goods") . " g on g.id=og.goodsid where og.orderid=" . $order['id']);
            if (!empty($goods)) {
                foreach ($goods as $g) {
                    /*
                     * 退款优化V1.10
        ...
        //是否全返商品，并检测是否允许退款
        $fullback_log = pdo_fetchall("select * from " . tablename('vending_machine_fullback_log') . " where orderid = " . $orderid . " and uniacid = " . $uniacid . " ");
        $fullbackkprice = 0;
        if ($fullback_log && is_array($fullback_log)) {
            foreach ($fullback_log as $key => $value) {
                $fullbackgoods = pdo_fetch("select refund from " . tablename('vending_machine_fullback_goods') . " where goodsid = " . $value['goodsid'] . " and uniacid = " . $uniacid . " ");
    ...
    public function submit()
    {
        ...
        if ($order['refundstate'] == 0) {
            ...
            pdo_insert('vending_machine_order_refund', $refund);
            $refundid = pdo_insertid();
            pdo_update('vending_machine_order', array('refundid' => $refundid, 'refundstate' => $refundstate), array('id' => $orderid, 'uniacid' => $uniacid));
        } else {
            //修改退款申请
            pdo_update('vending_machine_order', array('refundstate' => $refundstate), array('id' => $orderid, 'uniacid' => $uniacid));
            pdo_update('vending_machine_order_refund', $refund, array('id' => $refundid, 'uniacid' => $uniacid));
        }
        //模板消息
    ...
    private function setRefund($orderid,$refundid)
    {
        ...
        $item = pdo_fetch("SELECT * FROM " . tablename('vending_machine_order') . " WHERE id = :id and uniacid=:uniacid Limit 1", array(':id' => $id, ':uniacid' => $_W['uniacid']));
//        dump($item);
        if (empty($item)) {
            if ($_W['isajax']) {
                show_json(0, "未找到订单!");
        ...
        if (!empty($refundid)) {
            $refund = pdo_fetch('select * from ' . tablename('vending_machine_order_refund') . ' where id=:id limit 1', array(':id' => $refundid));
        ...
        $r_type = array( '0' => '退款', '1' => '退货退款', '2' => '换货');
            ...
            if ($refund['status'] < 0 || $refund['status'] == 1) {
                pdo_update('vending_machine_order', array('refundstate' => 0), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
                show_json(0,'未找需要处理的维权申请，不需处理！');
            }

            if (empty($refund['refundno'])) { //退款单号
                $refund['refundno'] = m('common')->createNO('order_refund', 'refundno', 'SR');
                pdo_update('vending_machine_order_refund', array('refundno' => $refund['refundno']), array('id' => $refund['id']));
            }
            ...
            if ($refundstatus == 0) {
                ...
                if ($raid == 0) {
                    $raddress = pdo_fetch('select * from '.tablename('vending_machine_refund_address').' where isdefault=1 and uniacid=:uniacid and merchid=0 limit 1',array(':uniacid'=>$uniacid));
                } else {
                    $raddress = pdo_fetch('select * from '.tablename('vending_machine_refund_address').' where id=:id and uniacid=:uniacid and merchid=0 limit 1',array(':id'=>$raid,':uniacid'=>$uniacid));
                }

                if (empty($raddress)) {
                    $raddress = pdo_fetch('select * from '.tablename('vending_machine_refund_address').' where uniacid=:uniacid and merchid=0 order by id desc limit 1',array(':uniacid'=>$uniacid));
                }
                ...
                pdo_update('vending_machine_order_refund', $change_refund, array('id' => $item['refundid']));
                ...
                pdo_update('vending_machine_order_refund', $refund_data, array('id'=>$item['refundid'], 'uniacid' => $uniacid));

                $order_data = array();
                $order_data['refundstate'] = 0;
                $order_data['status'] = 3;
                $order_data['refundtime'] = $time;
                pdo_update('vending_machine_order', $order_data, array('id'=>$item['id'], 'uniacid' => $uniacid));
                ...
                //订单号
                if ($item['parentid'] > 0) {
                    $parent_item = pdo_fetch("SELECT id,ordersn,ordersn2,price,transid,paytype,apppay FROM " . tablename('vending_machine_order') . " WHERE id = :id and uniacid=:uniacid Limit 1", array(':id' => $item['parentid'], ':uniacid' => $_W['uniacid']));
                ...
                //购物积分
                $goods = pdo_fetchall("SELECT g.id,g.credit, o.total,o.realprice,g.isfullback FROM " . tablename('vending_machine_order_goods') .
                    " o left join " . tablename('vending_machine_goods') . " g on o.goodsid=g.id " . " WHERE o.orderid=:orderid and o.uniacid=:uniacid", array(':orderid' => $item['id'], ':uniacid' => $uniacid));
                ...
                if ($item['paytype'] == 1) {
                    ...
                    if ($item['apppay']==2){
                        ...
                        if (!empty($ispeerpay)){
                            ...
                            $peerpaysql = "SELECT * FROM " . tablename('vending_machine_order_peerpay_payinfo') . " WHERE pid = :pid";
                            ...
                            if ($pay_refund_price > 0) {
                                // $res = pdo_get('vending_machine_order',array('ordersn'=>$ordersn,'uniacid'=>$_W['uniacid']),array('random_code'));
                                // $random_code = '';
...
//                }
                ...
                //更新订单退款状态
                pdo_update('vending_machine_order', array('refundstate' => 0, 'status' => -1, 'refundtime' => $time), array('id' => $item['id'], 'uniacid' => $uniacid));
                ...
                foreach ($goods as $g) {
                    ...
                    pdo_update('vending_machine_goods', array('salesreal' => $salesreal), array('id' => $g['id']));
                }
...
//                plog('order.op.refund.submit', $log);
                ...
                pdo_update('vending_machine_order_refund', array('reply' => $refundcontent, 'status' => -1, 'endtime' => $time), array('id' => $item['refundid']));

//                plog('order.op.refund.submit', "订单退款拒绝 ID: {$item['id']} 订单号: {$item['ordersn']} 原因: {$refundcontent}");

                //更新订单退款状态
                pdo_update('vending_machine_order', array('refundstate' => 0), array('id' => $item['id'], 'uniacid' => $uniacid));
                ...
                $goods = pdo_fetchall("SELECT g.id,g.credit, o.total,o.realprice FROM " . tablename('vending_machine_order_goods') .
                    " o left join " . tablename('vending_machine_goods') . " g on o.goodsid=g.id " . " WHERE o.orderid=:orderid and o.uniacid=:uniacid", array(':orderid' => $item['id'], ':uniacid' => $uniacid));
