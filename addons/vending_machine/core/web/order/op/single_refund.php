...
class Single_Refund_Page extends WebPage {
    ...
    protected function opData() {
        ...
        //订单商品 关联 订单表
        $item = pdo_fetch('select o.*,og.id as ordergoodsid,og.single_refundid,og.single_refundstate,og.single_refundtime from ' . tablename('vending_machine_order_goods') .' og '
        ...
        if (!empty($item['single_refundid'])) {
            $refund = pdo_fetch('select * from ' . tablename('vending_machine_order_single_refund') . ' where id=:id limit 1', array(':id' => $item['single_refundid']));
    ...
    function submit() {
        ...
        if ($_W['ispost']) {
            ...
            if ($refund['status'] < 0 || $refund['status'] == 1) {
                //pdo_update('vending_machine_order_goods', array('single_refundstate' => 0), array('id' => $item['ordergoodsid'], 'uniacid' => $_W['uniacid']));
                show_json(0,'未找到需要处理的维权申请，不需处理！');
            }

            if (empty($refund['refundno'])) { //退款单号
                $refund['refundno'] = m('common')->createNO('order_refund', 'refundno', 'SR');
                pdo_update('vending_machine_order_single_refund', array('refundno' => $refund['refundno']), array('id' => $refund['id']));
            }
            ...
            if ($refundstatus == 0) {
                ...
                pdo_update('vending_machine_order_single_refund', $change_refund, array('id' => $item['single_refundid']));
                ...
                pdo_update('vending_machine_order_single_refund', $refund_data, array('id'=>$item['single_refundid'], 'uniacid' => $uniacid));

                $order_data = array();
                $order_data['single_refundstate'] = 9;
                pdo_update('vending_machine_order_goods', $order_data, array('id'=>$item['ordergoodsid'], 'uniacid' => $uniacid));
                ...
                pdo_update('vending_machine_order_single_refund', array('reply' => $refundcontent, 'status' => -1, 'endtime' => $time), array('id' => $item['single_refundid']));

                plog('order.op.refund.submit', "订单退款拒绝 ID: {$item['id']} 订单号: {$item['ordersn']} 原因: {$refundcontent}");

                //更新订单商品退款状态
                pdo_update('vending_machine_order_goods', array('single_refundstate' => 8), array('id' => $item['ordergoodsid'], 'uniacid' => $uniacid));

                //查询其它 订单商品 是否有正在维权中的
                $is_single_refund=pdo_fetchcolumn('select count(id) from '.tablename('vending_machine_order_goods').'where orderid=:orderid and (single_refundstate=1 or single_refundstate=2)',array(':orderid'=>$item['id']));

                //如果其它 订单商品 没有正在维权中
                if(empty($is_single_refund)){
                    //更新订单维权时间
                    pdo_update('vending_machine_order', array('refundtime' => $time), array('id' => $item['id'], 'uniacid' => $uniacid));
                }
...
//            }
        ...
        //更新订单退款状态
        pdo_update('vending_machine_order_goods', array('single_refundstate' => 9,'single_refundtime' => $time,'nocommission'=>1), array('id' => $item['ordergoodsid'], 'uniacid' => $_W['uniacid']));


        $order_goods=pdo_fetchall("select single_refundid,single_refundstate,single_refundtime,sendtype,sendtime from ".tablename('vending_machine_order_goods')." where orderid=:orderid",array(':orderid'=>$item['id']));
        ...
        if($refund_num==count($order_goods)){
            ...
            pdo_update('vending_machine_order', array('status' => -1,'canceltime' => $time,'refundid'=>0), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
            plog('order.op.close', "订单关闭 ID: {$item['id']} 订单号: {$item['ordersn']}");
        }
        
        // 没有没发货的商品，更新订单状态
        if ($nosend == 0 && $item['sendtype'] != 0) {
            pdo_update('vending_machine_order', array('status' => 2), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
        }

        //如果没有正在维权中的订单商品
        if(empty($apply_refund_num)){
            //更新订单维权时间
            pdo_update('vending_machine_order', array('refundtime' => $time,'refundid'=>0), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
        }
        ...
        pdo_update('vending_machine_goods', array('salesreal' => $salesreal), array('id' => $item['ordergoodsid']));
