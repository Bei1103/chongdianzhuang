...
class Batchsend_Page extends WebPage {
...
//                $expresssn = '';
                ...
                $sql = "select id,status,refundid from " . tablename('vending_machine_order') . " where ordersn=:ordersn and uniacid=:uniacid and isparent=0 and merchid=:merchid";
                ...
                if (!empty($order)) {
                    ...
                    if (!empty($order['refundid'])) {
                        $refund = pdo_fetch('select id from ' . tablename('vending_machine_order_refund') . ' where id=:id limit 1', array(':id' => $order['refundid']));
                        if (!empty($refund)) {
                            $refund_flag = 1;
                            pdo_update('vending_machine_order_refund', array('status' => -1, 'endtime' => $time), array('id' => $order['refundid']));
                        }
                    }
                    ...
                    pdo_update('vending_machine_order', $data, array('id' => $order['id']));
