...
class Index_Page extends WebPage {
	...
	function settemplateid()
	{
		...
		//默认模板消息
		$templatetype = pdo_fetch('select `name`,templatecode,content,templatename  from ' . tablename('vending_machine_member_message_template_type') . ' where typecode=:typecode  limit 1', array(':typecode' => $tag));
        ...
        // 当前获取的默认模板消息
        $defaulttemp = pdo_fetch('select *  from ' . tablename('vending_machine_member_message_template_default') . ' where typecode=:typecode and uniacid=:uniacid  limit 1',
        ...
        /**
         * 先判断该类型时候获取过（每次请求模板消息接口返回的模板id不同）
        ...
        if (in_array($templatetype['templatename'], $templateTiele)) {
            ...
            if(empty($defaulttemp)) {
                ...
                pdo_update('vending_machine_member_message_template_default', array('templateid'=>$template_list[$found_key]['template_id']),
                    array('typecode' =>$tag,'uniacid' => $_W['uniacid']));
            }
            ...
            else // 申请成功的情况
            {
                ...
                {
                    pdo_update('vending_machine_member_message_template_default', array('templateid'=>$result['template_id']),
                        array('typecode' =>$tag,'uniacid' => $_W['uniacid']));
                }
            }
        }
...
/*
		...
		if (isset($data['openid2'])) {
			if (!empty($data['openid2'])) {
				...
				$salers2 = pdo_fetchall("select id,nickname,avatar,openid from " . tablename('vending_machine_member') . ' where openid in (' . implode(",", $openids2) . ") and uniacid={$_W['uniacid']}");
			}
		}
		...
		if (isset($data['openid3'])) {
			if (!empty($data['openid3'])) {
				...
				$salers3 = pdo_fetchall("select id,nickname,avatar,openid from " . tablename('vending_machine_member') . ' where openid in (' . implode(",", $openids3) . ") and uniacid={$_W['uniacid']}");
			}
		}
		...
		if (isset($data['openid4'])) {
			if (!empty($data['openid4'])) {
				...
				$salers4 = pdo_fetchall("select id,nickname,avatar,openid from " . tablename('vending_machine_member') . ' where openid in (' . implode(",", $openids4) . ") and uniacid={$_W['uniacid']}");
			}
		}
		...
		//模板消息
		$template_list = pdo_fetchall('SELECT id,title,typecode FROM ' . tablename('vending_machine_member_message_template') . ' WHERE uniacid=:uniacid ', array(':uniacid' => $_W['uniacid']));
	...
	protected function upload_cert($fileinput) {
		global $_W;
		$path = IA_ROOT . "/addons/vending_machine/cert";
		load()->func('file');
		mkdirs($path);
	...
	function payset() {
		...
		if ($_W['ispost']) {
			...
			pdo_update('vending_machine_sysset', array('sec' => iserializer($sec)), array('uniacid' => $_W['uniacid']));
		...
		$payments = pdo_fetchall("SELECT id,title FROM ".tablename('vending_machine_payment')." WHERE uniacid=:uniacid and paytype = 0 ",array(':uniacid'=>$_W['uniacid']));
		$paymentalis = pdo_fetchall("SELECT id,title FROM ".tablename('vending_machine_payment')." WHERE uniacid=:uniacid and paytype = 1 ",array(':uniacid'=>$_W['uniacid']));
        if (empty($payments)){
            $payments = array();
            ...
            if (!empty($payment['wechat']['mchid'])){
                ...
                $payments[] = $default;
                pdo_insert('vending_machine_payment',$default);
		...
		$url =$_W['siteroot']."addons/vending_machine/payment/wechat/notify.php";
		load()->func('communication');
		$resp = ihttp_get($url);
		include $this->template();
		exit;
	...
	function templat() {
		...
		$styles = array();
		$dir = IA_ROOT . "/addons/vending_machine/template/mobile/";
	...
	public function wap() {
		...
		$styles = array();
		$dir = IA_ROOT . "/addons/vending_machine/template/account/";
		if ($handle = opendir($dir)) {
			while (($file = readdir($handle)) !== false) {
				if ($file != ".." && $file != ".") {
					if (is_dir($dir . "/" . $file)) {
        ...
        if ($_W['ispost']) {
            ...
            if (empty($data) || empty($data['new_area'])) {
                ...
                if (!empty($array['new_area'])) {
                    ...
                    $change_data['city'] = '';
                    $change_data['area'] = '';
                    pdo_update('vending_machine_member', $change_data, array('uniacid'=>$uniacid));
                    pdo_update('vending_machine_member_address', $change_data, array('uniacid'=>$uniacid));
                } else {
            ...
            if(empty($data)){
                ...
                pdo_insert('vending_machine_area_config', $array);
            }else{
                if (!empty($array)) {
                    pdo_update('vending_machine_area_config', $array, array('id'=>$data['id'], 'uniacid'=>$uniacid));
                }
            }
            $data = m('util')->get_area_config_data();
        ...
        if ($_W['ispost']) {
            ...
            if (empty($_GPC['express_type'])) {
                $data['express_bird'] = array(
                    ...
                    'express_bird_customer_name'=>trim($_GPC['express_bird_customer_name']),//原本是intval()取整,但是需要字母加数字,所以改成trim()
                );
