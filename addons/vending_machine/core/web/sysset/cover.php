...
class Cover_Page extends WebPage {

	protected function _cover($key,$name,$url) {
		
		global $_W, $_GPC;
		$rule = pdo_fetch("select * from " . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => "vending_machine{$name}入口设置"));
		$keyword =false;
		$cover = false;
		if (!empty($rule)) {
		...
		if ($_W['ispost']) {
			...
			if(!empty($keyword1)){
				if($keyword1['name']!='vending_machine'.$name.'入口设置'){
					show_json(0, '关键字已存在!');
				}
			}
			...
			$rule_data = array(
				'uniacid' => $_W['uniacid'],
				'name' => "vending_machine{$name}入口设置",
				'module' => 'cover',
				'displayorder' => 0,
