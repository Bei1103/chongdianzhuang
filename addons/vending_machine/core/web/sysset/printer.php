...
class Printer_Page extends WebPage
{
	...
	function main() {
		...
		$list = pdo_fetchall("SELECT * FROM " . tablename('vending_machine_member_printer_template') . " WHERE {$condition}  ORDER BY id asc limit " . ($pindex - 1) * $psize . ',' . $psize, $params);
		$total = pdo_fetchcolumn("SELECT count(*) FROM " . tablename('vending_machine_member_printer_template') . " WHERE {$condition}", $params);
	...
	protected function post() {
		...
		$id = intval($_GPC['id']);
		if (!empty($id)) {
			$list = pdo_fetch('SELECT * FROM ' . tablename('vending_machine_member_printer_template') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			if (empty($id)) {
				...
				pdo_insert('vending_machine_member_printer_template', $data);
				$id = pdo_insertid();
				plog('sysset.printer.add', "添加打印机模板 ID: {$id} 标题: {$data['title']} ");
			} else {
				pdo_update('vending_machine_member_printer_template', $data, array('id' => $id));
	...
	function delete() {
		...
		$items = pdo_fetchall("SELECT id,title FROM " . tablename('vending_machine_member_printer_template') . " WHERE id in( $id ) AND uniacid={$_W['uniacid']} AND merchid=0");
		if (!empty($items))
        {
            foreach ($items as $item) {
                pdo_delete('vending_machine_member_printer_template', array('id' => $id));
	...
	function query() {
		...
		$ds = pdo_fetchall('SELECT id,title FROM ' . tablename('vending_machine_member_printer_template') . " WHERE {$condition} order by id asc", $params);
		if ($_GPC['suggest']) {
	...
	function printer_list()
    {
        ...
        $list = pdo_fetchall("SELECT * FROM " . tablename('vending_machine_member_printer') . " WHERE {$condition}  ORDER BY id asc limit " . ($pindex - 1) * $psize . ',' . $psize, $params);
        $total = pdo_fetchcolumn("SELECT count(*) FROM " . tablename('vending_machine_member_printer') . " WHERE {$condition}", $params);
        $pager = pagination2($total, $pindex, $psize);
    ...
    protected function printer_post()
    {
        ...
        if (!empty($id)) {
            $list = pdo_fetch('SELECT * FROM ' . tablename('vending_machine_member_printer') . ' WHERE id=:id and uniacid=:uniacid and merchid=0', array(':id' => $id, ':uniacid' => $_W['uniacid']));
            $print_data = json_decode($list['print_data'],true);
        ...
        if ($_W['ispost']) {
            ...
            if (empty($id)) {
                $data['createtime'] = time();
                pdo_insert('vending_machine_member_printer', $data);
                $id = pdo_insertid();
                plog('sysset.printer.printer_add', "添加打印机 ID: {$id} 标题: {$data['title']} ");
            } else {
                pdo_update('vending_machine_member_printer', $data, array('id' => $id));
    ...
    function printer_delete() {
        ...
        $items = pdo_fetchall("SELECT id,title FROM " . tablename('vending_machine_member_printer') . " WHERE id in( $id ) AND uniacid={$_W['uniacid']}  and merchid=0");
        foreach ($items as $item) {
            pdo_delete('vending_machine_member_printer', array('id' => $id));
    ...
    function printer_query() {
        ...
        $ds = pdo_fetchall('SELECT id,title FROM ' . tablename('vending_machine_member_printer') . " WHERE  {$condition} order by id asc", $params);
        if ($_GPC['suggest']) {
            die(json_encode(array('value' => $ds)));
        }
        include $this->template();
...
//        dump(com_run('printer::sendOrderMessage',8190));die;
        ...
        $order_template = pdo_fetchall('SELECT * FROM ' . tablename('vending_machine_member_printer_template') . ' WHERE uniacid=:uniacid ', array(':uniacid' => $_W['uniacid']));
        include $this->template();
    }
