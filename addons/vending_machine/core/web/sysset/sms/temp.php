...
class Temp_Page extends ComWebPage {
	...
	public function main() {
		...
		$list = pdo_fetchall("SELECT * FROM " . tablename('vending_machine_sms') . " WHERE 1 {$condition}  ORDER BY id desc limit " . ($pindex - 1) * $psize . ',' . $psize, $params);
		$total = pdo_fetchcolumn("SELECT count(*) FROM " . tablename('vending_machine_sms') . " WHERE 1 {$condition}", $params);
	...
	protected function post() {
		...
		$id = intval($_GPC['id']);
		if (!empty($_GPC['id'])) {
			$item = pdo_fetch('SELECT * FROM ' . tablename('vending_machine_sms') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		...
		if($_W['ispost']){
			...
			if(empty($id)){
				...
				pdo_insert('vending_machine_sms', $arr);
				$id = pdo_insertid();
				plog('sysset.sms.temp.add', "添加短信模板 ID: {$id} 标题: {$arr['title']} ");
			}else{
				pdo_update('vending_machine_sms', $arr, array('id'=>$id, 'uniacid'=>$_W['uniacid']));
	...
	public function delete() {
		...
		$items = pdo_fetchall("SELECT id, name FROM " . tablename('vending_machine_sms') . " WHERE id in( $id ) and uniacid=:uniacid ", array(':uniacid'=>$_W['uniacid']));
		if(!empty($items)){
			foreach ($items as $item){
				pdo_delete('vending_machine_sms', array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
	...
	public function status() {
		...
		if(!empty($items)){
			foreach ($items as $item){
				pdo_update('vending_machine_sms', array('status'=>$status), array('id'=>$item['id'], 'uniacid'=>$_W['uniacid']));
