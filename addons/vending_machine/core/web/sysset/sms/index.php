...
class Index_Page extends ComWebPage {
	...
	public function set() {
		...
		$item = pdo_fetch("SELECT * FROM " . tablename('vending_machine_sms_set') . " WHERE uniacid=:uniacid ", array(':uniacid'=>$_W['uniacid']));
		...
		if($_W['ispost']){
			...
			if(empty($item)){
				...
				pdo_insert('vending_machine_sms_set', $arr);
				$id = pdo_insertid();
			}else{
				pdo_update('vending_machine_sms_set', $arr, array('id'=>$item['id'], 'uniacid'=>$_W['uniacid']));
			}
			show_json(1);
		}
