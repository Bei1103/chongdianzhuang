...
$id = intval($_GPC['id']);
if(!empty($id)){
    pdo_update('vending_machine_goods',array('newgoods'=>0),array('id'=>$id));

}
$item = pdo_fetch("SELECT * FROM " . tablename('vending_machine_goods') . " WHERE id = :id and uniacid = :uniacid", array(':id' => $id,':uniacid'=>$_W['uniacid']));
...
{
    $card = pdo_fetch("SELECT * FROM " . tablename('vending_machine_goods_cards') . " WHERE id = :id and uniacid = :uniacid", array(':id' => $item['cardid'],':uniacid'=>$_W['uniacid']));
}
if(p('offic')){
...
//佣金计算
$commission_level = pdo_fetchall("SELECT * FROM " . tablename('vending_machine_commission_level') . " WHERE uniacid = '{$_W['uniacid']}' ORDER BY commission1 asc");
...
//    }
    ...
    if($_GPC['isverify'] ==2 && $id){
        //判断是否为此商品设置了赠品
        $nowtime = TIMESTAMP;
        $gift = pdo_fetch("select id,title from ".tablename('vending_machine_gift')." WHERE goodsid = :goodsid and uniacid=:uniacid and endtime>=:endtime", array(':goodsid' => $id, ':uniacid' => $_W['uniacid'],':endtime' => $nowtime));
...
//      'discounts' => is_array($_GPC['discounts']) ? json_encode($_GPC['discounts']) : json_encode(array()),
    ...
    $data['fee_type'] =intval($_GPC['fee_type']);
    $data['weight_unit'] =intval($_GPC['weight_unit']);
    ...
    if (is_array($_GPC['cates'])) {
        ...
        foreach ($cates as $key=>$cid) {
            ...
            $c = pdo_fetch('select level from ' . tablename('vending_machine_category') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $cid, ':uniacid' => $_W['uniacid']));
            ...
            if($key==0){
                ...
                if($c['level']==1){ //一级
                    ...
                    $crow = pdo_fetch('select parentid from ' . tablename('vending_machine_category') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $cid, ':uniacid' => $_W['uniacid']));
                    ...
                    $tcateid = $cid;
                    $tcate = pdo_fetch('select id,parentid from ' . tablename('vending_machine_category') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $cid, ':uniacid' => $_W['uniacid']));
                    $ccateid = $tcate['parentid'];
                    $ccate = pdo_fetch('select id,parentid from ' . tablename('vending_machine_category') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $ccateid, ':uniacid' => $_W['uniacid']));
        ...
        if ($_GPC['diyformtype'] == 2) {
            ...
            if ($_GPC['diysave'] == 1) {
                ...
                if (empty($is_save)) {
                    ...
                    pdo_insert('vending_machine_diyform_type', $insert);
                    $data['diysaveid'] = pdo_insertid();
                } else {
                    pdo_update('vending_machine_diyform_type', $insert, array('id' => $diysaveid));
                }
            }
            ...
            if(!empty($opencard))
            {
                ...
                if (!empty($card)) {
                    ...
                    if($change)
                    {
                        ...
                        pdo_update('vending_machine_goods_cards', $carddata, array('id' => $card['id']));
                    }
                    $cardid = $card['id'];
                ...
                }else
                {
                    ...
                    pdo_insert('vending_machine_goods_cards', $carddata);
                    $cardid = pdo_insertid();
    ...
    if (empty($id)) {
        $data['merchid'] = 0;
        pdo_insert('vending_machine_goods', $data);
        $id = pdo_insertid();
        plog('goods.add', "添加商品 ID: {$id}<br>".(!empty($data['nocommission']) ? "是否参与分销 -- 否" : "是否参与分销 -- 是"));
    } else {
        unset($data['createtime']);
        $old_data = pdo_fetch('SELECT nocommission,marketprice,total FROM '.tablename('vending_machine_goods').' WHERE id=:id AND uniacid=:uniacid',array(':id'=>$id,':uniacid'=>$_W['uniacid']));
        pdo_update('vending_machine_goods', $data, array('id' => $id));
    ...
    for ($k = 0; $k < $len; $k++) {
        ...
        if (!is_numeric($get_param_id)) {
            pdo_insert("vending_machine_goods_param", $a);
            $param_id = pdo_insertid();
        } else {
            pdo_update("vending_machine_goods_param", $a, array('id' => $get_param_id));
    ...
    if (count($paramids) > 0) {
        pdo_query("delete from " . tablename('vending_machine_goods_param') . " where goodsid=$id and id not in ( " . implode(',', $paramids) . ")");
    } else {
        pdo_query("delete from " . tablename('vending_machine_goods_param') . " where goodsid=$id");
    }
    ...
    for ($k = 0; $k < $len; $k++) {
        ...
        if (is_numeric($get_spec_id)) {
            pdo_update("vending_machine_goods_spec", $a, array("id" => $get_spec_id));
            $spec_id = $get_spec_id;
        } else {
            pdo_insert("vending_machine_goods_spec", $a);
        ...
        for ($n = 0; $n < $itemlen; $n++) {
            ...
            if (is_numeric($get_item_id)) {
                pdo_update("vending_machine_goods_spec_item", $d, array("id" => $get_item_id));
                $item_id = $get_item_id;
            } else {
                pdo_insert("vending_machine_goods_spec_item", $d);
        ...
        //删除其他的
        if (count($itemids) > 0) {
            pdo_query("delete from " . tablename('vending_machine_goods_spec_item') . " where uniacid={$_W['uniacid']} and specid=$spec_id and id not in (" . implode(",", $itemids) . ")");
        } else {
            pdo_query("delete from " . tablename('vending_machine_goods_spec_item') . " where uniacid={$_W['uniacid']} and specid=$spec_id");
        }
        //更新规格项id
        pdo_update("vending_machine_goods_spec", array("content" => serialize($itemids)), array("id" => $spec_id));
        $specids[] = $spec_id;
    }
    //删除其他的
    if (count($specids) > 0) {
        pdo_query("delete from " . tablename('vending_machine_goods_spec') . " where uniacid={$_W['uniacid']} and goodsid=$id and id not in (" . implode(",", $specids) . ")");
    } else {
        pdo_query("delete from " . tablename('vending_machine_goods_spec') . " where uniacid={$_W['uniacid']} and goodsid=$id");
    }
    //保存规格
    ...
    for ($k = 0; $k < $len; $k++) {
        ...
        if (empty($get_option_id)) {
            pdo_insert("vending_machine_goods_option", $a);
            $option_id = pdo_insertid();
//            plog('goods.option.add', "添加商品规格 ID: {$option_id}");
        } else {

            pdo_update("vending_machine_goods_option", $a, array('id' => $get_option_id));
...
//            plog('goods.option.edit', "修改商品规格 ID: {$get_option_id}");
            //修改规格名称
            if( p('groups') ){
                pdo_update( 'vending_machine_groups_goods_option' , array( 'title' => $a['title'],'specs' => $newids ) , array( 'goods_option_id' => $get_option_id ) );
            }
    ...
    pdo_update('vending_machine_goods', array('discounts'=>$discounts_json), array('id' => $id));
    ...
    pdo_update('vending_machine_goods', array('isdiscount_discounts'=>$is_discounts_json), array('id' => $id));
    ...
    pdo_update('vending_machine_goods', $commission_arr, array('id' => $id));

    if (count($optionids) > 0 && $data['hasoption'] !== 0) {
        pdo_query("delete from " . tablename('vending_machine_goods_option') . " where goodsid=$id and id not in ( " . implode(',', $optionids) . ")");
        //删除拼团多规格
        if( p('groups') ){
            pdo_query("delete from " . tablename('vending_machine_groups_goods_option') . " where goodsid=$id and goods_option_id not in ( " . implode(',', $optionids) . ")");
        }
        ...
        pdo_query("delete from " . tablename('vending_machine_goods_option') . " where goodsid=$id");
        $sql = "update ".tablename('vending_machine_goods')." set minprice = marketprice,maxprice = marketprice where id = $id and hasoption=0;";
        pdo_query($sql);

        //删除拼团多规格  修改拼团商品状态
        if( p('groups') ){
            pdo_query("delete from " . tablename('vending_machine_groups_goods_option') . " where goodsid=$id");
            pdo_update( 'vending_machine_groups_goods' , array( 'more_spec' => 0 ) , array( 'gid' => $id ) );
        }
    }


    //如果是有促销,那么更新最大最小价格
    $sqlgoods = "SELECT id,title,thumb,marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,total,description,merchsale FROM " . tablename('vending_machine_goods') . " where id=:id and uniacid=:uniacid limit 1";
    $goodsinfo = pdo_fetch($sqlgoods,array(':id'=>$id,':uniacid'=>$_W['uniacid']));
    $goodsinfo = m('goods')->getOneMinPrice($goodsinfo);

    pdo_update('vending_machine_goods',array('minprice'=>$goodsinfo['minprice'],'maxprice'=>$goodsinfo['maxprice']),array('id'=>$id,'uniacid'=>$_W['uniacid']));
    ...
    if ($data['type'] == 3 && $com_virtual) {
        ...
        if ($data['hasoption'] !== 0 && ($data['totalcnf'] != 3) && empty($data['unite_total'])) {
            pdo_update("vending_machine_goods", array("total" => $totalstocks), array("id" => $id));
        }
    }
...
if (!empty($id)) {
    ...
    $allspecs = pdo_fetchall("select * from " . tablename('vending_machine_goods_spec') . " where goodsid=:id order by displayorder asc", array(":id" => $id));
    foreach ($allspecs as &$s) {
        $s['items'] = pdo_fetchall("select a.id,a.specid,a.title,a.thumb,a.show,a.displayorder,a.valueId,a.virtual,b.title as title2 from " . tablename('vending_machine_goods_spec_item') . " a left join " . tablename('vending_machine_virtual_type') . " b on b.id=a.virtual  where a.specid=:specid order by a.displayorder asc", array(":specid" => $s['id']));
    }
    unset($s);

    //商品参数
    $params = pdo_fetchall("select * from " . tablename('vending_machine_goods_param') . " where goodsid=:id order by displayorder asc", array(':id' => $id));
    ...
    $options = pdo_fetchall("select * from " . tablename('vending_machine_goods_option') . " where goodsid=:id order by id asc", array(':id' => $id));
    //排序好的specs
    $specs = array();
    //找出数据库存储的排列顺序
    if (count($options) > 0) {
...
//             商品类型如果为虚拟卡密则不允许修改库存
    ...
    if ($merchid == 0) {
        ...
        if (!empty($item['storeids'])) {
            $stores = pdo_fetchall('select id,storename from ' . tablename('vending_machine_store') . ' where id in (' . $item['storeids'] . ' ) and uniacid=' . $_W['uniacid']);
        }
    }
    ...
    if (!empty($item['noticeopenid'])) {
        ...
        if (isset($item['noticeopenid'])) {
            if (!empty($item['noticeopenid'])) {
                ...
                $salers = pdo_fetchall("select id,nickname,avatar,openid from " . tablename('vending_machine_member') . ' where openid in (' . implode(",", $openids) . ") and uniacid={$_W['uniacid']}");
            }
        }
...
//查询快递模板
$dispatch_data = pdo_fetchall('select * from ' . tablename('vending_machine_dispatch') . ' where uniacid=:uniacid and merchid=:merchid and enabled=1 order by displayorder desc', array(':uniacid' => $_W['uniacid'], ':merchid' =>$merchid));
...
if ($com_virtual) {
    $virtual_types = pdo_fetchall("select * from " . tablename('vending_machine_virtual_type') . " where uniacid=:uniacid and merchid=:merchid and recycled = 0 order by id asc", array(":uniacid" => $_W['uniacid'], ':merchid' =>$merchid));
}
...
if ($merchid == 0) {
    $details = pdo_fetchall('select detail_logo,detail_shopname,detail_btntext1, detail_btnurl1 ,detail_btntext2,detail_btnurl2,detail_totaltitle from '
        . tablename('vending_machine_goods') . " where uniacid=:uniacid and detail_shopname<>'' group by detail_shopname", array(':uniacid' => $_W['uniacid']));
