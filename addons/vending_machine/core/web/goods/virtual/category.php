...
class Category_Page extends ComWebPage {
    ...
    function main() {
        ...
        if (!empty($_W['ispost'])) {
            ...
            foreach ($_GPC['catname'] as $id => $catname) {
                ...
                if (strpos($id,'new_') !== false) {
                    pdo_insert('vending_machine_virtual_category', array('name' => $catname, 'uniacid' => $_W['uniacid'], 'merchid' => 0));
                    $insert_id = pdo_insertid();
                    plog('goods.virtual.category.add', "添加分类 ID: {$insert_id}");
                } else {
                    pdo_update('vending_machine_virtual_category', array('name' => $catname), array('id' => $id, 'merchid' => 0));
        ...
        $list = pdo_fetchall("SELECT * FROM " . tablename('vending_machine_virtual_category') . " WHERE uniacid = '{$_W['uniacid']}' and merchid=0 ORDER BY id DESC");
    ...
    function delete() {
        ...
        global $_W, $_GPC;
        $id = intval($_GPC['id']);
        $item = pdo_fetch("SELECT id,name FROM " . tablename('vending_machine_virtual_category') . " WHERE id = '$id' and merchid=0 AND uniacid=" . $_W['uniacid'] . "");
        if (empty($item)) {
            $this->message('抱歉，分类不存在或是已经被删除！', webUrl('goods/virtual/category', array('op' => 'display')), 'error');
        }
        pdo_delete('vending_machine_virtual_category', array('id' => $id));
