...
class Index_Page extends WebPage {
    ...
    function main($goodsfrom='sale') {
        ...
        if (!empty($_GPC['keyword'])) {
            ...
            $sqlcondition = ' left join ' . tablename('vending_machine_goods_option') . ' op on g.id = op.goodsid';
            if ($merch_plugin) {
                $sqlcondition .= " left join " . tablename('vending_machine_merch_user') . " merch on merch.id = g.merchid and merch.uniacid=g.uniacid";
            }
         ...
         */
        if (p('merch')) {
            $merchSql = ' LEFT JOIN '. tablename('vending_machine_merch_user') .' merch_user ON merch_user.id = g.merchid ';
            $condition .= ' AND (g.merchid = 0 or (g.merchid > 0 and merch_user.accounttime > '.time().')) ';
        }


        $sql = 'SELECT g.id FROM ' . tablename('vending_machine_goods') . 'g '. $merchSql . $sqlcondition . $condition . $groupcondition;
        $total_all = pdo_fetchall($sql, $params);
        $total = count($total_all);
        unset($total_all);
        if (!empty($total)) {
            $sql = 'SELECT g.id,g.merchid,g.cates,g.status,g.displayorder,g.thumb,g.title,g.marketprice,g.total,g.salesreal,g.isnew,g.ishot,g.isdiscount,g.isrecommand,g.issendfree,g.istime,g.isnodiscount,g.checked FROM ' . tablename('vending_machine_goods') . 'g ' . $merchSql . $sqlcondition . $condition . $groupcondition . ' ORDER BY'.$orderby.' g.`status` DESC, g.`displayorder` DESC,
                g.`id` DESC ';
            ...
            foreach($list as $key => &$value){
                ...
                $sale_cpcount=pdo_fetch("SELECT sum(og.total)  as sale_count   FROM ims_vending_machine_order_goods  og LEFT JOIN ims_vending_machine_order o on og.orderid=o.id  WHERE og.goodsid=:gsid and o.`status`>=:status and o.refundstate = 0 and og.uniacid=:uniacid",array(':gsid'=>$value['id'],':status'=>1,':uniacid'=>$_W['uniacid']));
    ...
    function create() {
        ...
        if ($com_virtual) {
            $virtual_types = pdo_fetchall("select * from " . tablename('vending_machine_virtual_type') . " where uniacid=:uniacid and merchid=:merchid and recycled = 0 order by id asc", array(":uniacid" => $_W['uniacid'], ':merchid' =>0));
        }
    ...
    function delete() {
        ...
        $items = pdo_fetchall("SELECT id,title FROM " . tablename('vending_machine_goods') . " WHERE id in( $id ) AND uniacid=" . $_W['uniacid']);
        foreach ($items as $item) {
            pdo_update('vending_machine_goods', array('deleted' => 1), array('id' => $item['id']));
    ...
    function status() {
        ...
        //'新'字角标去除
        pdo_query("update ".tablename('vending_machine_goods')." set newgoods = 0 where id in ( {$id} ) AND uniacid=" . $_W['uniacid']);

        $items = pdo_fetchall("SELECT id,title,status,isstatustime,statustimestart,statustimeend FROM " . tablename('vending_machine_goods') . " WHERE id in( $id ) AND uniacid=" . $_W['uniacid']);
        ...
        foreach ($items as $item) {
            ...
            pdo_update('vending_machine_goods', array('status' => intval($_GPC['status'])), array('id' => $item['id']));
    ...
    function checked() {
        ...
        foreach ($items as $item) {
            pdo_update('vending_machine_goods', array('checked' => intval($_GPC['checked'])), array('id' => $item['id']));
    ...
    function delete1() {
        ...
        foreach ($items as $item) {
            pdo_delete('vending_machine_goods', array('id' => $item['id']));
    ...
    function restore() {
        ...
        foreach ($items as $item) {
            pdo_update('vending_machine_goods', array('deleted' => 0), array('id' => $item['id']));
    ...
    function property() {
        ...
        if (in_array($type, array('new', 'hot', 'recommand', 'discount', 'time', 'sendfree', 'nodiscount'))) {
            ...
            pdo_update("vending_machine_goods", array("is" . $type => $data), array("id" => $id, "uniacid" => $_W['uniacid']));
            if ($type == 'new') {
                $typestr = "新品";
            } else if ($type == 'hot') {
                $typestr = "热卖";
        ...
        if (in_array($type, array('status'))) {
            pdo_update("vending_machine_goods", array($type => $data), array("id" => $id, "uniacid" => $_W['uniacid']));
    ...
    function change() {
        ...
        $goods = pdo_fetch('select id,hasoption,marketprice,dowpayment,`type`,isdiscount,isdiscount_time from ' . tablename('vending_machine_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $id));
        if (empty($goods)) {
            show_json(0, array('message' => '参数错误'));
        }
        ...
        $result = pdo_update('vending_machine_goods', array($type => $value), array('id' => $id));
        ...
        /*if ($goods['hasoption'] == 0 && $type != 'displayorder' && ($goods['isdiscount'] ==0 || $goods['isdiscount_time']<time()) && $type != 'total' ) {
            $sql = "update ".tablename('vending_machine_goods')." set minprice = marketprice,maxprice = marketprice where id = {$goods['id']} and hasoption=0;";
            pdo_query($sql);
        }*/
    ...
    function query(){
        ...
        $ds = pdo_fetchall('SELECT id,title,thumb,marketprice,productprice,share_title,share_icon,description,minprice,costprice,total,sales,islive,liveprice FROM ' . tablename('vending_machine_goods') . " WHERE 1 {$condition} order by createtime desc", $params);
        foreach($ds as &$value){
    ...
    function goodsprice()
    {
        global $_W;
        $sql = "update ".tablename('vending_machine_goods')." g set 
g.minprice = (select min(marketprice) from ".tablename('vending_machine_goods_option')." where g.id = goodsid),
g.maxprice = (select max(marketprice) from ".tablename('vending_machine_goods_option')." where g.id = goodsid)
where g.hasoption=1 and g.uniacid=".$_W['uniacid'].";
update ".tablename('vending_machine_goods')." set minprice = marketprice,maxprice = marketprice where hasoption=0 and uniacid=".$_W['uniacid'].";";
        pdo_run($sql);
        show_json(1);
    }
    ...
    function ajax_batchcates(){
        ...
        foreach ($goodsids as $goodsid){
            ...
            if(!empty($iscover)){
                ...
                pdo_update('vending_machine_goods', $data, array('id' => $goodsid));
            }else{
                //不覆盖原有分类
                $goods = pdo_fetch('select pcate,ccate,tcate,cates,pcates,ccates,tcates  from ' . tablename('vending_machine_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $goodsid, ':uniacid' => $_W['uniacid']));
