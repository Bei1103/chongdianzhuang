...
class Index_Page extends WebPage {
    ...
    public function main() {
        ...
        if(!empty($_W['shopversion'])) {
            ...
            //待发货详细信息
            $order_sql ="select id,ordersn,createtime,address,price,invoicename from " . tablename('vending_machine_order') . " where uniacid = :uniacid and merchid=0 and isparent=0 and deleted=0 AND ( status = 1 or (status=0 and paytype=3) ) ORDER BY createtime ASC LIMIT 20";
            ...
            //公告信息
            $notice = pdo_fetchall("SELECT * FROM " . tablename('vending_machine_system_copyright_notice') . " ORDER BY displayorder ASC,createtime DESC LIMIT 10" );
    ...
    public function view() {
        global $_GPC;
        $id = intval($_GPC['id']);
        $item = pdo_fetch("SELECT * FROM " . tablename('vending_machine_system_copyright_notice') . " WHERE id = $id ORDER BY displayorder ASC,createtime DESC" );
    ...
    public function notice() {
        ...
        $list = pdo_fetchall("SELECT * FROM " . tablename('vending_machine_system_copyright_notice') . " WHERE 1 {$condition}  ORDER BY displayorder DESC, createtime DESC limit " . ($pindex - 1) * $psize . ',' . $psize, $params);
        $total = pdo_fetchcolumn("SELECT count(*) FROM " . tablename('vending_machine_system_copyright_notice') . " WHERE 1 {$condition}", $params);
    ...
    public function ajax() {
        ...
        $goods_totals = pdo_fetchcolumn(
            'SELECT COUNT(1) FROM ' . tablename('vending_machine_goods') ." WHERE uniacid = :uniacid and status=1 and deleted=0 and total<=0 and total<>-1  ",
            $paras
        );
        //待审核提现
        $finance_total = pdo_fetchcolumn("select count(1) from " . tablename('vending_machine_member_log') . " log "
            . " left join " . tablename('vending_machine_member') . " m on m.openid=log.openid and m.uniacid= log.uniacid"
        ...
        //分销商总数
        $commission_agent_total = pdo_fetchcolumn("select count(*) from" . tablename('vending_machine_member') . " as  dm "
            . " where dm.uniacid =:uniacid and dm.isagent =1 and dm.status = 1", array(':uniacid' => $_W['uniacid']));

        //待审核分销商
        $commission_agent_status0_total = pdo_fetchcolumn("select count(1) from" . tablename('vending_machine_member') . " as  dm "
            . " where dm.uniacid =:uniacid and dm.isagent =1 and dm.status=:status", array(':uniacid' => $_W['uniacid'], ':status' => 0));

        //待审核佣金提现申请
        $commission_apply_status1_total = pdo_fetchcolumn("select count(1) from" . tablename('vending_machine_commission_apply') . " a "
            . " left join " . tablename('vending_machine_member') . " m on m.uid = a.mid"
            . " left join " . tablename('vending_machine_commission_level') . " l on l.id = m.agentlevel"
            . " where a.uniacid=:uniacid and a.status=:status", array(':uniacid' => $_W['uniacid'], ':status' => 1));

        //待打款佣金提现申请
        $commission_apply_status2_total = pdo_fetchcolumn("select count(1) from" . tablename('vending_machine_commission_apply') . " a "
            . " left join " . tablename('vending_machine_member') . " m on m.uid = a.mid"
            . " left join " . tablename('vending_machine_commission_level') . " l on l.id = m.agentlevel"
    ...
    protected function selectGoodsRank($day=0)
    {
        ...
        $sql = "SELECT g.id,g.title,g.thumb,"
            . "(select ifnull(sum(og.price),0) from  " . tablename('vending_machine_order_goods') . " og left join " . tablename('vending_machine_order') . " o on og.orderid=o.id  where o.status>=1 and og.goodsid=g.id {$condition})  as money,"
            . "(select ifnull(sum(og.total),0) from  " . tablename('vending_machine_order_goods') . " og left join " . tablename('vending_machine_order') . " o on og.orderid=o.id  where o.status>=1 and og.goodsid=g.id {$condition}) as count  "
            . "from " . tablename('vending_machine_goods') . " g  "
