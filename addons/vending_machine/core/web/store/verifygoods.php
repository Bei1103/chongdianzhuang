...
class Verifygoods_Page extends WebPage{
    ...
    function main() {
        ...
        $sql = 'select vg.*,g.id as goodsid ,g.title,g.thumb,o.ordersn,o.paytime,s.storename,o.openid  from '
            . tablename('vending_machine_verifygoods') . '   vg
		 left join ' . tablename('vending_machine_store') . ' s  on s.id = vg.storeid
        ...
        foreach($list as &$row)
        {
            ...
            $verifygoodlogs = pdo_fetchall('select  *  from ' . tablename('vending_machine_verifygoods_log') . '  where  verifygoodsid =:id  ', array(':id' => $row['id']));
        ...
        $total = pdo_fetchcolumn('select  COUNT(*)   from ' . tablename('vending_machine_verifygoods') . '   vg
		 left join ' . tablename('vending_machine_store') . ' s  on s.id = vg.storeid
		 left join ' . tablename('vending_machine_order_goods') . ' og on vg.ordergoodsid = og.id
		 left join ' . tablename('vending_machine_order') . ' o on o.id = og.orderid
		 left join ' . tablename('vending_machine_goods') . ' g on og.goodsid = g.id
    ...
    function detail() {
        ...
        $item = pdo_fetch('select vg.*,g.title,g.subtitle,g.thumb,o.ordersn,o.paytime,o.openid  from ' . tablename('vending_machine_verifygoods') . '   vg
        ...
        $verifygoodlogs = pdo_fetchall('select vgl.*,s.storename,sa.salername  from ' . tablename('vending_machine_verifygoods_log') . '   vgl
        ...
        if ($_W['ispost']) {
            ...
            pdo_update('vending_machine_verifygoods', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
    ...
    function invalid() {
        ...
        $item = pdo_fetch("SELECT id FROM " . tablename('vending_machine_verifygoods') . " WHERE id=".$id."  AND uniacid=" . $_W['uniacid']);

        if(!empty($item))
        {
            pdo_update('vending_machine_verifygoods', array('invalid' => $type), array('id' => $item['id']));
    ...
    function verifygoodslog() {
        ...
        $sql = 'select vgl.id,g.id as goodsid ,g.title,g.thumb,o.ordersn,vgl.verifydate,vgl.verifynum,o.paytime,s.storename,sa.salername,vgl.remarks,o.openid  from ' . tablename('vending_machine_verifygoods_log') . '   vgl
    ...
    function verifygoodslogdelete() {
        ...
        $item = pdo_fetch("SELECT id,verifygoodsid FROM " . tablename('vending_machine_verifygoods_log') . " WHERE id=".$id." AND uniacid=" . $_W['uniacid']);

        if(!empty($item))
        {
            pdo_delete('vending_machine_verifygoods_log', array('id' => $item['id']));
            pdo_update('vending_machine_verifygoods', array('used' => 0), array('id' => $item['verifygoodsid']));
