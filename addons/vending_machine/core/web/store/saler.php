...
class Saler_Page extends WebPage {
    ...
    function main() {
        ...
        $sql = "SELECT s.*,m.nickname,m.avatar,m.realname,store.storename FROM " . tablename('vending_machine_saler') . "  s "
    ...
    protected function post() {
        ...
        if (!empty($item)) {
            ...
            if(p('newstore'))
            {
                $role = pdo_fetch("SELECT * FROM " . tablename('vending_machine_newstore_perm_role')." WHERE id =:id and uniacid=:uniacid limit 1", array(':uniacid' => $_W['uniacid'], ':id' => $item['roleid']));
            }
        ...
        if ($_W['ispost']) {
           ...
           if(p('newstore')){
               ...
               if(empty($item['username'])) {
                   ...
                   $usernames = pdo_fetchcolumn("SELECT count(*) FROM " . tablename('vending_machine_saler') . " WHERE username=:username limit 1", array(':username' => $_GPC['username']));
                   if ($usernames > 0) {
               ...
               if(!empty($_GPC['pwd']))
               {
                   $salt = random(8);
                   while (1) {
                       $saltcount = pdo_fetchcolumn('select count(*) from ' . tablename('vending_machine_saler') . ' where salt=:salt limit 1', array(':salt' => $salt));
                       if ($saltcount <= 0) {
                           break;
                       }
                       $salt = random(8);
    ...
    function query() {
        ...
        $ds = pdo_fetchall("SELECT s.*,m.nickname,m.avatar,m.mobile,m.realname,store.storename,m.openid FROM " . tablename('vending_machine_saler') . "  s "
