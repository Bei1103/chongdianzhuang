...
class Verifyorder_Page extends WebPage {
...
//        订单号，核销员，核销门店，核销码，会员姓名，收件人姓名，快递单号，商品名称，商品编码
        if (!empty($_GPC['searchfield']) && !empty($_GPC['keyword'])) {
            ...
            if ($searchfield == 'ordersn') {
                ...
                if(p('merch')){
                    $leftsql .= " left join " . tablename('vending_machine_merch_saler') . " ms on (ms.id = vgl.salerid or ms.id=vol.salerid or ms.openid=o.verifyopenid) and ms.uniacid=o.uniacid and o.ismerch=1";
                ...
                if(p('merch')){
                    $leftsql.=" left join " . tablename('vending_machine_merch_store') . " mstore on  ( mstore.id = o.verifystoreid or store.id = vgl.storeid or store.id = vol.storeid ) and o.ismerch=1";
...
//        if ($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0 and o.istrade=0 ' || !empty($sqlcondition)){
        $sql = "select distinct o.* , a.realname as arealname,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea, a.street as astreet,a.address as aaddress,
                  ...
                  r.rtype,r.status as rstatus,o.sendtype,store.storename{$field} from " . tablename('vending_machine_order') . " o"
            ...
            .  " left join " . tablename('vending_machine_store') . " store on ( store.id = vgl.storeid or store.id = vol.storeid or store.id=o.verifystoreid) and o.ismerch=0"
            .$leftsql
...
//            }
            ...
            foreach ($list as &$row) {
                ...
                if (!empty($row['verifyopenid']) && $row['verifytype'] == 0) {
                    ...
                    if ($row['merchid'] > 0) {
                        $row['storeinfo'] = "[".pdo_fetchcolumn('select storename from ' . tablename('vending_machine_merch_store') . ' where id=:storeid limit 1 ', array(':storeid' => $row['verifystoreid']))."]";
                        $salermember = pdo_fetch("SELECT * FROM ".tablename('vending_machine_merch_saler')." s left join ".tablename('vending_machine_member')." m on m.openid=s.openid
                         WHERE s.openid='{$row['verifyopenid']}'");
                        $row['salerinfo'] = "[" . $salermember['id'] . "]" . $salermember['salername'] . "(" . $salermember['nickname'] . ")";
                    } else {
                        $row['storeinfo'] = "[".pdo_fetchcolumn('select storename from ' . tablename('vending_machine_store') . ' where id=:storeid limit 1 ', array(':storeid' => $row['verifystoreid']))."]";
                        $salermember = pdo_fetch("SELECT s.id,s.salername,m.nickname FROM ".tablename('vending_machine_saler')." s left join ".tablename('vending_machine_member')." m on m.openid=s.openid
                         WHERE s.openid='{$row['verifyopenid']}'");
                        $row['salerinfo'] = "[" . $salermember['id'] . "]" . $salermember['salername'] . "(" . $salermember['nickname'] . ")";
                    }
                ...
                if (!empty($row['storeid'])) {
                    if ($row['merchid'] > 0) {
                        $row['pickname'] = pdo_fetchcolumn('select storename from ' . tablename('vending_machine_merch_store') . ' where id=:storeid limit 1 ', array(':storeid' => $row['storeid']));
                    } else {
                        $row['pickname'] = pdo_fetchcolumn('select storename from ' . tablename('vending_machine_store') . ' where id=:storeid limit 1 ', array(':storeid' => $row['storeid']));
                    }
                }
...
//            pdo_debug();die;
            ...
            $t = pdo_fetch(
                ...
                . " left join " . tablename('vending_machine_verifygoods_log') . " vgl on vgl.verifygoodsid = vg.id"
                . " left join " . tablename('vending_machine_verifyorder_log') . " vol on vol.orderid=o.id "
                . " left join " . tablename('vending_machine_store') . " store on ( store.id = vgl.storeid or store.id = vol.storeid ) and o.ismerch=0"
                .$leftsql
                . " $sqlcondition WHERE $condition AND o.status =3", $paras);
