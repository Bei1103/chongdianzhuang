...
require VENDING_MACHINE_INC . 'com_processor.php';
...
class CouponProcessor extends ComProcessor {
	...
	function getGuess($coupon, $openid) {
		...
		$lasttimes = 1;
		$times = 0;
		$guess = pdo_fetch('select id,times from ' . tablename('vending_machine_coupon_guess') . ' where couponid=:couponid and openid=:openid and pwdkey=:pwdkey and uniacid=:uniacid limit 1 '
	...
	function respondText($obj) {
		...
		if (isset($_SESSION['vending_machine_coupon_key'])) {
			$couponkey = $_SESSION['vending_machine_coupon_key'];
		} else {
			$_SESSION['vending_machine_coupon_key'] = $content;
		}
		
		$coupon = pdo_fetch('select id,couponname,pwdkey2,pwdask,pwdsuc,pwdfail,pwdfull,pwdtimes,pwdurl,pwdwords,pwdown,pwdexit,pwdexitstr,total from ' . tablename('vending_machine_coupon') . ' where pwdkey2=:pwdkey2 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':pwdkey2' => $couponkey));
		$words = explode(",", $coupon['pwdwords']);


		if (empty($coupon)) {
			$obj->endContext();
			unset($_SESSION['vending_machine_coupon_key']);
			return $this->responseEmpty();
		}
        ...
        if($coupon['total'] == '-1'){
            ...
            $gettotal = pdo_fetchcolumn('select count(*) from ' . tablename('vending_machine_coupon_data') . ' where couponid=:couponid and uniacid=:uniacid limit 1', array(':couponid' => $coupon['id'], ':uniacid' => $_W['uniacid']));
        ...
        if (!$obj->inContext) {
			$guessok = pdo_fetch('select id,times from ' . tablename('vending_machine_coupon_guess') . ' where couponid=:couponid and openid=:openid and pwdkey=:pwdkey and ok=1 and uniacid=:uniacid limit 1 '
					, array(':couponid' => $coupon['id'], ':openid' => $openid, ':pwdkey' => $coupon['pwdkey2'], ':uniacid' => $_W['uniacid']));
			if (!empty($guessok)) {
			...
			$guess = pdo_fetch('select id,times from ' . tablename('vending_machine_coupon_guess') . ' where couponid=:couponid and openid=:openid and pwdkey=:pwdkey and uniacid=:uniacid limit 1 ', array(':couponid' => $coupon['id'], ':openid' => $openid, ':pwdkey' => $coupon['pwdkey2'], ':uniacid' => $_W['uniacid']));
			...
			if (empty($guess)) {
				...
				pdo_insert('vending_machine_coupon_guess', $guess);
			} else {
				pdo_update('vending_machine_coupon_guess', array('times' => $guess['times'] + 1, 'ok' => $ok ? 1 : 0), array('id' => $guess['id']));
			}
