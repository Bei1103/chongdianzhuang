...
class VerifyProcessor extends ComProcessor
{
    ...
    public function __construct()
    {
        ...
        $this->sessionkey = VENDING_MACHINE_PREFIX . 'order_wechat_verify_info';
        $this->codekey = VENDING_MACHINE_PREFIX . 'order_wechat_verify_code';
    }
    ...
    public function respond($obj = null)
    {
        ...
        if ($msgtype == 'text' || $event == 'click') {
            $saler = pdo_fetch('select * from ' . tablename('vending_machine_saler') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            if (empty($saler)) {
                $saler = pdo_fetch('select * from ' . tablename('vending_machine_merch_saler') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }
            ...
            if (!$obj->inContext) {
                ...
                if (is_numeric($content)) {
                    ...
                    if (strlen($content) >= 8) {
                        ...
                        $member = pdo_get('vending_machine_member',array('openid'=>$order['openid']));
