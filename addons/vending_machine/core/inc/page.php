...
class Page extends WeModuleSite
{
    public function runTasks()
    {
        ...
        //自动完成订单
        $lasttime = strtotime(m('cache')->getString('finishorder', 'global'));
        $interval = 60;
        //如果上次自动关闭时间小
        $current = time();
        if ($lasttime + $interval <= $current) {
            m('cache')->set('finishorder', date('Y-m-d H:i:s', $current), 'global');
            ihttp_request(VENDING_MACHINE_TASK_URL . "order/finish.php", null, null, 10);
        }
        ...
        if ($lasttime + $interval <= $current) {
            ...
            (ihttp_request(VENDING_MACHINE_TASK_URL . "order/receive.php", null, null, 10));
        }
        ...
        if ($lasttime + $interval <= $current) {
            ...
            ihttp_request(VENDING_MACHINE_TASK_URL . "order/close.php", null, null, 10);
        }
        ...
        if ($lasttime + $interval_v <= $current) {
            ...
            ihttp_request(VENDING_MACHINE_TASK_URL . "order/close.php", array('uniacid'=>$_W['uniacid']), null, 10);
        }
        ...
        if ($lasttime + $interval <= $current) {
            ...
            ihttp_request(VENDING_MACHINE_TASK_URL . "order/fullback.php", null, null, 10);
        }
        /*
         * 预售商品到期自动下架
         * */
        ...
        if ($lasttime + $interval <= $current) {
            ...
            ihttp_request(VENDING_MACHINE_TASK_URL . "goods/presell.php", null, null, 10);
        }
        ...
        if ($lasttime + $interval <= $current) {
            ...
            ihttp_request(VENDING_MACHINE_TASK_URL . "goods/status.php", null, null, 10);
        }
        ...
        if (com('coupon')) {
            ...
            if ($lasttime + $interval <= $current) {
                ...
                //require_once VENDING_MACHINE_PATH.'core/task/order/willclose.php';
                ihttp_request(VENDING_MACHINE_TASK_URL . "order/willclose.php", null, null, 10);
            }
        }
        ...
        if (com('coupon')) {
            ...
            if ($lasttime + $interval <= $current) {
                ...
                ihttp_request(VENDING_MACHINE_TASK_URL . "coupon/back.php", null, null, 10);
            }
        }
        ...
        if ($lasttime + $interval <= $current) {
            ...
            ihttp_request(VENDING_MACHINE_TASK_URL . "notice/sendnotice.php", array('uniacid' => $_W['uniacid']), null, 10);
        }
        ...
        if ($lasttime + $interval <= $current) {
            ...
            ihttp_request(VENDING_MACHINE_TASK_URL . "cycelbuy/sendnotice.php", array('uniacid' => $_W['uniacid']), null, 10);
        }
        ...
        if ($lasttime + $interval <= $current) {
            ...
            ihttp_request(VENDING_MACHINE_TASK_URL . "cycelbuy/receive.php", array('uniacid' => $_W['uniacid']), null, 10);
        }
        ...
        if (p('groups')) {
            ...
            if ($groups_order_lasttime + $groups_order_interval <= $groups_order_current) {
                ...
                ihttp_request($_W['siteroot'] . "addons/vending_machine/plugin/groups/task/order.php", null, null, 10);
            }
            /*
             * 拼团失败自动退款
             * */
            ...
            if ($groups_team_lasttime + $groups_team_interval <= $groups_team_current) {
                ...
                ihttp_request($_W['siteroot'] . "addons/vending_machine/plugin/groups/task/refund.php?uniacid={$_W['uniacid']}", null, null, 10);
            }
            /*
             * 拼团发货自动收货
             * */
            ...
            if ($groups_receive_lasttime + $groups_receive_interval <= $groups_receive_current) {
                ...
                ihttp_request($_W['siteroot'] . "addons/vending_machine/plugin/groups/task/receive.php", null, null, 10);
            }
        }
        ...
        if (p('seckill')) {
            ...
            if ($lasttime + $interval <= $current) {
                ...
                ihttp_request($_W['siteroot'] . "addons/vending_machine/plugin/seckill/task/delete.php", null, null, 10);
            }
        }
        //卡密延迟60秒发送
//        ihttp_request($url=VENDING_MACHINE_TASK_URL . "order/virtualsend.php", array('uniacid'=>$_W['uniacid'],'acid'=>$_W['acid']),null,1);
//        exit('run finished.');
        ...
        if (p('friendcoupon')) {
            ...
            if ($lasttime + $interval <= $current) {
                ...
                ihttp_request($_W['siteroot'] . "addons/vending_machine/plugin/friendcoupon/task/sendMessage.php?uniacid={$_W['uniacid']}", null, null, 10);
            }
        }
        ...
        if (p('merch')) {
            ...
            if ($lasttime + $interval <= $current) {
                ...
                ihttp_request(VENDING_MACHINE_TASK_URL . "plugin/merch.php", null, null, 10);
            }
        }
    }
    ...
    public function template($filename = '', $type = TEMPLATE_INCLUDEPATH, $account = false){
        ...
        $moduleroot = IA_ROOT . "/addons/vending_machine";
