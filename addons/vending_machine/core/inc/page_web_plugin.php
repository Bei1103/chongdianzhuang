...
class PluginWebPage extends WebPage
{
    ...
    public function __construct($_init = true)
    {
        ...
        $this->modulename = 'vending_machine';
        ...
        if(strpos($this->pluginname,'open_messikefu')!==false){
            ...
            $key = pdo_fetch("SELECT * FROM ".tablename('vending_machine_open_plugin')." WHERE plugin = :plugin", array(':plugin' => $this->pluginname));
...
//            如果key为空或者没有设置的有问题的时候
            ...
            $info =  $this->checkOpen($key['key'],$key['plugin'],$key['domain']);
               ...
               if (!is_error($redis)) {
                ...
                pdo_update('vending_machine_open_plugin',array('expirtime'=>time()+ 172800),array('id'=>$key['id']));
            }
        }
