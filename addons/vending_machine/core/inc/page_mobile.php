...
class MobilePage extends Page {
    ...
    public function __construct() {
        ...
        if(!empty($wap['open']) /*&& !is_weixin()*/ && empty($preview)&&empty($openid)&&empty($type)) {
        ...
        {
            $this->merch_user = pdo_fetch("select * from ".tablename('vending_machine_merch_user')." where id=:id limit 1",array(':id'=>intval($_GPC['merchid'])));
        }
    }
    public function followBar($diypage=false, $merch=false) {
        ...
        if (!$followed || (!empty($diyfollowbar['params']['showtype']) && !empty($diyfollowbar['params']['isopen']))) {
            ...
            if (!empty($mid) && $memberid!=$mid) {
                if (!empty($_SESSION[VENDING_MACHINE_PREFIX . '_shareid']) && $_SESSION[VENDING_MACHINE_PREFIX . '_shareid'] == $mid) {
                    $mid = $_SESSION[VENDING_MACHINE_PREFIX . '_shareid'];
                }
                $member = m('member')->getMember($mid);
                if (!empty($member)) {
                    $_SESSION[VENDING_MACHINE_PREFIX . '_shareid'] = $mid;
                    $friend = true;
    ...
    public function MemberBar($diypage=false, $merch=false) {
        ...
        if (!empty($data['become_goodsid'])) {
            $goods = pdo_fetch('select id,title,thumb from ' . tablename('vending_machine_goods') . ' where id=:id and uniacid=:uniacid limit 1 ', array(':id' => $data['become_goodsid'], ':uniacid' => $_W['uniacid']));
        } else {
            return;
        }
    ...
    public function footerMenus($diymenuid=null, $ismerch=false, $texts=array()) {
        ...
        $cartcount = pdo_fetchcolumn('select ifnull(sum(total),0) from ' . tablename('vending_machine_member_cart') . ' where uniacid=:uniacid and openid=:openid and deleted=0 and isnewstore=0  and selected =1', $params);
        ...
        if(p('commission') && intval($_W['shopset']['commission']['level']>0)){
            ...
            if(!empty($diymenuid)) {
                $menu = pdo_fetch("SELECT * FROM " . tablename('vending_machine_diypage_menu') . " WHERE id=:id and uniacid=:uniacid limit 1 ", array(':id'=>$diymenuid, ':uniacid'=>$_W['uniacid']));
                if(!empty($menu)) {
                    $menu = $menu['data'];
        ...
        if(!empty($diypagedata)) {
            ...
            if(!empty($diypageid)) {
                ...
                if(!empty($page)) {
                    ...
                    if($type=='home'){
                        $cpinfos = false;
                        $sale_sql = 'SELECT * FROM '.tablename('vending_machine_sendticket').' WHERE uniacid = '.intval($_W['uniacid']);
                        ...
                        {
                            $order = pdo_fetch("select id,price,`virtual`,createtime  from ".tablename("vending_machine_order")." where uniacid=:uniacid and status = 0 and paytype<>3 and openid=:openid order by createtime desc limit 1",array(":uniacid"=>$_W['uniacid'],":openid"=>$_W['openid']));
                            ...
                            if(!empty($order))
                            {
                                ...
                                $goods = pdo_fetchall("select g.*,og.total as totals  from ".tablename("vending_machine_order_goods")." og inner join ".tablename("vending_machine_goods")." g on og.goodsid = g.id   where og.uniacid=:uniacid    and og.orderid=:orderid  limit 3",array(":uniacid"=>$_W['uniacid'],":orderid"=>$order['id']));

                                $goodstotal = pdo_fetchcolumn("select COUNT(*)  from ".tablename("vending_machine_order_goods")." og inner join ".tablename("vending_machine_goods")." g on og.goodsid = g.id   where og.uniacid=:uniacid    and og.orderid=:orderid ",array(":uniacid"=>$_W['uniacid'],":orderid"=>$order['id']));
    ...
    public function diyDanmu($diy=false) {
        ...
        if(empty($danmu['params']['datatype'])){
            ...
            $danmu['data'] = pdo_fetchall("SELECT nickname, avatar as imgurl FROM". tablename("vending_machine_member")." WHERE uniacid=:uniacid AND nickname!='' AND avatar!='' ".$condition." ORDER BY rand() LIMIT 10", array(":uniacid"=>$_W['uniacid']));
        ...
        elseif($danmu['params']['datatype']==1){
            $danmu['data'] = pdo_fetchall("SELECT m.nickname, m.avatar as imgurl, o.createtime as time FROM". tablename("vending_machine_order")." o LEFT JOIN ".tablename("vending_machine_member")." m ON m.openid=o.openid WHERE o.uniacid=:uniacid AND m.nickname!='' AND m.avatar!='' ORDER BY o.createtime DESC LIMIT 10", array(":uniacid"=>$_W['uniacid']));
        }
    ...
    private function checkOpen(){
        global $_GPC;
        if($_GPC['canChenk']){
            $key = pdo_fetchall("SELECT * FROM ".tablename('vending_machine_open_plugin'));
