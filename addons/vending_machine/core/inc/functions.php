...
if (!function_exists('m')) {
    function m($name = '')
    {
        ...
        $model = VENDING_MACHINE_CORE . "model/" . strtolower($name) . '.php';
...
if (!function_exists('d')) {
    function d($name = '')
    {
        ...
        $model = VENDING_MACHINE_CORE . "data/" . strtolower($name) . '.php';
        if (!is_file($model)) {
            die(' Data Model ' . $name . ' Not Found!');
        }
        require_once VENDING_MACHINE_INC . "data_model.php";
        require_once $model;
...
if (!function_exists('socket')) {
    function socket($name)
    {
        ...
        $socket = VENDING_MACHINE_PLUGIN . strtolower($name) . '/core/socket.php';
        if (!is_file($socket)) {
            return false;
        }
        require_once VENDING_MACHINE_CORE . "inc/plugin_model.php";
        require_once $socket;
...
if (!function_exists('p')) {
    function p($name = '')
    {
        ...
        $model = VENDING_MACHINE_PLUGIN . strtolower($name) . '/core/model.php';
        if (!is_file($model)) {
            return false;
        }
...
if (!function_exists('com')) {
    function com($name = '')
    {
        ...
        $model = VENDING_MACHINE_CORE . "com/" . strtolower($name) . '.php';
        if (!is_file($model)) {
            return false;
        }

        require_once VENDING_MACHINE_CORE . "inc/com_model.php";
        require_once $model;
...
if (!function_exists('com_perm_check_plugin')) {
    function com_perm_check_plugin($pluginname = '')
    {
        ...
        //先判断公众号的
        $ac_perm = pdo_fetch('select  plugins from ' . tablename('vending_machine_perm_plugin') . ' where acid=:acid limit 1', array(':acid' => $acid));
        if (!empty($ac_perm)) {
...
if (!function_exists('com_perm_check_com')) {
    function com_perm_check_com($comname)
    {
        ...
        $acid = pdo_fetchcolumn("SELECT acid FROM " . tablename('account_wechats') . " WHERE `uniacid`=:uniacid LIMIT 1", array(':uniacid' => $_W['uniacid']));
        //先判断公众号的
        $ac_perm = pdo_fetch('select  coms from ' . tablename('vending_machine_perm_plugin') . ' where acid=:acid limit 1', array(':acid' => $acid));
        if (!empty($ac_perm)) {
...
if (!function_exists('check_operator_perm')) {
    function check_operator_perm($pluginname = '')
    {
        ...
        //当前操作员
        $item = pdo_fetch("SELECT * FROM " . tablename('vending_machine_perm_user') . " WHERE uid =:uid and deleted=0 and uniacid=:uniacid limit 1", array(':uniacid' => $_W['uniacid'], ':uid' => $uid));
        if (!empty($item)) {
            //所属角色
            $role = pdo_fetch("SELECT * FROM " . tablename('vending_machine_perm_role') . " WHERE id =:id and deleted=0 and uniacid=:uniacid limit 1", array(':uniacid' => $_W['uniacid'], ':id' => $item['roleid']));
            if (!empty($role)) {
...
if (!function_exists('is_h5app')) {
    function is_h5app()
    {
        if (!empty($_SERVER['HTTP_USER_AGENT']) && strpos($_SERVER['HTTP_USER_AGENT'], 'vending_machine')!==false) {
            return true;
        }
        return false;
    }
...
if (!function_exists('get_auth')) {
    ...
    {
        global $_W;
        $set = pdo_fetch('select sets from ' . tablename('vending_machine_sysset') . ' order by id asc limit 1');
...
if (!function_exists('rc')) {
    function rc($plugin = '')
    {
        ...
        $resp = ihttp_request(VENDING_MACHINE_AUTH_URL, array('ip' => $ip, 'id' => $id, 'code' => $auth['code'], 'domain' => $domain, 'plugin' => $plugin), null, 1);
        ...
        foreach ($options['items'] as $item) {
            if ($options['type'] == 'image') {
                $html .= "<div class='multi-item' data-{$options['key']}='{$item[$options['key']]}' data-name='{$name}'>
                                      <img class='img-responsive img-thumbnail' src='" . tomedia($item[$options['thumb']]) . "' onerror='this.src=\"../addons/vending_machine/static/images/nopic.png\"' style='width:100px;height:100px;'>
                ...
                $html .= "
                    <tr class='multi-product-item' data-{$options['key']}='{$item['id']}' >
                       ...
                       <input type='hidden'  value='{$item['goodsid']}' name='{$id}'>
                        <td style='width:80px;'>
                            <img src='" . tomedia($item[$options['thumb']]) . "' style='width:70px;border:1px solid #ccc;padding:1px' onerror=\"this.src='../addons/vending_machine/static/images/nopic.png'\">
                        </td>
...
if (!function_exists('mobileUrl')) {
    function mobileUrl($do = '', $query = null, $full = false)
    {
        ...
        $query = array_merge(array('m' => "vending_machine"), $query);
        ...
        if (empty($query['mid'])) {
            ...
            if (!empty($_W['openid']) && !is_weixin() && !is_h5app()) {
                ...
                if (!empty($myid)) {
                    $member = pdo_fetch("select id,isagent,status from" . tablename('vending_machine_member') . 'where id=' . $myid);
...
if (!function_exists("redis")) {
    ...
    function redis()
    {
        ...
        if (is_null($redis)) {
            ...
            if (!isset($_W['config']['setting']['redis']) && !isset($_W['config']['setting']['vending_machine_redis'])) {
                return error(-1, '未配置 redis, 请检查 data/config.php 中参数设置');
            }

            $config = isset($_W['config']['setting']['vending_machine_redis']) ? $_W['config']['setting']['vending_machine_redis'] : $_W['config']['setting']['redis'];
...
if (!function_exists("tpl_form_field_image2")) {
    function tpl_form_field_image2($name, $value = '', $default = '', $options = array())
    {
        ...
        if (empty($default)) {
            $default = '../addons/vending_machine/static/images/nopic.png';
        }
        $val = $default;
        if (!empty($value)) {
            $val = tomedia($value);
        } else {
            $val = '../addons/vending_machine/static/images/default-pic.jpg';
        }
		...
		<script type="text/javascript">
			...
			function deleteImage(elm){
				require(["jquery"], function($){
					$(elm).prev().attr("src", "../addons/vending_machine/static/images/default-pic.jpg");
	...
	function uploadMultiImage(elm) {
		...
		util.image( "", function(urls){
			$.each(urls, function(idx, url){
				$(elm).parent().parent().next().append(\'<div class="multi-item"><img onerror="this.src=\\\'../addons/vending_machine/static/images/nopic.png\\\'; this.title=\\\'图片未找到.\\\'" src="\'+url.url+\'" class="img-responsive img-thumbnail"><input type="hidden" name="\'+name+\'[]" value="\'+url.attachment+\'"><em class="close" title="删除这张图片" onclick="deleteMultiImage(this)">×</em></div>\');
			});
		}, ' . json_encode($options) . ');
	}
...
<div class="multi-item">
	<img src="' . tomedia($row) . '" onerror="this.src=\'../addons/vending_machine/static/images/nopic.png\'; this.title=\'图片未找到.\'" class="img-responsive img-thumbnail">
...
if (!function_exists('tpl_form_field_shop_daterange')) {
    function tpl_form_field_shop_daterange($name, $value = array(), $time = false)
    {
        $s = '';
...
if (!function_exists('tpl_form_field_shop_date')) {
    function tpl_form_field_shop_date($name, $value = '', $withtime = false)
    {
        $s = '';
        ...
        if (!defined('TPL_INIT_COORDINATE')) {
            $s .= '<script type="text/javascript">
                    function showCoordinate(elm) {
                            ...
                            },"' . VENDING_MACHINE_URL . 'template/web/util/area/map.html' . '");
    }
...
if (!function_exists('tpl_goods_selector')) {
    ...
    function tpl_goods_selector($name, $data_gs = '', $option = array())
    {
        ...
        //包含模板文件
        $path = IA_ROOT . '/addons/vending_machine/template/web_v3/util/tpl_goods_selector.html';
        include $path;
...
function queue_push($job,$delay = 0,$priority = 1024){
    ...
    pdo_insert("vending_machine_queue",array(
        'channel' => 'queue',
        'job' => serialize($job),
        'pushed_at' => time(),
        'ttr' => null,
