...
class Receiver extends WeModuleReceiver {
    ...
    public function saleVirtual($obj=null)
    {
        ...
        $open_redis = function_exists('redis') && !is_error(redis());
        if( $open_redis ) {
            $redis_key1 = "vending_{$_W['uniacid']}_member_salevirtual_isagent";
            $redis_key2 = "vending_{$_W['uniacid']}_member_salevirtual";
            $redis = redis();
            if (!is_error($redis)) {
                if ($redis->get($redis_key1) != false) {
                    ...
                    $totalagent = pdo_fetchcolumn("select count(*) from" . tablename('vending_machine_member') . " where uniacid =" . $_W['uniacid'] . " and isagent =1");
                    $totalmember = pdo_fetchcolumn("select count(*) from" . tablename('vending_machine_member') . " where uniacid =" . $_W['uniacid']);
        ...
        //超级海报会员关注
        if ($_SESSION['shop']['poster_member']){
            $user['isnew'] = true;
            $_SESSION['shop']['poster_member'] = null;
        }
        if ($user['isnew'])
        {
