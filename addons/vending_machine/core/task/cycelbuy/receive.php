...
if(!empty($order)){
    ...
    foreach($order as $k => $v){
        $last_periods= pdo_fetch("select * from " . tablename('vending_machine_cycelbuy_periods') . ' where uniacid=:uniacid and orderid=:orderid order by id desc  limit 1', array(':uniacid' => $_W['uniacid'], ':orderid' => $v['id']));
        $cycel = pdo_fetchall('select * from ' .tablename('vending_machine_cycelbuy_periods'). ' where  orderid = '.$v['id'].' and status = 1 and uniacid = '.$_W['uniacid'].' order by receipttime asc limit 1');
        ...
        if($sendtime+$days >= time()){
            ...
            if(!empty($last_periods)){
                if($last_periods['id']==$cycel[0]['id']){
                    pdo_update('vending_machine_cycelbuy_periods', array( 'status' => 2,'finishtime'=>time()), array('orderid' => $v['id'], 'uniacid' => $_W['uniacid']));
                    pdo_update('vending_machine_order', array('status' => 3,'finishtime'=>time()), array('id' =>$v['id'],'status'=>2));
                    //会员升级
...
//                    优惠券返利
                    ...
                    pdo_update('vending_machine_cycelbuy_periods', array( 'status' => 2,'finishtime'=>time()), array('id' => $cycel[0]['id'], 'uniacid' => $_W['uniacid']));
                }
            }
