...
if (!empty($sets)) {
    foreach ($sets as $set) {
        //查询已过期多商户sql
        $sql = 'SELECT id,accounttime FROM ' . tablename('vending_machine_merch_user') . ' WHERE accounttime <= '.time().' AND uniacid = :uniacid';
        $params = array(
            ':uniacid' => $set['uniacid']
        );
        //查询结果
        ...
        if (!empty($merchUsers)) {
            foreach ($merchUsers as $merchUser) {
                pdo_update('vending_machine_goods', array('status' => 0), array('merchid' => $merchUser['id'], 'uniacid' => $set['uniacid']));
            }
        }
    }
}
