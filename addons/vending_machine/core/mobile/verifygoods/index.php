...
class Index_Page extends MobilePage {
    ...
    function main() {
        ...
        /*$verifygoods = pdo_fetchall('select vg.*,g.title,g.subtitle,g.thumb  from ' . tablename('vending_machine_verifygoods') . '   vg
    ...
    function detail() {
        ...
        if(!empty($_GPC['card_id'])&&!empty($_GPC['encrypt_code'])&&!empty($_GPC['openid']))
        {
            ...
            $item = pdo_fetch('select vg.*,g.title,g.subtitle,g.thumb  from ' . tablename('vending_machine_verifygoods') . '   vg
		 inner join ' . tablename('vending_machine_order_goods') . ' og on vg.ordergoodsid = og.id
		 left  join ' . tablename('vending_machine_order') . ' o on vg.orderid = o.id
		 left  join ' . tablename('vending_machine_order_refund') . ' orf on o.refundid = orf.id
		 inner join ' . tablename('vending_machine_goods') . ' g on og.goodsid = g.id
...
//				header('location: ' . mobileUrl('verifygoods'));
        ...
        if(empty($verifycode)||$item['codeinvalidtime']<time())
        {
            ...
            while (1) {
                $count = pdo_fetchcolumn('select count(*) from ' . tablename('vending_machine_verifygoods') . ' where verifycode=:verifycode and uniacid=:uniacid limit 1', array(':verifycode' => $verifycode, ':uniacid' => $_W['uniacid']));
                if ($count <= 0) {
                    break;
                }
            ...
            pdo_update("vending_machine_verifygoods",$data,array("id"=>$item['id']));
        ...
        $goodsstore = pdo_fetch('select * from ' . tablename('vending_machine_store') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $item['storeid'], ':uniacid' => $_W['uniacid']));
