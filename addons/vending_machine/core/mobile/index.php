...
class Index_Page extends MobilePage {
	...
	function getpage(){
		...
		$advs = pdo_fetchall("select id,advname,link,thumb from " . tablename('vending_machine_adv') . ' where uniacid=:uniacid and iswxapp=0 and enabled=1 order by displayorder desc', array(':uniacid' => $uniacid));
		$navs = pdo_fetchall("select id,navname,url,icon from " . tablename('vending_machine_nav') . ' where uniacid=:uniacid and iswxapp=0 and status=1 order by displayorder desc', array(':uniacid' => $uniacid));
		$cubes = is_array($_W['shopset']['shop']['cubes'])?$_W['shopset']['shop']['cubes']:array();
		$banners = pdo_fetchall("select id,bannername,link,thumb from " . tablename('vending_machine_banner') . ' where uniacid=:uniacid and iswxapp=0 and enabled=1 order by displayorder desc', array(':uniacid' => $uniacid));
		...
		if(!empty($_W['shopset']['shop']['indexrecommands'])){
			...
			if(!empty($goodids)){
				$indexrecommands = pdo_fetchall("select * from " . tablename('vending_machine_goods') . " where id in( $goodids ) and uniacid=:uniacid and deleted = 0 and status=1 order by instr('{$goodids}',id),displayorder desc", array(':uniacid' => $uniacid));
		...
		$notices = pdo_fetchall("select id, title, link, thumb from " . tablename('vending_machine_notice') . ' where uniacid=:uniacid and iswxapp=0 and status=1 order by displayorder desc limit 5', array(':uniacid' => $uniacid));
