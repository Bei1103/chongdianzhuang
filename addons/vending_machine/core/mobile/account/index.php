...
class Index_Page extends MobilePage
{
    ...
    public function login(){
        ...
        if($_W['ispost']){
            ...
            $member =pdo_fetch('select id,openid,mobile,pwd,salt from '.tablename('vending_machine_member').' where mobile=:mobile and mobileverify=1 and uniacid=:uniacid limit 1',array(':mobile'=>$mobile,':uniacid'=>$_W['uniacid']));
    ...
    public function verifycode(){
        ...
        if(!empty($set['wap']['smsimgcode'])){
            ...
            // 兼容微擎生成验证码加密方式改动
            $key = function_exists('complex_authkey') ? complex_authkey()
