...
class My_Page extends MobileLoginPage {
	...
	function detail() {
		...
		$id = intval($_GPC['id']);
		$data = pdo_fetch('select * from ' . tablename('vending_machine_coupon_data') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
		if (empty($data)) {
			if (empty($coupon)) {
		...
		$num = pdo_fetchcolumn('select ifnull(count(*),0) from ' . tablename('vending_machine_coupon_data') . ' where couponid=:couponid and openid=:openid and uniacid=:uniacid and used=0 '
	...
	function getlist(){
		...
		pdo_update('vending_machine_coupon_data', array('isnew' => 0), array('uniacid' => $_W['uniacid'], 'openid' =>$_W['openid']));
	...
	function showcoupons2() {
		...
		$data = pdo_fetch('select c.*  from ' . tablename('vending_machine_coupon_data') . '  cd inner join  ' . tablename('vending_machine_coupon') . ' c on cd.couponid = c.id  where cd.id=:id and cd.uniacid=:uniacid and coupontype =0  limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
...
//						$table .=' union all ';
//					}
//					$table .='select * from '.tablename('vending_machine_goods').' where FIND_IN_SET('.$cateid.',cates) and deleted =0';
//
//				}
//
//				$table .=') g';
