...
class Detail_Page extends MobilePage {
    ...
    function main() {
        ...
        if($coupon['limitgoodtype']!=0)
        {
            ...
            $goods = pdo_fetchall('select `title` from ' . tablename('vending_machine_goods') . ' where uniacid=:uniacid '.$where, array(':uniacid' => $_W['uniacid']), 'id');
        }
        if($coupon['limitgoodcatetype']!=0)
        {
            ...
            $category = pdo_fetchall('select `name`  from ' . tablename('vending_machine_category') . ' where uniacid=:uniacid   '.$where, array(':uniacid' => $_W['uniacid'],), 'id');
        }
        ...
        if($coupon['islimitlevel'] ==1) {
            ...
            if(!empty($coupon['limitmemberlevels'])||$coupon['limitmemberlevels']=='0')
            {
                ...
                //会员等级
                $level1 = pdo_fetchall('select * from ' . tablename('vending_machine_member_level') . ' where uniacid=:uniacid and  id in ('.$coupon['limitmemberlevels'].') ', array(':uniacid' => $_W['uniacid']));
            ...
            if((!empty($coupon['limitagentlevels'])||$coupon['limitagentlevels']=='0')&&$hascommission) {
                //分销商等级
                $level2 = pdo_fetchall('select * from ' . tablename('vending_machine_commission_level') . ' where uniacid=:uniacid and id  in ('.$coupon['limitagentlevels'].') ', array(':uniacid' => $_W['uniacid']));
            ...
            if((!empty($coupon['limitpartnerlevels'])||$coupon['limitpartnerlevels']=='0')&&$hasglobonus) {
                //股东等级
                $level3 = pdo_fetchall('select * from ' . tablename('vending_machine_globonus_level') . ' where uniacid=:uniacid and  id in('.$coupon['limitpartnerlevels'].') ', array(':uniacid' => $_W['uniacid']));
            ...
            if((!empty($coupon['limitaagentlevels'])||$coupon['limitaagentlevels']=='0')&&$hasabonus) {
                //区域代理
                $level4 = pdo_fetchall('select * from ' . tablename('vending_machine_abonus_level') . ' where uniacid=:uniacid and  id in ('.$coupon['limitaagentlevels'].') ', array(':uniacid' => $_W['uniacid']));
    ...
    public function pay($a=array(), $b=array()){
        ...
        if ($coupon['money'] > 0) {
            //删除以前无效的记录
            pdo_delete('vending_machine_coupon_log', array('couponid' => $id, 'openid' => $openid, 'status' => 0,'paystatus'=>0));
            $needpay = true;
            //找出上次支付但未参加的记录（例如断电，断网等特殊情况)
            $lastlog = pdo_fetch('select * from ' . tablename('vending_machine_coupon_log') . ' where couponid=:couponid and openid=:openid  and status=0 and paystatus=1 and uniacid=:uniacid limit 1', array(':couponid' => $id, ':openid' => $openid, ':uniacid' => $_W['uniacid']));
            if (!empty($lastlog)) {
                show_json(1, array('logid' => $lastlog['id']));
            }
        }else{
            //删除以前无效的记录
            pdo_delete('vending_machine_coupon_log', array('couponid' => $id, 'openid' => $openid, 'status' => 0));
        }
...
//        }
        ...
        if ($needpay) {
            ...
            //支付方式
            pdo_update('vending_machine_coupon_log', array('paytype' => $useweixin ? 1 : 0), array('id' => $logid));
    ...
    public function payresult($a=array()){
        ...
        $log = pdo_fetch('select id,logno,status,paystatus,couponid from ' . tablename('vending_machine_coupon_log') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $logid, ':uniacid' => $_W['uniacid']));
        if(empty($log)){
            show_json(-1, "订单未找到");
        }
