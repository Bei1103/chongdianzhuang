...
class Share_Page extends MobileLoginPage {
    ...
    function enjoy(){
        ...
        $openid = $_W['openid'];
        if (!empty($_GPC['sid'])) {
            $asql = 'SELECT * FROM '.tablename('vending_machine_sendticket_share').' WHERE uniacid = '.intval($_W['uniacid']).' AND id = '.intval($_GPC['sid']).' AND status = 1 AND (expiration = 0 OR (expiration = 1 AND endtime >= '.TIMESTAMP.'))';
        ...
        $csql = 'SELECT * FROM '.tablename('vending_machine_coupon_data').' WHERE uniacid = '.intval($_W['uniacid']).' AND shareident = "'.trim($_GPC['ident']).'" AND openid = "'.$openid.'"';
        ...
        if (!empty($clist)) {
            ...
            if(!empty($cpid)){
                ...
                foreach ($cpid as $ck => $cv) {
                    $couponsql = 'SELECT * FROM '.tablename('vending_machine_coupon').' WHERE uniacid = '.intval($_W['uniacid']).' AND id = '.$cv;
        ...
        $storesql = 'SELECT * FROM '.tablename('vending_machine_sysset').' WHERE uniacid = '.intval($_W['uniacid']);
    ...
    function getCoupon($cpids){
        global $_W;
        $sql = 'SELECT * FROM '.tablename('vending_machine_coupon').' WHERE  uniacid = '.intval($_W['uniacid']).' AND id IN ('.$cpids.')';
...
//        $did = array();
        for($i = 0;$i<$num;$i++){
            ...
            $datas = pdo_insert('vending_machine_coupon_data', $data);
        }
    ...
    function shareCoupon(){
        ...
        if($shareOpenid == $openid){
            ...
            pdo_update('vending_machine_order',$data,array('id'=>intval($_GPC['orderid'])));
    ...
    function sharePage(){
        ...
        foreach ($firendlist as $fk => $fv) {
            ...
            $firendlist[$fk]['nickname'] = $member['nickname'];
            $firendlist[$fk]['text'] = $this -> textClause($fv['textkey']);
//            $groupSql = 'SELECT * FROM '.tablename('vending_machine_coupon_data').' WHERE uniacid = '.intval($_W['uniacid']).' AND openid = "'.$fv['openid'].'" AND shareident = "'.$fv['shareident'].'" AND id <> '.intval($fv['did']) ;
...
//            }
    ...
    function shareSuccess(){
        ...
        $openid = $_W['openid'];
            ...
            $orderSql = 'SELECT isshare FROM '.tablename('vending_machine_order').' WHERE uniacid = '.intval($_W['uniacid']).' AND id = '.intval($_GPC['orderid']);
            ...
            $couponDataSql = 'SELECT * FROM '.tablename('vending_machine_coupon_data').' WHERE uniacid = '.intval($_W['uniacid']).' AND shareident = "'.$_GPC['ident'].'"';
