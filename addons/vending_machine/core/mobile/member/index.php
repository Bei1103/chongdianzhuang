	...
	function main() {
		...
		if(com('wxcard')) {
			...
			if (($wxcardupdatetime + 86400) < time()) {
				...
				pdo_update("vending_machine_member", array('wxcardupdatetime' => time()), array('openid' => $_W['openid']));
			}
		}
        ...
        $statics = array(
            'order_0'=>  pdo_fetchcolumn('select count(*) from '.tablename('vending_machine_order').' where openid=:openid and status=0 and (isparent=1 or (isparent=0 and parentid=0)) and paytype<>3 and uniacid=:uniacid and istrade=0 and userdeleted=0',$params),
            'order_1'=>  pdo_fetchcolumn('select count(*) from '.tablename('vending_machine_order').' where openid=:openid and (status=1 or (status=0 and paytype=3) or status=2) and isparent=0 and refundid=0 and uniacid=:uniacid and istrade=0 and userdeleted=0',$params),
            'order_3'=>  pdo_fetchcolumn('select count(*) from '.tablename('vending_machine_order').' where openid=:openid and status=3 and isparent=0 and refundid=0 and uniacid=:uniacid and istrade=0 and userdeleted=0',$params),
            'order_4'=>  pdo_fetchcolumn('select count(*) from '.tablename('vending_machine_order').' where openid=:openid and status=-1 and isparent=0 and uniacid=:uniacid and istrade=0 and userdeleted=0',$params),
        );
...
//			$statics['newcoupon'] = pdo_fetchcolumn("SELECT 1  FROM ".tablename('vending_machine_coupon_data')." where openid=:openid and uniacid=:uniacid and  isnew=1", array(':openid'=>$_W['openid'],':uniacid'=>$_W['uniacid']));
			...
			if($hascoupon){
                ...
                $cardnum =  pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('vending_machine_wxcard') . " where  uniacid=:uniacid and gettype =1", array(':uniacid'=>$_W['uniacid']));
			    $cardnum += $cardnum;
            }
		}
        ...
        if(p('mmanage')){
            $roleuser = pdo_fetch("SELECT id, uid, username, status FROM". tablename("vending_machine_perm_user"). "WHERE openid=:openid AND uniacid=:uniacid AND status=1 LIMIT 1", array(":openid"=>$_W['openid'], ":uniacid"=>$_W['uniacid']));
        }
        ...
        //IC卡
        $iccard=false;//默认不能使用IC卡
        if(p('iccard') && m('plugin')->permission('iccard')&&is_weixin())
        {
            $iccard=true;
        }
