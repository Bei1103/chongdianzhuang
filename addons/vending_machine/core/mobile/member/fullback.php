...
class Fullback_Page extends MobileLoginPage {
    ...
    function main() {
        ...
        $info = pdo_fetchall("SELECT * FROM". tablename("vending_machine_fullback_log").$condition, $params);
        $alldata = array();
        $allday = array();
        $fullbackday = array();
        $createtime = array();
    ...
    function get_list(){
        ...
        $list = array();
        $list = pdo_fetchall("select fl.*,g.thumb,g.title from " . tablename('vending_machine_fullback_log') . " as fl
            left join ".tablename('vending_machine_goods')." as g on g.id = fl.goodsid
            left join ".tablename('vending_machine_order_goods')." as og on og.orderid = fl.orderid and og.goodsid = fl.goodsid 
            where 1 {$condition} group by fl.id order by fl.createtime desc LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);

        $total = pdo_fetchcolumn('select count(1) from ' . tablename('vending_machine_fullback_log') . " as fl
        ...
        foreach ($list as &$row) {
            ...
            if ($row['optionid']>0){
                $optionname = pdo_get('vending_machine_order_goods',array('optionid'=>$row['optionid']),array('optionname'));
