...
class Recharge_Page extends MobileLoginPage {
    ...
    function submit() {
        ...
        $sql = "DELETE FROM " . tablename('vending_machine_member_log') . " WHERE openid= '{$_W['openid']}' AND status = 0 AND uniacid = {$_W['uniacid']} AND createtime < (unix_timestamp()-86400)";
        pdo_fetch($sql);
        ...
        pdo_insert('vending_machine_member_log', $log);
        $logid = pdo_insertid();
        $type = $_GPC['type'];
    ...
    function wechat_complete() {
        ...
        $log = pdo_fetch('SELECT * FROM ' . tablename('vending_machine_member_log') . ' WHERE `id`=:id and `uniacid`=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $logid));
        if(empty($log)){
...
/*
    function alipay_complete() {
        ...
        if (!empty($log) && empty($log['status'])) {
            ...
            //充值状态
            pdo_update('vending_machine_member_log', array('status' => 1, 'rechargetype' => 'alipay', 'apppay'=>is_h5app()?1:0), array('id' => $log['id']));
            //增加会员余额
