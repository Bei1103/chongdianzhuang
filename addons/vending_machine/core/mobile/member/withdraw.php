...
class Withdraw_Page extends MobileLoginPage {
    ...
    function main() {
        ...
        if($set['withdrawcashcard'] == 1) {
            ...
            $banklist = pdo_fetchall("SELECT * FROM " . tablename('vending_machine_commission_bank') . " WHERE 1 {$condition}  ORDER BY displayorder DESC", $params);
        }
    ...
    function submit() {
        ...
        pdo_insert('vending_machine_member_log', $apply);
        $logid = pdo_insertid();
        if (empty($logid)) {
    ...
    function getLastApply($openid, $applytype = -1)
    {
        ...
        $sql = "select applytype,alipay,bankname,bankcard,realname from " . tablename('vending_machine_member_log') . " where openid=:openid and uniacid=:uniacid";
