...
class Activation_Page extends MobileLoginPage
{
    ...
    function main()
    {
        ...
        $item = pdo_fetch("select * from " . tablename("vending_machine_member") . " where uniacid=:uniacid and openid =:openid limit 1 ", array(":uniacid" => $_W['uniacid'], ":openid" => $_W['openid']));
        ...
        pdo_update('vending_machine_member', $arr, array('openid' => $_W['openid'], 'uniacid' => $_W['uniacid']));
        ...
        if (empty($CardActivation['openactive'])) {
            ...
            if (empty($result) || is_wxerror($result)) {
                ...
                if (empty($item['membercardactive'])) {
                    pdo_update('vending_machine_member', array('membercardactive' => 1), array('openid' => $_W['openid'], 'uniacid' => $_W['uniacid']));
    ...
    public function submit()
    {
        ...
        $ret = pdo_get('vending_machine_member', array(
            'uniacid' => $_W['uniacid'],
            'openid' => $_W['openid']
        ));
        ...
        if (!empty($CardActivation['openactive'])) {
            if (!empty($CardActivation['sms_active']) && !empty($CardActivation['mobile'])) {
                ...
                $key = '__vending_machine_member_verifycodesession_' . $_W['uniacid'] . '_' . trim($_GPC['mobile']);
                $code = $_SESSION[$key];
    ...
    public function verifycode()
    {
        ...
        $member = pdo_fetch('select id,openid,mobile,pwd,salt from ' . tablename('vending_machine_member') . ' where mobile=:mobile and openid <>:openid  and mobileverify=1 and uniacid=:uniacid limit 1', array(':mobile' => $mobile, ':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']));
