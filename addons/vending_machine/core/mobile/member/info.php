...
class Info_Page extends MobileLoginPage {
...
//                pdo_update( 'vending_machine_member', array('avatar' => $userinfo['headimgurl']),array('id' => $this->member['id'] , 'uniacid' => $this->member['uniacid']) );
//                //如果不同的话替换掉取出的原来的信息
...
//        }
	...
	function submit() {
		...
		if ($template_flag == 1) {
			...
			pdo_update('vending_machine_member', $m_data, array('openid' => $_W['openid'], 'uniacid' => $_W['uniacid']));
	...
	public function face() {
        ...
        if($_W['ispost']){
            ...
            pdo_update('vending_machine_member', array('avatar'=>$avatar, 'nickname'=>$nickname), array('id'=>$member['id'], 'uniacid'=>$_W['uniacid']));
            show_json(1);
        }
