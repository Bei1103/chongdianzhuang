...
class Single_Refund_Page extends MobileLoginPage {
...
//        dump(iunserializer($order['consume']));die;
        ...
        //存在全返，无法进行单商品退款
        $fullback_goods_count = pdo_fetchcolumn('select count(og.id) from ' . tablename('vending_machine_order_goods') .' og '
            . ' left join ' . tablename('vending_machine_goods') . ' g on g.id=og.goodsid'
        ...
        $refund = pdo_fetch("select id.orderid from ".tablename('vending_machine_order_refund')." where uniacid = :uniacid and orderid = :orderid and :goodsid in (ordergoodsids) and status > 0 limit 1 ",array(':uniacid'=>$uniacid,':orderid'=>$order['id'],':goodsid'=>$order_goodsid));
        if(!empty($refund)){
        ...
        //订单所有商品 关联 退款表
        $order_goods = pdo_fetchall('select og.id,og.single_refundid,og.single_refundstate,r.applyprice from ' . tablename('vending_machine_order_goods') .' og '
            . ' left join ' . tablename('vending_machine_order_single_refund') . ' r on r.id=og.single_refundid'
...
//        }
        ...
        if (!empty($single_refundid)) {
            $refund = pdo_fetch("select * from " . tablename('vending_machine_order_single_refund') . ' where id=:id and ordergoodsid=:ordergoodsid and uniacid=:uniacid limit 1'
...
//
        ...
        if ($order['single_refundstate'] == 0) {
            ...
            pdo_insert('vending_machine_order_single_refund', $refund);
            $single_refundid = pdo_insertid();
            pdo_update('vending_machine_order_goods', array('single_refundid' => $single_refundid, 'single_refundstate' => $refundstate), array('id' => $order_goodsid, 'uniacid' => $uniacid));
        } else {
            //修改退款申请
            $refund['status']=0;
            pdo_update('vending_machine_order_goods', array('single_refundstate' => $refundstate), array('id' => $order_goodsid, 'uniacid' => $uniacid));
            pdo_update('vending_machine_order_single_refund', $refund, array('id' => $single_refundid, 'uniacid' => $uniacid));
        }

        pdo_update('vending_machine_order', array('refundstate' => 3,'refundtime'=>0), array('id' => $order['id'], 'uniacid' => $uniacid));
    ...
    function cancel() {
        ...
        pdo_update('vending_machine_order_single_refund', $change_refund, array('id' => $single_refundid, 'uniacid' => $uniacid));
        pdo_update('vending_machine_order_goods', array('single_refundstate' => 0), array('id' => $order_goodsid, 'uniacid' => $uniacid));

        $order_goods=pdo_fetchall("select single_refundid,single_refundstate,single_refundtime from ".tablename('vending_machine_order_goods')." where orderid=:orderid",array(':orderid'=>$order['id']));
        ...
        if(empty($apply_refund_num) && !empty($refund_num)){
            pdo_update('vending_machine_order', array('refundtime'=>time()), array('id' => $order['id'], 'uniacid' => $uniacid));
        }

        if(empty($apply_refund_num) && empty($refund_num)){
            pdo_update('vending_machine_order', array('refundstate' => 0,'refundtime'=>0), array('id' => $order['id'], 'uniacid' => $uniacid));
        }
    ...
    function receive(){
        ...
        pdo_update('vending_machine_order_single_refund', $refund_data, array('id'=>$single_refundid, 'uniacid' => $uniacid));

        $order_data = array();
        $order_data['single_refundstate'] = 9;
        pdo_update('vending_machine_order_goods', $order_data, array('id'=>$order_goodsid, 'uniacid' => $uniacid));

        //查询其它 订单商品 是否有正在维权中的
        $is_single_refund=pdo_fetchcolumn('select count(id) from '.tablename('vending_machine_order_goods').'where orderid=:orderid and (single_refundstate=1 or single_refundstate=2)',array(':orderid'=>$order['id']));
