...
class Pay_Page extends MobileLoginPage
{
...
//            }
            ...
            if (pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('vending_machine_order_peerpay_payinfo')." WHERE openid = :openid AND pid = :pid",array(':openid'=>$_W['openid'],':pid'=>$ispeerpay['id']))){
                $this->message('每人只能代付一次');
            }
            $peerpayMessage = trim($_GPC['peerpaymessage']);
            $peerpay_info = (float)pdo_fetchcolumn("select SUM(price) from " . tablename('vending_machine_order_peerpay_payinfo') . ' where pid=:pid limit 1', array(':pid' => $ispeerpay['id']));
            ...
            if (empty($peerprice) || $peerprice<=0){
                ...
                $openid = pdo_fetchcolumn("SELECT openid FROM ".tablename('vending_machine_order')." WHERE id = :id AND uniacid = :uniacid LIMIT 1",array(':id'=>$orderid,':uniacid'=>$_W['uniacid']));
            }
        }

        $order = pdo_fetch("select * from " . tablename('vending_machine_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(':id' => $orderid, ':uniacid' => $uniacid, ':openid' => $openid));
        ...
        if((empty($order['isnewstore']) || empty($order['storeid']))&&empty($order['istrade']))
        {
            ...
            if(p('newstore')){
                $sql = "select og.id,g.title, og.goodsid,og.optionid,ng.stotal as stock,og.total as buycount,ng.gstatus as status,ng.deleted,g.maxbuy,g.usermaxbuy,g.istime,g.timestart,g.timeend,g.buylevels,g.buygroups,g.totalcnf,og.seckill,og.storeid "
                    . " from " . tablename('vending_machine_order_goods')
                    . " og left join  " . tablename('vending_machine_goods') . " g  on g.id=og.goodsid and g.uniacid=og.uniacid"
                    . " inner join " . tablename('vending_machine_newstore_goods') . " ng on g.id=ng.goodsid   "
                ...
                foreach ($order_goods as $data) {
                    ...
                    if ($data['totalcnf'] == 1) {
                        if (!empty($data['optionid'])) {
                            $option = pdo_fetch('select id,marketprice,stock from ' . tablename('vending_machine_newstore_goods_option') . ' where optionid=:optionid and goodsid=:goodsid and uniacid=:uniacid and storeid=:storeid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $data['goodsid'], ':optionid' => $data['optionid'],':storeid'=>$data['storeid']));
                            if (!empty($option)) {
                                if ($option['stock'] != -1) {
                                    if (empty($option['stock'])) {
                                        if ($_W['ispost']) {
        ...
        $log = pdo_fetch('SELECT * FROM ' . tablename('core_paylog') . ' WHERE `uniacid`=:uniacid AND `module`=:module AND `tid`=:tid limit 1', array(':uniacid' => $uniacid, ':module' => 'vending_machine', ':tid' => $order['ordersn']));
        if (!empty($ispeerpay)){
        ...
        //秒杀商品
        $seckill_goods = pdo_fetchall('select goodsid,optionid,seckill from  ' . tablename('vending_machine_order_goods') . ' where orderid=:orderid and uniacid=:uniacid and seckill=1 ', array(':uniacid' => $_W['uniacid'], ':orderid' => $orderid));
        ...
        if (is_weixin()) {
            ...
            if (!empty($order['ordersn2'])) {
                ...
                if($var>=100){
                    $var = $var+1;
                    pdo_update('vending_machine_order', array('ordersn2' =>$var), array('id' => $orderid, 'uniacid' => $uniacid, 'openid' => $openid));
                }
    ...
    function orderstatus()
    {
        ...
        $uniacid = $_W['uniacid'];
        $orderid = intval($_GPC['id']);
        $order = pdo_fetch("select status from " . tablename('vending_machine_order') . ' where id=:id and uniacid=:uniacid limit 1'
            , array(':id' => $orderid, ':uniacid' => $uniacid));
        if ($order['status'] >= 1) {
            @session_start();
            $_SESSION[VENDING_MACHINE_PREFIX . "_order_pay_complete"] = 1;
            show_json(1);
        }
        show_json(0);
    }
    ...
    function complete()
    {
        ...
        if (!empty($ispeerpay)) {//代付订单
            ...
            $openid = pdo_fetchcolumn("select openid from " . tablename('vending_machine_order') . ' where id=:orderid and uniacid=:uniacid limit 1', array(':orderid' => $orderid, ':uniacid' => $uniacid));
            $peerpay_info = (float)pdo_fetchcolumn("select SUM(price) price from " . tablename('vending_machine_order_peerpay_payinfo') . ' where pid=:pid limit 1'
        ...
        if (is_h5app() && empty($orderid)) {
            ...
            $orderid = pdo_fetchcolumn("select id from " . tablename('vending_machine_order') . ' where ordersn=:ordersn and uniacid=:uniacid and openid=:openid limit 1', array(':ordersn' => $ordersn, ':uniacid' => $uniacid, ':openid' => $openid));
        }
        ...
        $order = pdo_fetch("select * from " . tablename('vending_machine_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1'
...
//        }
        ...
        if ($order['ispackage'] > 0) {
            $package = pdo_fetch("SELECT * FROM " . tablename('vending_machine_package') . " WHERE uniacid = " . $uniacid . " and id = " . $order['packageid'] . " ");
            if (empty($package)) {
                show_json(0, '未找到套餐！');
            }
        ...
        if ($type == 'cash' && empty($ispeerpay)) {
            ...
            pdo_update('vending_machine_order',array('cashtime'=>time()),array('id'=>$order['id']));
            //模板消息
        ...
        if ($type == 'credit') {
            ...
            if (!is_error($payquery) || !is_error($payquery_jie) || !empty($ispeerpay)) {
                ...
                if (is_h5app()) {
                    pdo_update('vending_machine_order', array('apppay' => 1), array('id' => $order['id']));
                }
                ...
                if (!empty($ispeerpay)){
                    ...
                    pdo_update('vending_machine_order_peerpay_payinfo', $udata, array('pid'=>$ispeerpay['id'],'openid'=>$_W['openid']));
                    m('order')->setOrderPayType($order['id'], 21, $gpc_ordersn);
                }
                $pay_result = m('order')->payResult($ret);
                @session_start();
    ...
    function success()
    {
        ...
        /*@session_start();
        if (empty($_SESSION[VENDING_MACHINE_PREFIX . "_order_pay_complete"])) {
            if (empty($order['istrade'])) {
                header('location: ' . mobileUrl('order'));
            } else {
        ...
        unset($_SESSION[VENDING_MACHINE_PREFIX . "_order_pay_complete"]);*/
        if(empty($_COOKIE['order_pay_complete'])){
            header('location: ' . mobileUrl('order'));
        }
        setcookie('order_pay_complete',0,time()-60);
        ...
        if (!empty($ispeerpay)) {//代付
            ...
            $peerprice = pdo_fetch("SELECT `price` FROM ".tablename('vending_machine_order_peerpay_payinfo')." WHERE uid = :uid ORDER BY id DESC LIMIT 1",array(':uid'=>$peerpayuid['id']));
            ...
            //商品
            $goods = pdo_fetchall("select og.goodsid,og.price,g.title,g.thumb,og.total,g.credit,og.optionid,og.optionname as optiontitle,g.isverify,g.storeids from " . tablename('vending_machine_order_goods') . " og "
            ...
            if (!empty($order['addressid'])) {
                ...
                if (!is_array($address)) {
                    $address = pdo_fetch('select * from  ' . tablename('vending_machine_member_address') . ' where id=:id limit 1', array(':id' => $order['addressid']));
                }
            }
            ...
            if (!empty($order['storeid'])) {
                if ($merchid > 0) {
                    $store = pdo_fetch('select * from  ' . tablename('vending_machine_merch_store') . ' where id=:id limit 1', array(':id' => $order['storeid']));
                } else {
                    $store = pdo_fetch('select * from  ' . tablename('vending_machine_store') . ' where id=:id limit 1', array(':id' => $order['storeid']));
                }
            }
            ...
            if ($order['isverify']) {
                ...
                if (p('newstore')&& !empty($order['isnewstore'])){
                    $stores = pdo_fetchall('select * from ' . tablename('vending_machine_store') . ' where id = :id  and uniacid=:uniacid and status=1  order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid'],':id'=>$order['storeid']));
                    ...
                    if ($merchid > 0) {
                        $stores = pdo_fetchall('select * from ' . tablename('vending_machine_merch_store') . ' where id in (' . implode(',', $storeids) . ') and uniacid=:uniacid and merchid=:merchid and status=1 order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
                    } else {
                        $stores = pdo_fetchall('select * from ' . tablename('vending_machine_store') . ' where id in (' . implode(',', $storeids) . ') and uniacid=:uniacid and status=1 order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid']));
                    }
                }
            }*/
        ...
        /*if(!empty($order['virtual']) && !empty($order['virtual_str'])){
            ...
            $virtualtemp = pdo_fetch('SELECT linktext, linkurl FROM '. tablename('vending_machine_virtual_type'). ' WHERE id=:id AND uniacid=:uniacid LIMIT 1', array(':id'=>$order['virtual'], ':uniacid'=>$_W['uniacid']));
        }*/
        /*
    ...
    public function peerpay()
    {
        ...
        $peerpay = (int)pdo_fetchcolumn("select orderid from " . tablename('vending_machine_order_peerpay') . ' where orderid=:id and uniacid=:uniacid limit 1'
            , array(':id' => $orderid, ':uniacid' => $uniacid));
        ...
        $order = pdo_fetch("select * from " . tablename('vending_machine_order') . ' where id=:id and uniacid=:uniacid limit 1'
            , array(':id' => $orderid, ':uniacid' => $uniacid));

        if($order['ordersn2']>=100){
            $order['ordersn2'] = 0;
            pdo_update('vending_machine_order', array('ordersn2' =>0), array('id' => $orderid, 'uniacid' => $uniacid));
        }
        if (!empty($order['ordersn2'])){
        ...
        if ($_W['ispost']){
            ...
            $res = pdo_insert('vending_machine_order_peerpay',$data);
...
//        $ordergoods = pdo_fetch("select * from " . tablename('vending_machine_order_goods') . ' where orderid=:id and uniacid=:uniacid limit 1'
//            , array(':id' => $orderid, ':uniacid' => $uniacid));
...
//            , array(':id' => $ordergoods['goodsid'], ':uniacid' => $uniacid));
        $goods =  pdo_fetchall("SELECT g.id,og.goodsid,og.total,g.title,g.thumb,g.type,g.status,og.price,og.title as gtitle,og.optionname as optiontitle,og.optionid,op.specs,g.merchid,og.seckill,og.seckill_taskid,
                ...
                FROM " . tablename('vending_machine_order_goods') . " og "
            . " left join " . tablename('vending_machine_goods') . " g on og.goodsid = g.id "
    ...
    public function peerpayshare()
    {
        ...
        $uniacid = $_W['uniacid'];
        $peerpay = pdo_fetch("select p.*,o.openid from " . tablename('vending_machine_order_peerpay') . ' p join '.tablename('vending_machine_order').' o on o.id=p.orderid where p.orderid=:id and p.uniacid=:uniacid limit 1'
        ...
        $member = m('member')->getMember($peerpay['openid']);
    ...
    public function peerpaydetail()
    {
        ...
        $peerid = intval($_GPC['id']);
        $uniacid = $_W['uniacid'];
        $peerpay = pdo_fetch("select p.*,o.openid,o.address,o.isparent,o.id as oid from " . tablename('vending_machine_order_peerpay') . ' p join '.tablename('vending_machine_order').' o on o.id=p.orderid where p.orderid=:id and p.uniacid=:uniacid limit 1'
            , array(':id' => $peerid, ':uniacid' => $uniacid));
        if (empty($peerpay)){
            header('location: ' . mobileUrl('order'));
        }
        ...
        $member = m('member')->getMember($peerpay['openid']);
        //已代付金额
        $peerpay_info = (float)pdo_fetchcolumn("select SUM(price) price from " . tablename('vending_machine_order_peerpay_payinfo') . ' where pid=:pid limit 1', array(':pid' => $peerpay['id']));
        //剩余未付款金额
        $rate_price = round($peerpay['peerpay_realprice'] - $peerpay_info,2);
        //完成百分比
        $rate = round($peerpay_info / $peerpay['peerpay_realprice'] * 100,2);
       ...
       // $ordergoods = pdo_fetchall("select * from " . tablename('vending_machine_order_goods') . ' where '.$scondition.' and uniacid=:uniacid ', array(':id' => $peerpay['orderid'], ':uniacid' => $uniacid));
...
//            $goods_info = pdo_fetch("select * from " . tablename('vending_machine_goods') . ' where id=:id and uniacid=:uniacid limit 1'
...
//     }
        ...
        if($peerpay['peerpay_type'] == 0){//单人代付
            ...
            //代付留言列表
            $message = pdo_fetchall("SELECT * FROM ".tablename('vending_machine_order_peerpay_payinfo')." WHERE pid = :pid ORDER BY id DESC LIMIT 3",array(':pid'=>$peerpay['id']));
