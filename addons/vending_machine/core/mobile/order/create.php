...
class Create_Page extends MobileLoginPage
{
    static $bargain_id;
    ...
    function main()
    {
        ...
        if (!$packageid) {
            ...
            if (p('bargain') && !empty($bargain_id)) {//??
                ...
                $bargain_act = pdo_fetch("SELECT *  FROM " . tablename('vending_machine_bargain_actor') . " WHERE id = :id AND openid = :openid AND status = '0'", array(':id' => $bargain_id, ':openid' => $_W['openid']));
                if (empty($bargain_act)) {
                    die('没有这个商品!');
                }
                $bargain_act_id = pdo_fetch("SELECT *  FROM " . tablename('vending_machine_bargain_goods') . " WHERE id = '{$bargain_act['goods_id']}'");
                if (empty($bargain_act_id)) {
                    die('没有这个商品!');
                }
                $if_bargain = pdo_fetch("SELECT bargain FROM " . tablename('vending_machine_goods') . " WHERE id = :id AND uniacid = :uniacid ", array(':id' => $bargain_act_id['goods_id'], ':uniacid' => $_W['uniacid']));
            ...
            if (empty($id)) {
                ...
                if (!empty($quickid)) {
                    $sql = 'SELECT c.goodsid,c.total,g.maxbuy,g.type,g.intervalfloor,g.intervalprice,g.issendfree,g.isnodiscount,g.ispresell,g.presellprice as gpprice,o.presellprice,g.preselltimeend,g.presellsendstatrttime,g.presellsendtime,g.presellsendtype'
                        ...
                        . ' FROM ' . tablename('vending_machine_quick_cart') . ' c '
                        . ' left join ' . tablename('vending_machine_goods') . ' g on c.goodsid = g.id '
                        . ' left join ' . tablename('vending_machine_goods_option') . ' o on c.optionid = o.id '
                    ...
                    $sql = 'SELECT c.goodsid,c.total,g.maxbuy,g.type,g.intervalfloor,g.intervalprice,g.issendfree,g.isnodiscount,g.ispresell,g.presellprice as gpprice,o.presellprice,g.preselltimeend,g.presellsendstatrttime,g.presellsendtime,g.presellsendtype'
                        ...
                        . ' FROM ' . tablename('vending_machine_member_cart') . ' c '
                        . ' left join ' . tablename('vending_machine_goods') . ' g on c.goodsid = g.id '
                        . ' left join ' . tablename('vending_machine_goods_option') . ' o on c.optionid = o.id '
                    ...
                    $sql = 'SELECT c.goodsid,c.total,g.maxbuy,g.type,g.intervalfloor,g.intervalprice,g.issendfree,g.isnodiscount,g.ispresell,g.presellprice as gpprice,o.presellprice,g.preselltimeend,g.presellsendstatrttime,g.presellsendtime,g.presellsendtype'
                        ...
                        . ' g.virtualsend,invoice,o.specs,g.merchid,g.checked,g.merchsale,g.unite_total,'
                        . ' g.buyagain,g.buyagain_islong,g.buyagain_condition, g.buyagain_sale, g.hasoption'
                        . ' FROM ' . tablename('vending_machine_exchange_cart') . ' c '
                        . ' left join ' . tablename('vending_machine_goods') . ' g on c.goodsid = g.id '
                        . ' left join ' . tablename('vending_machine_goods_option') . ' o on c.optionid = o.id '
                        . " where c.openid=:openid and c.selected=1 and  c.deleted=0 and c.uniacid=:uniacid  order by c.id desc";
                    $goods = pdo_fetchall($sql, array(':uniacid' => $uniacid, ':openid' => $openid));
                ...
                if (empty($goods)) {
                    ...
                    foreach ($goods as $k => $v) {
                        ...
                        $fullbackgoods = array();
                        if ($v['isfullback']) {
                            $fullbackgoods = pdo_fetch("SELECT * FROM " . tablename('vending_machine_fullback_goods') . " WHERE goodsid = :goodsid and uniacid = :uniacid and status = 1 limit 1 ", array(':goodsid' => $v['goodsid'], ':uniacid' => $uniacid));
                        }
                ...
                $sql = 'SELECT id as goodsid,type,title,weight,issendfree,isnodiscount,ispresell,presellprice,'
                    ...
                    . ' FROM ' . tablename('vending_machine_goods') . ' where id=:id and uniacid=:uniacid  limit 1';
                ...
                foreach ($optionsdata as $option) {
                    ...
                    if ($option['optionid'] > 0) {
                        $option = pdo_fetch('select id,title,marketprice,presellprice,goodssn,productsn,`virtual`,stock,weight,specs from ' . tablename('vending_machine_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $id, ':id' => $option['optionid']));
                        if (!empty($option)) {
                ...
                if ($data['merchid'] > 0) {
                    ...
                    $merchstatus = pdo_fetch(" SELECT status FROM " . tablename('vending_machine_merch_user') . " WHERE id=:id AND uniacid=:uniacid ", array(':id' => $data['merchid'], ':uniacid' => $_W['uniacid']));
...
//                    自定义表单
                ...
                if (!empty($liveid)) {
                    ...
                    if (!empty($isLiveGoods)) {
                        ...
                        if ($defaultcardid) {
                            $live_product = pdo_fetch("SELECT *  FROM " . tablename('vending_machine_goods') . " WHERE id = '{$data['goodsid']}'");
                            if ($live_product) {
                ...
                if ($data['seckillinfo'] && $data['seckillinfo']['status'] == 0) {
                    ...
                    if ($giftid) {
                        $gift = pdo_fetch("select id,title,thumb,activity,giftgoodsid,goodsid from " . tablename('vending_machine_gift') . "
                        ...
                        if (!empty($gift['giftgoodsid'])) {
                            ...
                            if ($giftGoodsid) {
                                foreach ($giftGoodsid as $key => $value) {
                                    $giftGood[$key] = pdo_fetch("select id,title,thumb,marketprice from " . tablename('vending_machine_goods') . " where uniacid = " . $uniacid . " and total > 0 and status = 2 and id = " . $value . " and deleted = 0 ");
                                }
                ...
                $fullbackgoods = array();
                if ($data['isfullback']) {
                    $fullbackgoods = pdo_fetch("SELECT * FROM " . tablename('vending_machine_fullback_goods') . " WHERE goodsid = :goodsid and uniacid = :uniacid and status = 1 limit 1 ", array(':goodsid' => $data['goodsid'], ':uniacid' => $uniacid));
                }
                ...
                if (!empty($optionid)) {
                    ...
                    from ' . tablename('vending_machine_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $id, ':id' => $optionid));
                    if (!empty($option)) {
                        $data['optionid'] = $optionid;
            ...
            foreach ($goods as &$g) {
                ...
                if ($g['seckillinfo'] && $g['seckillinfo']['status'] == 0) {
                    ...
                    if (p('task')) {
                        ...
                        if (!empty($task_id)) {
                            $rewarded = pdo_fetchcolumn("SELECT `rewarded` FROM " . tablename('vending_machine_task_extension_join') . " WHERE id = :id AND openid = :openid AND  uniacid = :uniacid", array(':id' => $task_id, ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
                ...
                if (!empty($g['seckillinfo']) && $g['seckillinfo']['status'] == 0) {
                    ...
                    //总购买量
                    if ($g['usermaxbuy'] > 0) {
                        $order_goodscount = pdo_fetchcolumn('select ifnull(sum(og.total),0)  from ' . tablename('vending_machine_order_goods') . ' og '
                            . ' left join ' . tablename('vending_machine_order') . ' o on og.orderid=o.id '
            ...
            $invoice_arr = "{}";
            if ($hasinvoice) {
                $invoicename = pdo_fetchcolumn('select invoicename from ' . tablename('vending_machine_order') . " where openid=:openid and uniacid=:uniacid and ifnull(invoicename,'')<>'' order by id desc limit 1", array(':openid' => $openid, ':uniacid' => $uniacid));
                // 解析发票格式
            ...
            if (!$isverify && !$isvirtual && !$ismerch) { //虚拟 或 卡密 或 不同多商户的商品 不读取自提点
                if ($merch_id > 0) {
                    $carrier_list = pdo_fetchall('select * from ' . tablename('vending_machine_merch_store') . ' where  uniacid=:uniacid and merchid=:merchid and status=1 and type in(1,3) order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid'], ':merchid' => $merch_id));
                } else {
                    $carrier_list = pdo_fetchall('select * from ' . tablename('vending_machine_store') . ' where  uniacid=:uniacid and status=1 and type in(1,3) order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid']));
                }
            }
            ...
            if ($isverify) {
                ...
                if (empty($storeids)) {
                    //门店加入支持核销的判断
                    if ($merchid > 0) {
                        $stores = pdo_fetchall('select * from ' . tablename('vending_machine_merch_store') . ' where  uniacid=:uniacid and merchid=:merchid and status=1 and type in(2,3) order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
                    } else {
                        $stores = pdo_fetchall('select * from ' . tablename('vending_machine_store') . ' where  uniacid=:uniacid and status=1 and type in(2,3) order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid']));
                    }
                } else {
                    if ($merchid > 0) {
                        $stores = pdo_fetchall('select * from ' . tablename('vending_machine_merch_store') . ' where id in (' . implode(',', $storeids) . ') and uniacid=:uniacid and merchid=:merchid and status=1 and type in(2,3) order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
                    } else {
                        $stores = pdo_fetchall('select * from ' . tablename('vending_machine_store') . ' where id in (' . implode(',', $storeids) . ') and uniacid=:uniacid and status=1 and type in(2,3) order by displayorder desc,id desc', array(':uniacid' => $_W['uniacid']));
                    }
                }
            } else {
                //默认地址
                $address = pdo_fetch('select * from ' . tablename('vending_machine_member_address') . ' where openid=:openid and deleted=0 and isdefault=1  and uniacid=:uniacid limit 1'
            ...
            foreach ($goods as $g) {
                ...
                if ($g['seckillinfo'] && $g['seckillinfo']['status'] == 0) {
                    ...
                    if ($giftid) {
                        $gift = array();
                        $giftdata = pdo_fetch("select giftgoodsid from " . tablename('vending_machine_gift') . " where uniacid = " . $uniacid . " and id = " . $giftid . " and status = 1 and starttime <= " . time() . " and endtime >= " . time() . " ");
                        if ($giftdata['giftgoodsid']) {
                            $giftgoodsid = explode(',', $giftdata['giftgoodsid']);

                            foreach ($giftgoodsid as $key => $value) {
                                $giftinfo = pdo_fetch("select id as goodsid,title,thumb from " . tablename('vending_machine_goods') . " where uniacid = " . $uniacid . " and total > 0 and status = 2 and id = " . $value . " and deleted = 0 ");
                                if ($giftinfo) {
                                    $gift[$key] = $giftinfo;
                                    $gift[$key]['total'] = 1;
                                }
                        ...
                        $giftgoods = array();
                        $gift_price = array();
                        $gifts = pdo_fetchall("select id,goodsid,giftgoodsid,thumb,title ,orderprice from " . tablename('vending_machine_gift') . "
                    where uniacid = " . $uniacid . " and status = 1 and starttime <= " . time() . " and endtime >= " . time() . " and orderprice <= " . $goodsprice . " and activity = 1 ");
                        foreach ($gifts as $key => $value) {
                            ...
                            foreach ($giftgoods as $k => $val) {
                                $giftgoodsdetail = pdo_fetch("select id,title,thumb,marketprice,total from " . tablename('vending_machine_goods') . " where uniacid = " . $uniacid . " and deleted = 0 and status = 2 and id = " . $val . " ");
                                if ($giftgoodsdetail) {
...
//                'storeid' => !empty($carrier_list) && !$isverify && !$isvirtual ? $carrier_list[0]['id'] : 0,
            ...
            $package = pdo_fetch("SELECT * FROM " . tablename('vending_machine_package') . " WHERE uniacid = " . $uniacid . " and id = " . $packageid . " ");
            ...
            foreach ($g as $key => $value) {
                ...
                $tablename = tablename('vending_machine_goods');
                $sql = <<<EOF
...
EOF;
                ...
                $packagegoods = array();
                if ($value['optionid'] > 0) {
                    $option = pdo_fetch("select title,packageprice,marketprice from " . tablename('vending_machine_package_goods_option') . "
                    ...
                    $packagegoods = pdo_fetch("select title,packageprice,marketprice from " . tablename('vending_machine_package_goods') . "
    ...
    function caculatecoupon($contype, $couponid, $wxid, $wxcardid, $wxcode, $goodsarr, $totalprice, $discountprice, $isdiscountprice, $isSubmit = 0, $discountprice_array = array(), $merchisdiscountprice = 0, $real_price = 0)
    {
        ...
        if ($contype == 0) {
            ...
            //平台户优惠券
            $sql = "select id,uniacid,card_type,logo_url,title, card_id,least_cost,reduce_cost,discount,merchid,limitgoodtype,limitgoodcatetype,limitgoodcateids,limitgoodids,merchid,limitdiscounttype  from " . tablename('vending_machine_wxcard');
            ...
            $sql = 'SELECT d.id,d.couponid,c.enough,c.backtype,c.deduct,c.discount,c.backmoney,c.backcredit,c.backredpack,c.merchid,c.limitgoodtype,c.limitgoodcatetype,c.limitgoodids,c.limitgoodcateids,c.limitdiscounttype  FROM ' . tablename('vending_machine_coupon_data') . " d";
            $sql .= " left join " . tablename('vending_machine_coupon') . " c on d.couponid = c.id";
    ...
    function caculate()
    {
        ...
        if (!empty($packageid)) {
            //套餐详情
            $package = pdo_fetch("SELECT id,title,price,freight,cash,starttime,endtime,dispatchtype FROM " . tablename('vending_machine_package') . "
        ...
        $address = pdo_fetch('select * from ' . tablename('vending_machine_member_address') . ' where  id=:id and openid=:openid and uniacid=:uniacid limit 1'
        ...
        if (is_array($goodsarr)) {
            ...
            foreach ($goodsarr as &$g) {
                ...
                if (!empty($optionid)) {
                    $option = pdo_fetch('select id,title,marketprice,presellprice,goodssn,productsn,stock,`virtual`,weight,liveprice,islive from ' . tablename('vending_machine_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $goodsid, ':id' => $optionid));
                    if (!empty($option)) {
                        $data['optionid'] = $optionid;
                        $data['optiontitle'] = $option['title'];
                        $data['marketprice'] = (intval($data['ispresell']) > 0 && ($data['preselltimeend'] > time() || $data['preselltimeend'] == 0)) ? $option['presellprice'] : $option['marketprice'];
...
//                //如果订单中有砍价产品那么就不能有满额抵扣了
                if (p('bargain')) {
                    $bargain_act_id = pdo_fetchall("SELECT *  FROM " . tablename('vending_machine_bargain_goods') . " WHERE goods_id = '{$g['goodsid']}' and status = 0");
                    if (!empty($bargain_act_id)) {
                        $allow_sale = false;
                    }
                }
...
//        cache_write($_W['openid'] . '_bargain_id', $bargain_id);
        ...
        if (!empty($packageid)) {
            ...
            //套餐商品
            $packgoods = pdo_fetchall("SELECT id,title,thumb,packageprice,`option`,goodsid FROM " . tablename('vending_machine_package_goods') . "
...
//        dump();
        ...
        $sql = 'SELECT diyformtype,diyfields,bargain,diyformid FROM ' . tablename('vending_machine_goods') . ' where id=:id and uniacid=:uniacid  limit 1';
        ...
        if ($giftid) {
            ...
            if ($giftdata['giftgoodsid']) {
                $giftgoodsid = explode(',', $giftdata['giftgoodsid']);
                foreach ($giftgoodsid as $key => $value) {
                    $giftinfo = pdo_fetch("select id as goodsid,title,thumb from " . tablename('vending_machine_goods') . " where uniacid = " . $uniacid . " and status = 2 and total>0 and id = " . $value . " and deleted = 0 ");
        ...
        foreach ($goods as $g) {
            ...
            if (($data['seckillinfo'] && $data['seckillinfo']['status'] == 0) || ($_GPC['card_id'] > 0)) {
                ...
                if (p('lottery')) {
                    $lottery_id = pdo_get('vending_machine_lottery_log', array('log_id' => $log_id), array('lottery_id'));
                    if ($lottery_id['lottery_id']) {
                        $is_goods = pdo_get('vending_machine_lottery', array('lottery_id' => $lottery_id['lottery_id']), array('is_goods'));
            ...
            if (!empty($optionid)) {
                ...
                $option = pdo_fetch('select id,title,marketprice,liveprice,presellprice,goodssn,productsn,stock,`virtual`,weight' . $sql_condition . ' from ' . tablename('vending_machine_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1', array(':uniacid' => $uniacid, ':goodsid' => $goodsid, ':id' => $optionid));
                ...
                if (!empty($option)) {
                    if (!empty($_SESSION['exchange']) && p('exchange')) {//如果是兑换中心订单
                        if ($option['exchange_stock'] <= 0) {//有兑换库存
                            ...
                            pdo_query("UPDATE " . tablename('vending_machine_goods_option') . " SET exchange_stock = exchange_stock - 1 WHERE id = :id AND uniacid = :uniacid", array(':id' => $optionid, ':uniacid' => $_W['uniacid']));//减库存
                        }
                    } else {//如果不是兑换中心订单
                    ...
                    if ($data['type'] != 4) {
                        ...
                        $packageoption = array();
                        if ($packageid) {
                            $packageoption = pdo_fetch("select packageprice ,marketprice from " . tablename('vending_machine_package_goods_option') . "
                ...
                if ($packageid) {
                    if (empty($g['giftStatus'])) {
                        $pg = pdo_fetch("select packageprice,marketprice from " . tablename('vending_machine_package_goods') . "
                ...
                if (!empty($_SESSION['exchange']) && p('exchange')) {//如果是兑换中心订单
                    if ($data['exchange_stock'] > 0) {//有兑换库存
                        pdo_query("UPDATE " . tablename('vending_machine_goods') . " SET exchange_stock = exchange_stock - 1 WHERE id = :id AND uniacid = :uniacid", array(':id' => $data['goodsid'], ':uniacid' => $_W['uniacid']));//减库存
            ...
            if (intval($_GPC['fromcart']) == 1) {
                ...
                if ($diyform_plugin) {
                    $cartdata = pdo_fetch('select id,diyformdataid,diyformfields,diyformdata from ' . tablename('vending_machine_member_cart') . " "
...
//        }
        ...
        if (($isverify || $dispatchtype) && !$isonlyverifygoods) {
            ...
            if ($isverify) {
                if ($verifytype == 0 || $verifytype == 1) {
                    ...
                    while (1) {
                        $count = pdo_fetchcolumn('select count(*) from ' . tablename('vending_machine_order') . ' where verifycode=:verifycode and uniacid=:uniacid limit 1', array(':verifycode' => $verifycode, ':uniacid' => $_W['uniacid']));
                        if ($count <= 0) {
                            break;
                        }
                        $verifycode = random(8, true);
                    ...
                    for ($i = 1; $i <= $totaltimes; $i++) {
                        ...
                        $verifycode = random(8, true);
                        while (1) {
                            $count = pdo_fetchcolumn('select count(*) from ' . tablename('vending_machine_order') . ' where concat(verifycodes,\'|\' + verifycode +\'|\' ) like :verifycodes and uniacid=:uniacid limit 1', array(':verifycodes' => "%{$verifycode}%", ':uniacid' => $_W['uniacid']));
                            if ($count <= 0) {
                                break;
                            }
                            $verifycode = random(8, true);
        ...
        if (!empty($goods[0]['bargain_id']) && p('bargain')) {//???
            $bargain_act = pdo_fetch("SELECT * FROM " . tablename('vending_machine_bargain_actor') . " WHERE id = :id AND openid = :openid ", array(':id' => $goods[0]['bargain_id'], ':openid' => $_W['openid']));
            if (empty($bargain_act)) {
                die('没有这个商品');
            }
            if ($_SESSION['taskcut']) {
            ...
            if (!pdo_update('vending_machine_bargain_actor', array('status' => 1), array('id' => $goods[0]['bargain_id'], 'openid' => $_W['openid']))) {
                die('下单失败');
            }
        ...
        if ($taskgoodsprice) {
            ...
            pdo_update('vending_machine_task_extension_join', array('rewarded' => serialize($taskGoodsInfo0)), array('id' => $task_id, 'uniacid' => $_W['uniacid'], 'openid' => $_W['openid']));
...
//自定义表单
        ...
        pdo_insert('vending_machine_order', $order);
        ...
        if (!empty($goods[0]['bargain_id']) && p('bargain')) {
            pdo_update('vending_machine_bargain_actor', array('order' => $orderid), array('id' => $goods[0]['bargain_id'], 'openid' => $_W['openid']));
        }
        if ($multiple_order == 0) {
            //开始创建一个订单
            ...
            foreach ($allgoods as $goods) {
                ...
                if (!empty($bargain_act) && p('bargain')) {
                    ...
                    pdo_query("UPDATE " . tablename('vending_machine_goods') . " SET sales = sales + 1 WHERE id = :id AND uniacid = :uniacid", array(':id' => $goods['goodsid'], ':uniacid' => $uniacid));
                }
                ...
                pdo_insert('vending_machine_order_goods', $order_goods);
        ...
        if (!empty($_SESSION['exchange']) && p('exchange')) {
            $codeResult = pdo_fetch("SELECT * FROM " . tablename('vending_machine_exchange_code') . " WHERE `key` = :key AND uniacid =:uniacid", array(':key' => $_SESSION['exchange_key'], ':uniacid' => $_W['uniacid']));
            if ($codeResult['status'] == 2) {
                show_json(0, '兑换失败:此兑换码已兑换');
            }
            $groupResult = pdo_fetch("SELECT * FROM " . tablename('vending_machine_exchange_group') . " WHERE id = :id AND uniacid = :uniacid ", array(':uniacid' => $_W['uniacid'], ':id' => $codeResult['groupid']));
            $record_exsit = pdo_fetch("SELECT * FROM " . tablename('vending_machine_exchange_record') . " WHERE `key`=:key AND uniacid = :uniacid", array(':key' => $_SESSION['exchange_key'], ':uniacid' => $_W['uniacid']));

            if (empty($record_exsit)) {
//                pdo_query("INSERT INTO ".tablename('vending_machine_exchange_record')."(`key`,`uniacid`,`title`,`serial`) VALUES('{$_SESSION['exchange_key']}','{$_W['uniacid']}','{$_SESSION['exchangetitle']}','{$_SESSION['exchangeserial']}')");
                $data = array('key' => $_SESSION['exchange_key'], 'uniacid' => $_W['uniacid'], 'title' => $_SESSION['exchangetitle'], 'serial' => $_SESSION['exchangeserial']);
                pdo_insert('vending_machine_exchange_record', $data);
            }
            ...
            if (empty($_SESSION['exchange_key']) || is_null($_SESSION['exchangeprice']) || is_null($_SESSION['exchangepostage'])) {
                ...
                if ($groupResult['mode'] != 6) {
                    $exchange_res = pdo_update('vending_machine_exchange_code', array('status' => 2), array('key' => $_SESSION['exchange_key'], 'status' => 1, 'uniacid' => $_W['uniacid']));//置状态
                    pdo_query("UPDATE " . tablename('vending_machine_exchange_group') . " SET `use` = `use` + 1 WHERE id = :id AND uniacid = :uniacid", array(':id' => $_SESSION['groupid'], ':uniacid' => $_W['uniacid']));
                } else {
                    pdo_update('vending_machine_exchange_code', array('goodsstatus' => 2), array('key' => $_SESSION['exchange_key'], 'uniacid' => $_W['uniacid']));//置状态
                    if (($codeResult['balancestatus'] == 2 || empty($groupResult['balance_type'])) && ($codeResult['scorestatus'] == 2 || empty($groupResult['score_type'])) && ($codeResult['redstatus'] == 2 || empty($groupResult['red_type'])) && ($codeResult['couponstatus'] == 2 || empty($groupResult['coupon_type']))) {
                        pdo_update("vending_machine_exchange_code", array('status' => 2), array('key' => $_SESSION['exchange_key'], 'status' => 1, 'uniacid' => $_W['uniacid']));
                    }
                }
            }
        }
        ...
        if (!is_null($_SESSION['exchangeprice']) && !empty($_SESSION['exchange']) && p('exchange')) {
            ...
            if ($groupResult['mode'] != 6) {//记录不存在
                ...
                pdo_update('vending_machine_exchange_record', $exchangedata, array('key' => $_SESSION['exchange_key'], 'uniacid' => $_W['uniacid']));
            } else {//记录已存在,说明是组合兑换
            ...
            pdo_update('vending_machine_exchange_cart', array('selected' => 0), array('uniacid' => $_W['uniacid'], 'openid' => $_W['openid'], 'selected' => 1));
        }
        ...
        /*if (is_array($carrier)) {
            ...
            pdo_update('vending_machine_member', $up, array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
            if (!empty($member['uid'])) {
                load()->model('mc');
        ...
        */
        ...
        if ($_GPC['fromcart'] == 1) {
            //删除购物车
            pdo_query('update ' . tablename('vending_machine_member_cart') . ' set deleted=1 where  openid=:openid and uniacid=:uniacid and selected=1 ', array(':uniacid' => $uniacid, ':openid' => $openid));
        }

        if (p('quick') && !empty($_GPC['fromquick'])) {
            // 删除快速购买购物车
            pdo_update("vending_machine_quick_cart", array("deleted" => 1), array("quickid" => intval($_GPC['fromquick']), "uniacid" => $_W['uniacid'], "openid" => $_W['openid']));
        }
        ...
        if (empty($virtualid)) {
            ...
            if (isset($allgoods[0])) {
                $vgoods = $allgoods[0];
//                虚拟卡密不在增加虚拟销量
//                pdo_update('vending_machine_goods', array('sales' => $vgoods['sales'] + $vgoods['total']), array('id' => $vgoods['goodsid']));
            }
        }
    ...
    protected function singleDiyformData($id = 0)
    {
        ...
        if ($diyform_plugin && !empty($id)) {
            ...
            $sql = 'SELECT id as goodsid,type,diyformtype,diyformid,diymode,diyfields FROM ' . tablename('vending_machine_goods') . ' where id=:id and uniacid=:uniacid  limit 1';
    ...
    function diyform()
    {
        ...
        if (empty($goods_temp)) {
            pdo_insert('vending_machine_diyform_temp', $insert);
            $gdid = pdo_insertid();
        } else {
            pdo_update('vending_machine_diyform_temp', $insert, array('id' => $goods_temp['id']));
            $gdid = $goods_temp['id'];
        }
        ...
        if (!empty($cartid)) {
            ...
            pdo_update('vending_machine_member_cart', $cart_data, array('id' => $cartid));
        }
    ...
    function getcardprice()
    {
        ...
        $liveid = intval($_GPC['liveid']);
        if ($liveid > 0) {
            $live_product = pdo_fetch("SELECT *  FROM " . tablename('vending_machine_goods') . " WHERE id = '{$goodsarr[0]['goodsid']}'");
            if ($live_product) {
    ...
    function caculatecard($card_id, $dispatch_price, $lotterydiscountprice, $taskcut, $goodsarr, $totalprice, $discountprice, $isdiscountprice, $liveid, $isSubmit = 0, $goods_dispatch = array())
    {
        ...
        if (is_array($goodsarr)) {
            ...
            foreach ($goodsarr as $k => &$g) {
                ...
                if ($g['status'] != 2) {
                    if ($liveid > 0) {
                        $live_product = pdo_fetch("SELECT *  FROM " . tablename('vending_machine_goods') . " WHERE id = '{$g['goodsid']}'");
                        if ($live_product) {
...
//                    $deductmoney = $totalprice-$isdiscountprice-$discountprice-$deductenough_money;
    ...
    function isfullbackgoods($goodsid = 0, $optionid = 0)
    {
        ...
        $fullbackid = 0;
        if ($optionid > 0) {
            $fullback_goods = pdo_fetch('select id from' . tablename('vending_machine_fullback_goods') . "where goodsid=:goodsid and hasoption =1 and status=1 and uniacid=:uniacid and find_in_set('{$optionid}',optionid)", array(':uniacid' => $_W['uniacid'], ':goodsid' => $goodsid));
        } else {
            $fullback_goods = pdo_get('vending_machine_fullback_goods', array('status' => 1, 'goodsid' => $goodsid, 'uniacid' => $_W['uniacid']), 'id');
        }
        if ($fullback_goods) {
