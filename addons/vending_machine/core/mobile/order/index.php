...
class Index_Page extends MobileLoginPage {
        ...
        if ($show_status != '') {
            ...
            switch ($show_status)
            {
                case 0:
                    ...
                    break;
                case 1:
                    $condition.=' and (status=1 or (status=0 and paytype=3) or status=2)';
                    break;
                case 2:
...
paytype,expresssn,refundstate,dispatchtype,verifyinfo,merchid,isparent,iscycelbuy,cycelbuy_periodic,userdeleted{$s_string}
 from " . tablename('vending_machine_order') . " where 1 {$condition} order by createtime desc LIMIT " . ($pindex - 1) * $psize . ',' . $psize, $params);
        $total = pdo_fetchcolumn('select count(*) from ' . tablename('vending_machine_order') . " where 1 {$condition}", $params);
        ...
        foreach ($list as &$row) {
            ...
            $sql = "SELECT og.id,og.goodsid,og.total,g.title,g.thumb,g.type,g.status,og.price,og.title as gtitle,og.optionname as optiontitle,og.optionid,op.specs,g.merchid,og.seckill,og.seckill_taskid,
                ...
                . " where $scondition and og.totalprice>0 order by og.id asc";
            ...
            if ($row['refundstate'] > 0 && !empty($row['refundid'])) {
                ...
                $refund = pdo_fetch("select * from " . tablename('vending_machine_order_refund') . ' where id=:id and uniacid=:uniacid and orderid=:orderid limit 1'
            ...
            if($row['isonlyverifygoods']){
                ...
                $row['canverify'] = false;
                $verifygood = pdo_fetch("select * from ".tablename('vending_machine_verifygoods').' where orderid=:orderid limit 1',array(':orderid'=>$row['id']));
                if(!empty($verifygood)){
                    $row['verifygoods_id'] = $verifygood['id'];
                    //剩余次数
                    $verifynum = pdo_fetchcolumn('select sum(verifynum) from ' . tablename('vending_machine_verifygoods_log') . '    where verifygoodsid =:id  ', array(':id' => $verifygood['id']));
                    //是否过期
...
//        }
        ...
        //如果维过权则取维权信息
        if($order['refundid'] !=0){
            $refund = pdo_fetch("SELECT *  FROM " . tablename('vending_machine_order_refund') . " WHERE orderid = :orderid and uniacid=:uniacid order by id desc", array(':orderid' => $order['id'], ':uniacid' => $_W['uniacid']));
        }
//        print_r($refund);die();
        ...
        $goods = pdo_fetchall("select og.id,og.single_refundstate,og.sendtime,og.goodsid,og.price,g.title,og.title as gtitle,g.thumb,g.status, g.cannotrefund, og.total,g.credit,og.optionid,
            ...
            og.seckill_taskid{$diyformfields}{$condition1},og.prohibitrefund  from " . tablename('vending_machine_order_goods') . " og "
            . " left join " . tablename('vending_machine_goods') . " g on g.id=og.goodsid "
            . " where $scondition and og.uniacid=:uniacid and og.totalprice>0 ", $param);
        ...
        foreach($goods as &$g){
            if($g['isfullback']){
                $isfullback=true;
                $fullbackgoods = pdo_fetch("SELECT * FROM ".tablename('vending_machine_fullback_goods')." WHERE goodsid = :goodsid and uniacid = :uniacid  limit 1 ",array(':goodsid'=>$g['goodsid'],':uniacid'=>$uniacid));
                if($g['optionid']){
                    $option = pdo_fetch("select `day`,allfullbackprice,fullbackprice,allfullbackratio,fullbackratio,isfullback
                      from ".tablename('vending_machine_goods_option')." where id = :id and uniacid = :uniacid ",array(":id"=>$g['optionid'],":uniacid"=>$uniacid));
...
//                }
                ...
                if (empty($storeids)) {
                    //全部门店
                    if ($merchid > 0) {
                        $stores = pdo_fetchall('select * from ' . tablename('vending_machine_merch_store') . ' where  uniacid=:uniacid and merchid=:merchid and status=1 and type in(2,3)', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
                    } else {
                        $stores = pdo_fetchall('select * from ' . tablename('vending_machine_store') . ' where  uniacid=:uniacid and status=1 and type in(2,3)', array(':uniacid' => $_W['uniacid']));
                    }
                } else {
                    if ($merchid > 0) {
                        $stores = pdo_fetchall('select * from ' . tablename('vending_machine_merch_store') . ' where id in (' . implode(',', $storeids) . ') and uniacid=:uniacid and merchid=:merchid and status=1 and type in(2,3)', array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
                    } else {
                        $stores = pdo_fetchall('select * from ' . tablename('vending_machine_store') . ' where id in (' . implode(',', $storeids) . ') and uniacid=:uniacid and status=1 and type in(2,3)', array(':uniacid' => $_W['uniacid']));
                    }
                }
...
//                print_r($stores);exit;
        ...
        if($order['sendtype']>0 && $order['status']>=1){
            $order_goods = pdo_fetchall("select orderid,goodsid,sendtype,expresscom,expresssn,express,sendtime from ".tablename('vending_machine_order_goods')."
        ...
        if(!empty($order['virtual']) && !empty($order['virtual_str'])){
            ...
            $virtualtemp = pdo_fetch('SELECT linktext, linkurl,description FROM '. tablename('vending_machine_virtual_type'). ' WHERE id=:id AND uniacid=:uniacid LIMIT 1', array(':id'=>$order['virtual'], ':uniacid'=>$_W['uniacid']));
        }
        ...
        if($order['sendtype']>0 && $sendtype == 0){
            for($i=1;$i<=intval($order['sendtype']);$i++){
                ...
                $bundlelist[$i]['goods'] = pdo_fetchall("select g.title,g.thumb,og.total,og.optionname as optiontitle,og.expresssn,og.express,
                    og.sendtype,og.expresscom,og.sendtime from " . tablename('vending_machine_order_goods') . " og "
                    . " left join " . tablename('vending_machine_goods') . " g on g.id=og.goodsid "
