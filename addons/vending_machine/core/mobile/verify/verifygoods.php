...
class Verifygoods_Page extends MobilePage {
	...
	function  detail() {
		...
		$item = pdo_fetch('select vg.*,g.id as goodsid ,g.title,g.subtitle,g.thumb,vg.storeid from ' . tablename('vending_machine_verifygoods') . '   vg
		...
		{
			$verifygoodlogs = pdo_fetchall('select *  from ' . tablename('vending_machine_verifygoods_log') . '    where verifygoodsid =:id  ', array(':id' => $item['id']));
			$verifynum = 0;
