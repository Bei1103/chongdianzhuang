...
class Verifyorder_Page extends MobilePage {
...
//        订单号，核销员，核销门店，核销码，会员姓名，收件人姓名，快递单号，商品名称，商品编码
        if (!empty($_GPC['searchfield']) && !empty($_GPC['keyword'])) {
            ...
            if ($searchfield == 'ordersn') {
                ...
                $sqlcondition = " left join " . tablename('vending_machine_store') . " store on store.id = o.verifystoreid and store.uniacid=o.uniacid";
            } else if ($searchfield == 'goodstitle') {
                $sqlcondition =  " inner join ( select DISTINCT(og.orderid) from " . tablename('vending_machine_order_goods') . " og left join " . tablename('vending_machine_goods') . " g on g.id=og.goodsid where og.uniacid = '$uniacid' and (locate(:keyword,g.title)>0)) gs on gs.orderid=o.id";
            } else if ($searchfield == 'goodssn') {
                $sqlcondition =  " inner join ( select DISTINCT(og.orderid) from " . tablename('vending_machine_order_goods') . " og left join " . tablename('vending_machine_goods') . " g on g.id=og.goodsid where og.uniacid = '$uniacid' and (((locate(:keyword,g.goodssn)>0)) or (locate(:keyword,og.goodssn)>0))) gs on gs.orderid=o.id";
            }
        }
        ...
        if ($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0 AND ( o.status = 3 and (isverify=1 or o.istrade=1)) and o.verifyopenid = :openid' || !empty($sqlcondition)){
            if ( $this->merchPluginExists() ) {
                $sql = "select o.* , a.realname as arealname,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea, a.street as astreet,a.address as aaddress, d.dispatchname,m.nickname,m.id as mid,m.realname as mrealname,m.mobile as mmobile,sm.id as salerid,sm.nickname as salernickname,s.salername,
                  r.rtype,r.status as rstatus,o.sendtype,0 as isverifygoods_order from " . tablename('vending_machine_order') . " o"
                    ...
                    . " left join " . tablename('vending_machine_saler') . " s on s.openid = o.verifyopenid and s.uniacid=o.uniacid"
                    . " left join " . tablename('vending_machine_merch_saler') . " ms on ms.openid = o.verifyopenid and ms.uniacid=o.uniacid"
            ...
            if ( $this->merchPluginExists() ) {
                $sql= "SELECT o.*,0 as isverifygoods_order FROM". tablename("vending_machine_order")." o "
                    ." LEFT JOIN ".tablename("vending_machine_saler")." s ON o.verifyopenid=s.openid"
                    ." LEFT JOIN ".tablename("vending_machine_merch_saler")." ms ON o.verifyopenid=ms.openid"
            ...
            if (!empty($list)){
                ...
                $openid_array = pdo_fetchall("SELECT openid,nickname,id as mid,realname as mrealname,mobile as mmobile FROM " .tablename('vending_machine_member')." WHERE openid IN ({$openid}) AND uniacid={$_W['uniacid']}",array(),'openid');
                ...
                $verifyopenid_array = pdo_fetchall("SELECT sm.id as salerid,sm.nickname as salernickname,sm.openid,s.salername FROM " .tablename('vending_machine_saler')." s LEFT JOIN ".tablename('vending_machine_member')." sm ON sm.openid = s.openid and sm.uniacid=s.uniacid WHERE s.openid IN ({$verifyopenid})",array(),'openid');
                foreach ($list as $key=>&$value){
...
//                }
                ...
                $order_goods = pdo_fetchall('select g.id,g.title,g.thumb,g.goodssn,og.goodssn as option_goodssn, g.productsn,og.productsn as option_productsn, og.total,
                    ...
                    og.diyformfields,op.specs,g.merchid,og.seckill,og.seckill_taskid,og.seckill_roomid,g.ispresell from ' . tablename('vending_machine_order_goods') . ' og '
        ...
        if ($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0' || !empty($sqlcondition)){
            $t = pdo_fetch(
                ...
                . " left join " . tablename('vending_machine_order_refund') . " r on r.id =o.refundid "
                . " left join " . tablename('vending_machine_member') . " m on m.openid=o.openid  and m.uniacid =  o.uniacid"
                . " left join " . tablename('vending_machine_member_address') . " a on o.addressid = a.id "
                . " left join " . tablename('vending_machine_saler') . " s on s.openid = o.verifyopenid and s.uniacid=o.uniacid"
                . " left join " . tablename('vending_machine_member') . " sm on sm.openid = s.openid and sm.uniacid=s.uniacid"
    ...
    public  function orderDataVerify(){
        ...
        //当前核销员
        $saler = pdo_fetch('select id,openid from ' . tablename('vending_machine_saler') . ' where status=1  and openid=:openid and uniacid=:uniacid limit 1', array(
...
//        订单号，核销员，核销门店，核销码，会员姓名，联系人，快递单号，商品名称，商品编码
        ...
        if ($condition != ' log.salerid = :salerid and log.uniacid = :uniacid ' || !empty($sqlcondition)){
            ...
            $sql = "select o.* ,1 as isverifygoods_order,g.id as verifygoodsid from " . tablename('vending_machine_verifygoods_log') . " log "
                ." left join  " . tablename('vending_machine_verifygoods') . " g on log.verifygoodsid = g.id "
                ." left join  " . tablename('vending_machine_order') . " o on g.orderid = o.id  "
                . " left join " . tablename('vending_machine_member') . " m on m.openid=o.openid and m.uniacid =  o.uniacid "
            ...
            $sql = "select o.* ,1 as isverifygoods_order,g.id as verifygoodsid from " . tablename('vending_machine_verifygoods_log') . " log "
                ." left join  " . tablename('vending_machine_verifygoods') . " g on log.verifygoodsid = g.id "
                ." left join  " . tablename('vending_machine_order') . " o on g.orderid = o.id where log.uniacid = :uniacid and log.salerid=:salerid GROUP BY o.id ORDER BY log.verifydate DESC  ";
        ...
        if (!empty($list)) {
            ...
            foreach ($list as &$value) {
                ...
                //核销次数统计
                $verifygoodlogs = pdo_fetchall('select *  from ' . tablename('vending_machine_verifygoods_log') . '    where verifygoodsid =:id  ', array(':id' => $value['verifygoodsid']));
                $verifynum = 0;
                foreach($verifygoodlogs as $verifygoodlog)
                {
                    $verifynum +=intval($verifygoodlog['verifynum']);
                }
                $verifygood =   pdo_fetch('select *  from ' . tablename('vending_machine_verifygoods') . '    where id =:id  ', array(':id' => $value['verifygoodsid']));
        ...
        if ($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0' || !empty($sqlcondition)){
            ...
            $t = pdo_fetch(
                'SELECT COUNT(*) as count, ifnull(sum(price),0) as sumprice   FROM ' . tablename('vending_machine_order') . " WHERE uniacid = :uniacid and ismr=0 and deleted=0 and isparent=0", $paras);
        }
    ...
    protected function selectOrderPrice($day=0)
    {
        ...
        if( $this->merchPluginExists() ) {
            ...
            $sql= "SELECT count(o.id) FROM". tablename("vending_machine_order")." o"
                ." LEFT JOIN ".tablename("vending_machine_saler")." s ON o.verifyopenid=s.openid"
                ." LEFT JOIN ".tablename("vending_machine_merch_saler")." ms ON o.verifyopenid=ms.openid"
        ...
        $sql= "SELECT sum(verifynum) FROM". tablename("vending_machine_verifygoods_log")."    WHERE 
    ...
    public function detail() {
        ...
        $order_goods = array();
        if($item['sendtype']>0){
            $order_goods = pdo_fetchall("SELECT orderid,goodsid,sendtype,expresssn,expresscom,express,sendtime FROM " . tablename('vending_machine_order_goods') . "
            WHERE orderid = ".$id." and sendtime > 0 and uniacid=".$_W['uniacid']." and sendtype > 0 group by sendtype order by sendtime desc ");
            foreach($order_goods as $key => $value){
                $order_goods[$key]['goods'] = pdo_fetchall('select g.id,g.title,g.thumb,og.sendtype,g.ispresell from ' . tablename('vending_machine_order_goods') . ' og '
                    . ' left join ' . tablename('vending_machine_goods') . ' g on g.id=og.goodsid '
        ...
        $dispatch = pdo_fetch("SELECT * FROM " . tablename('vending_machine_dispatch') . " WHERE id = :id and uniacid=:uniacid and merchid=0", array(':id' => $item['dispatchid'], ':uniacid' => $_W['uniacid']));
        ...
        $refund = pdo_fetch("SELECT * FROM " . tablename('vending_machine_order_refund') . " WHERE orderid = :orderid and uniacid=:uniacid order by id desc", array(':orderid' => $item['id'], ':uniacid' => $_W['uniacid']));
        ...
        if (com('verify')) {
            ...
            if (!empty($item['verifyopenid'])) {
                ...
                $saler['salername'] = pdo_fetchcolumn('select salername from ' . tablename('vending_machine_saler') . ' where openid=:openid and uniacid=:uniacid limit 1 ', array(':uniacid' => $_W['uniacid'], ':openid' => $item['verifyopenid']));
            }
            if (!empty($item['verifystoreid'])) {
                $store = pdo_fetch('select * from ' . tablename('vending_machine_store') . ' where id=:storeid limit 1 ', array(':storeid' => $item['verifystoreid']));
            }
                    ...
                    if (empty($item['dispatchtype']))
                    {
                        foreach ($verifyinfo as &$v) {
                            if ($v['verified'] || $item['verifytype']==1) {
                                $v['storename'] = pdo_fetchcolumn('select storename from ' . tablename('vending_machine_store') . " where id=:id limit 1", array(':id' => $v['verifystoreid']));
                                if (empty($v['storename'])) {
                                    $v['storename'] = "总店";
                                }
                                $v['nickname'] = pdo_fetchcolumn('select nickname from ' . tablename('vending_machine_member') . " where openid=:openid and uniacid=:uniacid limit 1", array(':openid' => $v['verifyopenid'], ':uniacid' => $_W['uniacid']));
                                $v['salername'] = pdo_fetchcolumn('select salername from ' . tablename('vending_machine_saler') . " where openid=:openid and uniacid=:uniacid limit 1", array(':openid' => $v['verifyopenid'], ':uniacid' => $_W['uniacid']));
                            }
                        }
                        unset($v);
                    }
