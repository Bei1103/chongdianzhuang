...
class Picker_Page extends MobilePage {
    ...
    function main()
    {
        ...
        if (!empty($goods) && $goods['hasoption']) {
            ...
            foreach($specs as &$spec) {
                $spec['items'] = pdo_fetchall('select * from '.tablename('vending_machine_goods_spec_item')." where specid=:specid and `show`=1 order by displayorder asc",array(':specid'=>$spec['id']));
            }
            unset($spec);
        ...
        $leveldiscount = pdo_fetch('SELECT * FROM' . tablename('vending_machine_member_level') . 'where id=:id and uniacid=:uniacid limit 1 ',array(':id' =>$member['level'], ':uniacid' => $_W['uniacid']));
...
//                获取会员等级
        ...
        if(!empty($member_levelid)){
            $member_level = pdo_fetch('select * from ' . tablename('vending_machine_member_level') . ' where id=:id and uniacid=:uniacid and enabled=1 limit 1', array(':id' =>$member_levelid, ':uniacid' => $_W['uniacid']));
...
//                    秒杀
        ...
        if($sale_plugin){
            ...
            $gifts = array();
            $giftgoods = array();
            $gifts = pdo_fetchall("select id,goodsid,giftgoodsid,thumb,title from ".tablename('vending_machine_gift')." where uniacid = ".$_W['uniacid']." and activity = 2 and status = 1 and starttime <= ".time()." and endtime >= ".time()."  ");
                ...
                foreach ($gid as $ke => $val){
                    if($val==$id){
                        ...
                        foreach($giftgoods as $k => $val){
                            $giftdata = pdo_fetch("select id,title,thumb,marketprice,total from ".tablename('vending_machine_goods')." where uniacid = ".$_W['uniacid']." and deleted = 0 and total > 0 and status = 2 and id = ".$val." ");
                            if(!empty($giftdata)){
                                $isgift = 1;
