...
class Package_Page extends MobilePage {
    ...
    function main() {
        ...
        $goodsid_array = array();
        if($goodsid){
            $packages_goods = pdo_fetchall("SELECT id,pid FROM ".tablename('vending_machine_package_goods')."
                    WHERE uniacid = ".$uniacid." and goodsid = ".$goodsid." group by pid  ORDER BY id DESC");
            foreach($packages_goods as $key => $value){
                $packages[$key] = pdo_fetch("SELECT id,title,thumb,price,goodsid FROM ".tablename('vending_machine_package')."
            ...
            $packages = pdo_fetchall("SELECT id,title,thumb,price,goodsid FROM ".tablename('vending_machine_package')."
        ...
        foreach($packages as $key => $value){
            ...
            foreach($goods as $k => $val){
                $g = pdo_fetch("SELECT id,marketprice FROM ".tablename('vending_machine_goods')."
    ...
    function detail() {
        ...
        $package = pdo_fetch("SELECT id,title,price,freight,share_title,share_icon,share_desc FROM ".tablename('vending_machine_package')."
                    WHERE uniacid = ".$uniacid." and id = ".$pid." ");

        $packgoods = array();
        $packgoods = pdo_fetchall("SELECT id,title,thumb,marketprice,packageprice,`option`,goodsid FROM ".tablename('vending_machine_package_goods')."
        ...
        foreach($packgoods as $key => $value){
            ...
            if($option_array[0]>0){
                $pgo = pdo_fetch("SELECT id,title,packageprice FROM ".tablename('vending_machine_package_goods_option')."
        ...
        foreach($packgoods as $key => $value){
            ...
            foreach($option_array[$key] as $k => $val){
                $packgoods[$key]['op'][$k] = pdo_fetch("SELECT id,title FROM ".tablename('vending_machine_goods_option')."
    ...
    function option(){
        ...
        $packgoods = pdo_fetch("SELECT id,title,`option` FROM ".tablename('vending_machine_package_goods')."
        ...
        foreach($optionid as $key => $value){
            $option[$key] = pdo_fetch("SELECT id,title,packageprice,optionid,goodsid FROM ".tablename('vending_machine_package_goods_option')."
        ...
        /*$option = pdo_fetchall("select optionid,title,goodsid,packageprice from ".tablename('vending_machine_package_goods_option')." where pid = ".$pid." and goodsid = ".$goodsid." and uniacid = ".$uniacid." ");*/
