...
class Code_Page extends MobilePage {
    function main() {
        global $_W, $_GPC;
        $openid =$_W['openid'];
        $uniacid = $_W['uniacid'];
        ...
        $codegoods = pdo_fetch("SELECT * FROM ".tablename('vending_machine_goodscode_good')." WHERE
            uniacid = ".$uniacid." and goodsid = ".$goodsid." and id = ".$id." and status = 1 ");
        if(empty($codegoods)){
            $this->message(array("message"=>"该商品不存在或已删除"), mobileUrl(''), 'error');
        }
        $goods = pdo_fetch("SELECT title,content FROM ".tablename('vending_machine_goods')." WHERE
