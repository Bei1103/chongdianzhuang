...
class Detail_Page extends MobilePage {
    ...
    function main() {
        ...
        if (p('task')){
            if (!empty($task_id)){
                $rewarded = pdo_fetchcolumn("SELECT `rewarded` FROM ".tablename('vending_machine_task_extension_join')." WHERE id = :id AND uniacid = :uniacid",array(':id'=>$task_id,':uniacid'=>$_W['uniacid']));
            ...
            $taskGoodsInfo = pdo_fetch('select price,reward_data,id from '.tablename('vending_machine_task_reward')." where id = :id and openid = :openid and senttime = 0 and gettime > 0",array(':id'=>$taskrewardgoodsid,':openid'=>$_W['openid']));
            @session_start();
        ...
        $gifts = array();
        $giftgoods = array();
        $gifts = pdo_fetchall("select id,goodsid,giftgoodsid,thumb,title from ".tablename('vending_machine_gift')." where uniacid = ".$uniacid." and activity = 2 and status = 1 and starttime <= ".time()." and endtime >= ".time()."  ");
            ...
            foreach ($gid as $ke => $val){
                if($val==$id){
                    ...
                    foreach($giftgoods as $k => $val){
                        ...
                        $giftinfo = pdo_fetch("select id,title,thumb,marketprice from ".tablename('vending_machine_goods')." where uniacid = ".$uniacid." and deleted = 0 and total > 0 and status = 2 and id = ".$val." ");
                        if($giftinfo){
                            $isgift = 1;
        ...
        if($goods['ispresell']>0 && ($goods['preselltimeend'] == 0 || $goods['preselltimeend'] > time()) && $goods['hasoption']==1){
            ...
            if(!empty($goods['hasoption'])){
                $presell = pdo_fetch("select min(presellprice) as minprice,max(presellprice) as maxprice from ".tablename('vending_machine_goods_option')." where goodsid = ".$id);
        ...
        if($goods['isfullback']){
            $isfullback = true;
            $fullbackgoods = pdo_fetch("SELECT * FROM ".tablename('vending_machine_fullback_goods')." WHERE uniacid = ".$uniacid." and goodsid = ".$id." and status=1 limit 1 ");
            if(empty($fullbackgoods)){
                ...
                if($goods['hasoption']==1){
                    $fullprice = pdo_fetch("select min(allfullbackprice) as minfullprice,max(allfullbackprice) as maxfullprice,min(allfullbackratio) as minfullratio
                            ...
                            from ".tablename('vending_machine_goods_option')." where goodsid = ".$id."");
        ...
        if(empty($member['updateaddress'])) {
            $address_list = pdo_fetchall('select id,datavalue from ' . tablename('vending_machine_member_address') . ' where openid=:openid and uniacid=:uniacid'
                ...
                foreach ($address_list as $k1 => $v1) {
                    if(!empty($v1['datavalue'])) {
                        ...
                        if (!empty($change_data['province']) && !empty($change_data['city']) && !empty($change_data['area'])) {
                            pdo_update('vending_machine_member_address', $change_data, array('id' => $v1['id']));
                        }
                    }
                }
                pdo_update('vending_machine_member', array('updateaddress' => 1), array('id' => $member['id']));
            }
        }
        ...
        if (empty($task_goods_data['is_task_goods'])) {
            ...
            if (p('bargain') && !is_weixin()){
                //如果是砍价商品,自动跳转
                $bargain = pdo_fetch("SELECT * FROM ".tablename('vending_machine_bargain_goods')." WHERE id = :id AND unix_timestamp(start_time)<".time()." AND unix_timestamp(end_time)>".time()." AND status = 0",array(':id'=>$goods['bargain']));
        ...
        $buyshow = 0;
        if ($goods['buyshow'] == 1) {
            $sql = "select o.id from " . tablename('vending_machine_order') . " o left join " . tablename('vending_machine_order_goods') . " g on o.id = g.orderid";
        ...
        //套餐
        $package_goods = pdo_fetch("select pg.id,pg.pid,pg.goodsid,p.displayorder from ".tablename('vending_machine_package_goods')." as pg
                        left join ".tablename('vending_machine_package')." as p on pg.pid = p.id
        ...
        if($package_goods['pid']){
            $packages = pdo_fetchall("SELECT id,title,thumb,packageprice FROM ".tablename('vending_machine_package_goods')."
        ...
        //规格specs
        $specs = pdo_fetchall('select * from ' . tablename('vending_machine_goods_spec') . " where goodsid=:goodsid and  uniacid=:uniacid order by displayorder asc", array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));
        $spec_titles = array();
        ...
        //参数
        $params = pdo_fetchall("SELECT * FROM " . tablename('vending_machine_goods_param') . " WHERE uniacid=:uniacid and goodsid=:goodsid order by displayorder asc", array(':uniacid' => $uniacid, ":goodsid" => $goods['id']));
        ...
        if (!empty($goods['hasoption'])){
            $options = pdo_fetchall('select id,stock,marketprice,islive,liveprice from ' . tablename('vending_machine_goods_option') . ' where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc', array(':goodsid' => $goods['id'], ':uniacid' => $_W['uniacid']),'stock');
...
//            }
        ...
        if($goods['isdiscount'] && $goods['isdiscount_time']>=time() && $goods['isdiscount_time_start'] < time()){
            ...
            if ( isset($options) && count($options) > 0 && $goods['hasoption']) {
                ...
                //更新最低价和最高价
                $sql = "update ".tablename('vending_machine_goods')." g set
        g.minprice = (select min(marketprice) from ".tablename('vending_machine_goods_option')." where goodsid = $id),
        g.maxprice = (select max(marketprice) from ".tablename('vending_machine_goods_option')." where goodsid = $id)
        where g.id = $id and g.hasoption=1";
                pdo_query($sql);
            } else {
                $sql = "update ".tablename('vending_machine_goods')." set minprice = marketprice,maxprice = marketprice where id = $id and hasoption=0;";
                pdo_query($sql);
            }
            $goods_price = pdo_fetch("select minprice,maxprice from " . tablename('vending_machine_goods') . " where id=:id and uniacid=:uniacid limit 1", array(':id' => $id, ':uniacid' => $_W['uniacid']));
        ...
        //如果是多规格，获取规格原价最大值
        if($goods['hasoption']>0){
            $productprice = pdo_fetchcolumn("select max(productprice) as productprice from ".tablename('vending_machine_goods_option')." where goodsid = :goodsid",array(':goodsid'=>$id));
        ...
        if (empty($this->merch_user))
        {
            $merch_flag = 0;
            if ($is_openmerch == 1 && $goods['merchid'] > 0) {
                $merch_user = pdo_fetch("select * from ".tablename('vending_machine_merch_user')." where id=:id limit 1",array(':id'=>intval($goods['merchid'])));
                if (!empty($merch_user)) {
                    $shop = $merch_user;
                    $merch_flag = 1;
                }
            ...
            if (empty($shop['selectgoods'])) {
                $statics = array(
                    'all'=>pdo_fetchcolumn('select count(1) from '.tablename('vending_machine_goods')." where uniacid=:uniacid {$sqlcon} and status=1 and deleted=0", $param),
                    'new'=>pdo_fetchcolumn('select count(1) from '.tablename('vending_machine_goods')." where uniacid=:uniacid {$sqlcon} and isnew=1 and status=1 and deleted=0", $param),
                    'discount'=>pdo_fetchcolumn('select count(1) from '.tablename('vending_machine_goods')." where uniacid=:uniacid {$sqlcon} and isdiscount=1 and status=1 and deleted=0", $param)
                );
            } else {
                ...
                $statics = array(
                    'all'=>count($goodsids),
                    'new'=>pdo_fetchcolumn("select count(1) from ".tablename('vending_machine_goods')." where uniacid=:uniacid {$sqlcon} and id in(".$shop['goodsids'].") and isnew=1 and status=1 and deleted=0", $param),
                    'discount'=>pdo_fetchcolumn("select count(1) from ".tablename('vending_machine_goods')." where uniacid=:uniacid {$sqlcon} and id in(".$shop['goodsids'].") and isdiscount=1 and status=1 and deleted=0", $param)
                );
            }
        }
        else
        {
            ...
            if (empty($shop['selectgoods'])) {
                $statics = array(
                    'all'=>pdo_fetchcolumn('select count(1) from '.tablename('vending_machine_goods')." where uniacid=:uniacid and merchid=:merchid and status=1 and deleted=0",array(':uniacid'=>$_W['uniacid'],':merchid'=>$goods['merchid'])),
                    'new'=>pdo_fetchcolumn('select count(1) from '.tablename('vending_machine_goods')." where uniacid=:uniacid and merchid=:merchid and isnew=1 and status=1 and deleted=0",array(':uniacid'=>$_W['uniacid'],':merchid'=>$goods['merchid'])),
                    'discount'=>pdo_fetchcolumn('select count(1) from '.tablename('vending_machine_goods')." where uniacid=:uniacid and merchid=:merchid and isdiscount=1 and status=1 and deleted=0",array(':uniacid'=>$_W['uniacid'],':merchid'=>$goods['merchid']))
                );
            } else {
                $goodsids = explode(",", $shop['goodsids']);
                $statics = array(
                    'all'=>count($goodsids),
                    'new'=>pdo_fetchcolumn('select count(1) from '.tablename('vending_machine_goods')." where uniacid=:uniacid and merchid=:merchid and id in( {$shop['goodsids']} ) and isnew=1 and status=1 and deleted=0",array(':uniacid'=>$_W['uniacid'],':merchid'=>$goods['merchid'])),
                    'discount'=>pdo_fetchcolumn('select count(1) from '.tablename('vending_machine_goods')." where uniacid=:uniacid and merchid=:merchid and id in( {$shop['goodsids']} ) and isdiscount=1 and status=1 and deleted=0",array(':uniacid'=>$_W['uniacid'],':merchid'=>$goods['merchid']))
                );
            }
        }
...
//            获取商品佣金
            if($goods['nocommission']==0){
                ...
                if (!empty($goods) && $goods['hasoption']) {
                    $optioncomm = pdo_fetchall('select * from ' . tablename('vending_machine_goods_option') . ' where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc', array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));
                }
...
//        }
        ...
        if($new_temp && $getComments){
            $showComments = pdo_fetchcolumn('select count(*) from '.tablename('vending_machine_order_comment')." where goodsid=:goodsid and level>=0 and deleted=0 and checked=0 and uniacid=:uniacid", array(':goodsid'=>$id,':uniacid'=>$_W['uniacid']));
        }
    ...
    function get_code(){
        ...
        $goods = pdo_fetch("select id,minprice,minprice,maxprice,hasoption,productprice,thumb_url,thumb,title from " . tablename('vending_machine_goods') . " where id=:id and uniacid=:uniacid limit 1", array(':id' => $id, ':uniacid' => $_W['uniacid']));
...
EOF;
    ...
    function querygift(){
        ...
        $gift = pdo_fetch("select * from ".tablename('vending_machine_gift')." where id=:id and status=1 and  uniacid =:uniacid limit 1",array(':id'=>$giftid,':uniacid'=>$_W['uniacid']));
        show_json(1,$gift);
    }
    ...
    function get_comments(){
        ...
        $count = array(
            "all"=>pdo_fetchcolumn('select count(*) from '.tablename('vending_machine_order_comment')." where goodsid=:goodsid and level>=0 and deleted=0 and checked=0 and uniacid=:uniacid",$params),
            "good"=>pdo_fetchcolumn('select count(*) from '.tablename('vending_machine_order_comment')." where goodsid=:goodsid and level>=5 and deleted=0 and checked=0 and uniacid=:uniacid",$params),
            "normal"=>pdo_fetchcolumn('select count(*) from '.tablename('vending_machine_order_comment')." where goodsid=:goodsid and level>=2 and level<=4 and deleted=0 and checked=0 and uniacid=:uniacid",$params),
            "bad"=>pdo_fetchcolumn('select count(*) from '.tablename('vending_machine_order_comment')." where goodsid=:goodsid and level<=1 and deleted=0 and checked=0 and uniacid=:uniacid",$params),
            "pic"=>pdo_fetchcolumn('select count(*) from '.tablename('vending_machine_order_comment')." where goodsid=:goodsid and ifnull(images,'a:0:{}')<>'a:0:{}' and deleted=0 and checked=0 and uniacid=:uniacid",$params)
        ) ;
        $list = array();
        if($count['all']>0){
            $percent = intval( $count['good'] / (empty($count['all'])?1:$count['all']) * 100);
            $list = pdo_fetchall('select nickname,level,content,images,createtime from '.tablename('vending_machine_order_comment')." where goodsid=:goodsid and deleted=0 and checked=0 and uniacid=:uniacid order by istop desc, createtime desc, id desc limit 2",array(':goodsid'=>$id,':uniacid'=>$_W['uniacid']));
            foreach($list as &$row){
    ...
    function getLevel($openid)
    {
        ...
        $level = pdo_fetch('select * from ' . tablename('vending_machine_commission_level') . ' where uniacid=:uniacid and id=:id limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $member['agentlevel']));
    ...
    function get_offic_list(){
        ...
        $total = pdo_fetchcolumn('select count(1) from ' . tablename('vending_machine_offic') . " as o where 1 {$condition}", $params);
        $list = array();
        if($total>0){
            $list = pdo_fetchall("SELECT o.*,m.avatar,m.nickname,g.thumb,g.title,g.minprice FROM ".tablename('vending_machine_offic')." as o
                    left join ".tablename('vending_machine_member')." as m on m.openid = o.openid
                    left join ".tablename('vending_machine_goods')." as g on g.id = o.goodsid
    ...
    function get_comment_list(){
        ...
        $list = pdo_fetchall("select * from ".tablename('vending_machine_order_comment')." "
        ...
        unset($row);
        $total = pdo_fetchcolumn('select count(*) from ' . tablename('vending_machine_order_comment') . " where goodsid=:goodsid  and uniacid=:uniacid and deleted=0 and checked=0 {$condition}", $params);
    ...
    function getCouponsbygood($goodid){
        ...
        if(!empty($goodid))
        {
            ...
            $goodparam[':id'] = $goodid;
            $sql = "select id,cates,marketprice,merchid   from " . tablename('vending_machine_goods') ;
        ...
        if (!empty($list)) {
            foreach ($list as $key =>&$row) {
                ...
                if($row['islimitlevel'] ==1) {
                    ...
                    {
                        //会员等级
                        $level1 = pdo_fetchall('select * from ' . tablename('vending_machine_member_level') . ' where uniacid=:uniacid and  id in ('.$row['limitmemberlevels'].') ', array(':uniacid' => $_W['uniacid']));
                    ...
                    if((!empty($row['limitagentlevels'])||$row['limitagentlevels']=='0')&&$hascommission) {
                        //分销商等级
                        $level2 = pdo_fetchall('select * from ' . tablename('vending_machine_commission_level') . ' where uniacid=:uniacid and id  in ('.$row['limitagentlevels'].') ', array(':uniacid' => $_W['uniacid']));
                    ...
                    if((!empty($row['limitpartnerlevels'])||$row['limitpartnerlevels']=='0')&&$hasglobonus) {
                        //股东等级
                        $level3 = pdo_fetchall('select * from ' . tablename('vending_machine_globonus_level') . ' where uniacid=:uniacid and  id in('.$row['limitpartnerlevels'].') ', array(':uniacid' => $_W['uniacid']));
                    ...
                    if((!empty($row['limitaagentlevels'])||$row['limitaagentlevels']=='0')&&$hasabonus) {
                        //区域代理
                        $level4 = pdo_fetchall('select * from ' . tablename('vending_machine_abonus_level') . ' where uniacid=:uniacid and  id in ('.$row['limitaagentlevels'].') ', array(':uniacid' => $_W['uniacid']));
    ...
    public function pay($a=array(), $b=array()){
        ...
        $openid = $_W['openid'];
        $id = intval($_GPC['id']);
        $coupon = pdo_fetch('select * from ' . tablename('vending_machine_coupon') . ' where id=:id and uniacid=:uniacid  limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
        ...
        pdo_insert('vending_machine_coupon_log', $log);
