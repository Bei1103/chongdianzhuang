...
class Virtual_ComModel extends ComModel {

    public function updateGoodsStock($id = 0) {

        global $_W, $_GPC;
        $goods = pdo_fetch('select `virtual`,merchid from ' . tablename('vending_machine_goods') . ' where id=:id and type=3 and uniacid=:uniacid limit 1', array(':id' => $id, ':uniacid' => $_W['uniacid']));
        ...
        if (!empty($goods['virtual'])) {
            //单规格
            $stock = pdo_fetchcolumn('select count(*) from ' . tablename('vending_machine_virtual_data') . " where typeid=:typeid and uniacid=:uniacid and merchid = :merchid and openid='' limit 1", array(':typeid' => $goods['virtual'], ':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
        } else {
            $virtuals = array();
            $alloptions = pdo_fetchall("select id, `virtual` from " . tablename('vending_machine_goods_option') . " where goodsid=$id");
            ...
            foreach ($alloptions as $opt) {
                ...
                $c = pdo_fetchcolumn('select count(*) from ' . tablename('vending_machine_virtual_data') . " where typeid=:typeid and uniacid=:uniacid and merchid = :merchid and openid='' limit 1", array(':typeid' => $opt['virtual'], ':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
                pdo_update('vending_machine_goods_option', array('stock' => $c), array('id' => $opt['id']));
        ...
        pdo_update("vending_machine_goods", array("total" => $stock), array("id" => $id));
    }
    ...
    public function updateStock($typeid = 0) {
        ...
        //先找单规格的
        $goods = pdo_fetchall('select id from ' . tablename('vending_machine_goods') . ' where `type`=3 and `virtual`=:virtual and uniacid=:uniacid', array(':virtual' => $typeid, ':uniacid' => $_W['uniacid']));
        foreach ($goods as $g) {
            $goodsids[] = $g['id'];
        }

        //多规格的
        $alloptions = pdo_fetchall("select id, goodsid from " . tablename('vending_machine_goods_option') . " where `virtual`=:virtual and uniacid=:uniacid", array(':uniacid' => $_W['uniacid'], ':virtual' => $typeid));
        foreach ($alloptions as $opt) {
    ...
    public function pay_befo($order){
        ...
        //虚拟物品直接发货
        $goods = pdo_fetch('select id,goodsid,total,realprice from ' . tablename('vending_machine_order_goods') . ' where  orderid=:orderid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':orderid' => $order['id']));
        $g = pdo_fetch('select id,credit,sales,salesreal from ' . tablename('vending_machine_goods') . ' where  id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $goods['goodsid']));
        ...
        if($open_redis) {
            ...
            $last_used = pdo_fetch('select id,typeid,is_top,sort_time from ' . tablename('vending_machine_virtual_data') . ' where  id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $last_id));
        ...
        $virtual_data = pdo_fetchall('SELECT id,typeid,fields FROM ' . tablename('vending_machine_virtual_data') . ' WHERE typeid=:typeid and orderid=:orderid and uniacid=:uniacid and merchid = :merchid '.$last_where.' order by '.$sort_order.' limit ' . $goods['total'], array(':orderid' => 0, ':typeid' => $order['virtual'], ':uniacid' => $_W['uniacid'], ':merchid' => $order['merchid']));
        ...
        $type = pdo_fetch('select fields from ' . tablename('vending_machine_virtual_type') . ' where id=:id and uniacid=:uniacid and merchid = :merchid limit 1 ', array(':id' => $order['virtual'], ':uniacid' => $_W['uniacid'], ':merchid' => $order['merchid']));
        ...
        foreach ($virtual_data as $vd) {
            ...
            //更新虚拟物品数据
            $virtual_data = pdo_update('vending_machine_virtual_data', array(
                'openid' => $order['openid'],
                'orderid' => $order['id'],
            ...
            if($virtual_data){
                pdo_update('vending_machine_virtual_type', 'usedata=usedata+1', array('id' => $vd['typeid']));
            }else{
                $success_flag = false;
            }
            //更新库存
//            $this->updateStock($vd['typeid']); //弃用（解决一次购买N件商品时卡顿的问题） SuniW-2018.7.23
        ...
        //更新订单的状态
        $time = time();
        pdo_update('vending_machine_order', array('virtual_info' => $virtual_info, 'virtual_str' => $virtual_str, 'sendtime' => $time), array('id' => $order['id']));
    ...
    public function pay($order,$ispeerpay=false) {
        ...
        $goods = pdo_fetch('select id,goodsid,total,realprice from ' . tablename('vending_machine_order_goods') . ' where  orderid=:orderid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':orderid' => $order['id']));
        $g = pdo_fetch('select id,credit,sales,salesreal from ' . tablename('vending_machine_goods') . ' where  id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $goods['goodsid']));
        $virtual_data = pdo_fetchall('SELECT id,typeid,fields FROM ' . tablename('vending_machine_virtual_data') . ' WHERE typeid=:typeid and openid=:openid and uniacid=:uniacid and merchid = :merchid order by id asc limit ' . $goods['total'], array(':openid' => '', ':typeid' => $order['virtual'], ':uniacid' => $_W['uniacid'], ':merchid' => $order['merchid']));
        $type = pdo_fetch('select fields from ' . tablename('vending_machine_virtual_type') . ' where id=:id and uniacid=:uniacid and merchid = :merchid limit 1 ', array(':id' => $order['virtual'], ':uniacid' => $_W['uniacid'], ':merchid' => $order['merchid']));


        //更新订单的状态
        $time = time();
        pdo_update('vending_machine_order', array('status' => '3', 'paytime' => $time, 'sendtime' => $time, 'finishtime' => $time), array('id' => $order['id']));
...
//
//            //更新虚拟物品数据
//            pdo_update('vending_machine_virtual_data', array(
//                'openid' => $order['openid'],
//                'orderid' => $order['id'],
//                'ordersn' => $order['ordersn'],
//                'price' => round($goods['realprice'] / $goods['total'], 2),
...
//        //更新订单的状态
//        $time = time();
//        pdo_update('vending_machine_order', array('virtual_info' => $virtual_info, 'virtual_str' => $virtual_str, 'status' => '3', 'paytime' => $time, 'sendtime' => $time, 'finishtime' => $time), array('id' => $order['id']));
//
//        //处理余额抵扣
//        /*if ($order['deductcredit2'] > 0) {
//            $shopset = m('common')->getSysset('shop');
