...
class Printer_ComModel extends ComModel
{
    const PRINTRT_365 = 0;
    ...
    public function printer($params,$templateid,$printerid,$type=0)
    {
        ...
        $printer = pdo_fetch("SELECT * FROM ".tablename('vending_machine_member_printer')." WHERE uniacid=:uniacid AND id=:id",array(':uniacid'=>$_W['uniacid'],':id'=>$printerid));
        if (empty($printer)){
            return error(-1,'小票打印机未找到');
        }
        $template = pdo_fetch("SELECT * FROM ".tablename('vending_machine_member_printer_template')." WHERE uniacid=:uniacid AND id=:id",array(':uniacid'=>$_W['uniacid'],':id'=>$templateid));
        if (empty($printer)){
...
//        $str = $this->getStr($selfMessage);
    ...
    public function getPrinterSet($merchid=0)
    {
        ...
        if (!empty($data['order_printer'])){
            $data['order_printer'] = pdo_fetchall('SELECT id,title FROM ' . tablename('vending_machine_member_printer') . ' WHERE uniacid=:uniacid AND id IN ('.$data['order_printer'].')', array(':uniacid' => $_W['uniacid']));
        }else{
    ...
    public function getStorePrinterSet($store)
    {
        ...
        if (!empty($store['order_printer'])){
            $store['order_printer'] = pdo_fetchall('SELECT id,title FROM ' . tablename('vending_machine_member_printer') . ' WHERE uniacid=:uniacid AND id IN ('.$store['order_printer'].')', array(':uniacid' => $_W['uniacid']));
        }else{
...
//        $this->params = $params;
    ...
    public function sendOrderMessage($orderid = 0,$verify = array())
    {
        ...
        $order = pdo_fetch('select * from ' . tablename('vending_machine_order') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $orderid));
        ...
        $order_goods = pdo_fetchall('select g.id,g.title,g.shorttitle,og.realprice,og.total,og.price,og.optionname as optiontitle,g.noticeopenid,g.noticetype,og.goodssn,og.productsn from ' . tablename('vending_machine_order_goods') . ' og '
        ...
        if (!empty($order['verifystoreid'])){
            $store = pdo_fetch('select * from ' . tablename('vending_machine_store') . " where id=:id and uniacid=:uniacid limit 1", array(':id' => $order['verifystoreid'], ':uniacid' => $_W['uniacid']));
            //获取打印设置
            $StorePrinterSet = $this->getStorePrinterSet($store);
        }
    ...
    public function sendCashierMessage($log,$templateid,$printerid,$operator)
    {
        ...
        if (!empty($log['isgoods'])){
            $goodsAll = pdo_fetchall("SELECT g.title,g.goodssn,og.price,og.total FROM ".tablename('vending_machine_cashier_goods')." g LEFT JOIN ".tablename('vending_machine_cashier_pay_log_goods')." og ON g.id=og.goodsid WHERE og.cashierid=:cashierid AND og.logid=:logid",array(':cashierid'=>$log['cashierid'],':logid'=>$log['id']));
        ...
        if (!empty($log['orderid'])){
            $order_goods = pdo_fetchall('select g.id,g.title,g.shorttitle,og.realprice,og.total,og.price,og.optionname as optiontitle,g.noticeopenid,g.noticetype from ' . tablename('vending_machine_order_goods') . ' og '
                . ' left join ' . tablename('vending_machine_goods') . ' g on g.id=og.goodsid '
