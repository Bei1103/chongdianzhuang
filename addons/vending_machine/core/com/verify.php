...
class Verify_ComModel extends ComModel {

    public function createQrcode($orderid = 0) {
        global $_W, $_GPC;
        $path = IA_ROOT . "/addons/vending_machine/data/qrcode/" . $_W['uniacid'];
        if (!is_dir($path)) {
            load()->func('file');
            mkdirs($path);
        }
        ...
        return $_W['siteroot'] . '/addons/vending_machine/data/qrcode/' . $_W['uniacid'] . '/' . $file;
    }
    ...
    public function allow($orderid, $times = 0,$verifycode = '',$openid = '') {
        ...
        $order = pdo_fetch('select * from ' . tablename('vending_machine_order') . ' where id=:id and uniacid=:uniacid  limit 1', array(':id' => $orderid, ':uniacid' => $uniacid));
        ...
        if (empty($merchid)) {
            $saler = pdo_fetch('select * from ' . tablename('vending_machine_saler') . ' where openid=:openid and uniacid=:uniacid and status=1 limit 1', array(
                ':uniacid' => $_W['uniacid'], ':openid' => $openid
            ));
        } else {
            if ($merch_plugin) {
                $saler = pdo_fetch('select * from ' . tablename('vending_machine_merch_saler') . ' where openid=:openid and uniacid=:uniacid and status=1 and merchid=:merchid limit 1', array(
        ...
        $allgoods = pdo_fetchall("select og.goodsid,og.price,g.title,g.thumb,og.total,g.credit,og.optionid,o.title as optiontitle,g.isverify,g.storeids,g.status".$sqlstr." from " . tablename('vending_machine_order_goods') . " og "
        ...
        if ($order['isverify'] || $order['istrade']) {
            ...
            if (!empty($order['storeid'])) {
                if ($merchid > 0) {
                    $store = pdo_fetch('select * from ' . tablename('vending_machine_merch_store') . ' where id=:id and uniacid=:uniacid and merchid = :merchid limit 1', array(':id' => $order['storeid'], ':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
    ...
    public function verify($orderid = 0, $times = 0,$verifycode = '',$openid = '') {
        ...
        if ($order['isverify'] || $order['istrade']) {
            if ($order['verifytype'] == 0 || $order['verifytype'] == 3) {
                ...
                pdo_insert('vending_machine_verifyorder_log', $verifyorder_log);
                if ($newstore_plugin && !empty($order['istrade'])) {
                    ...
                    pdo_insert('vending_machine_newstore_trade_log', $insert_data);
                }
                ...
                pdo_update('vending_machine_order',
...
//
                if ($order['status'] != 3) {
                    pdo_update('vending_machine_order', array('status' => 3, 'sendtime' => $current_time, 'finishtime' => $current_time), array('id' => $order['id']));
                ...
                if ($order['status'] != 3) {
                    pdo_update('vending_machine_order', array('status' => 3, 'sendtime' => $current_time, 'finishtime' => $current_time, 'verifytime' => $current_time, 'verified' => 1, 'verifyopenid' => $openid, 'verifystoreid' => $saler['storeid']), array('id' => $order['id']));
...
//                    $this->finish(array('status' => 3, 'sendtime' => $current_time, 'finishtime' => $current_time, 'verifytime' => $current_time, 'verified' => 1, 'verifyopenid' => $openid, 'verifystoreid' => $saler['storeid']),$order);
                ...
                pdo_update('vending_machine_order', array('verifyinfo' => iserializer($verifyinfo)), array('id' => $order['id']));
                $verifyorder_log = array();
                $verifyorder_log['uniacid'] = $_W['uniacid'];
                $verifyorder_log['orderid'] = $order['id'];
                $verifyorder_log['salerid'] = $saler['id'];
            ...
            pdo_update('vending_machine_order', array('status' => 3, 'fetchtime' => $current_time,'sendtime'=>$current_time, 'finishtime' => $current_time,'verifytime' => $current_time, 'verified' => 1, 'verifyopenid' => $openid, 'verifystoreid' => $saler['storeid']), array('id' => $order['id']));
...
//        m('member')->upgradeLevel($openid, $order['id']);
    ...
    public function getSalerInfo($openid, $merchid = 0) {
        ...
        if (empty($merchid)) {
            $table_name = tablename('vending_machine_saler');
        } else {
            $table_name = tablename('vending_machine_merch_saler');
    ...
    public function getStoreInfo($storeid, $merchid = 0) {
        ...
        if (empty($merchid)) {
            $table_name = tablename('vending_machine_store');
        } else {
            $table_name = tablename('vending_machine_merch_store');
