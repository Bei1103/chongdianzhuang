...
class Coupon_ComModel extends ComModel {

    protected function getUrl($do, $query = null) {

        $url = mobileUrl($do, $query, true);
        if (strexists($url, '/addons/vending_machine/')) {
            $url = str_replace("/addons/vending_machine/", '/', $url);
        }
    ...
    function get_last_count($couponid = 0) {
        global $_W;
        $coupon = pdo_fetch('SELECT id,total FROM ' . tablename('vending_machine_coupon') . ' WHERE id=:id and uniacid=:uniacid ', array(':id' => $couponid, ':uniacid' => $_W['uniacid']));
        if (empty($coupon)) {
            return 0;
        }
        if ($coupon['total'] == -1) {
        ...
        $gettotal = pdo_fetchcolumn('select count(*) from ' . tablename('vending_machine_coupon_data') . ' where couponid=:couponid and uniacid=:uniacid ', array(':couponid' => $couponid, ':uniacid' => $_W['uniacid']));
    ...
    function creditshop($logid = 0) {
        ...
        $log = pdo_fetch('SELECT * FROM ' . tablename('vending_machine_creditshop_log') . ' WHERE `id`=:id and `uniacid`=:uniacid  limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $logid));
        if (!empty($log)) {
            ...
            //模板消息
            $coupon = pdo_fetch('select * from ' . tablename('vending_machine_coupon') . ' where id=:id limit 1', array(':id' => $log['couponid']));
            ...
            //更新积分商城日志
            pdo_update('vending_machine_creditshop_log', array('status' => 3), array('id' => $logid));
        }
    }
    ...
    function getAvailableCoupons($type, $money = 0, $merch_array,$goods_array=array()){
        ...
        if($type==1)//充值
        {
            ...
            $sql.="and (c.enough=0 or ({$money}>=c.enough and c.enough>0)) ";
        }
        ...
        if (!empty($merch_array)) {
            foreach ($merch_array as $key => $value) {
                ...
                if ($merchid > 0) {
                    ...
                    $sql = "select d.id,d.couponid,d.gettime,c.timelimit,c.timedays,c.timestart,c.timeend,c.thumb,c.couponname,c.enough,c.backtype,c.deduct,c.discount,c.backmoney,c.backcredit,c.backredpack,c.bgcolor,c.thumb,c.merchid,c.limitgoodcatetype,c.limitgoodtype,c.limitgoodcateids,c.limitgoodids  from " . tablename('vending_machine_coupon_data') . " d";
                    $sql.=" left join " . tablename('vending_machine_coupon') . " c on d.couponid = c.id";
        ...
        /*if (!empty($goods_array)) {
            foreach ($goods_array as $key => $value) {
                ...
                $sql = "select id,cates,marketprice,merchid,`type`,discounts,istime,isdiscount,timestart,timeend,isdiscount_time,isdiscount_discounts   from " . tablename('vending_machine_goods') ;
        ...
        if($type==0) {
            $list = $this->checkcouponlimit($list,$goods_array);
        }
        ...
        if (!empty($list)) {
            foreach ($list as &$row) {
                ...
                //是否满额
                if($row['enough']>0&&$row['enough']>$money){
                    $row['color']='gray ';
                    $row['canuse']=0;
                }else{
                    $row['canuse']=1;
    ...
    function checkcouponlimit($list ,$goodlist)
    {
        ...
        foreach($list as $key=> $row)
        {
            ...
            //{
                ...
                {
                    if($row['merchid']!=$good['merchid'])
                    {
                        continue;
                    }

                    if($row['limitdiscounttype']==1&&$good['isdiscountprice']>0||$row['limitdiscounttype']==2&&$good['discountprice']>0||$row['limitdiscounttype']==3&&$good['isdiscountprice']>0&&$good['discountprice']>0){//不与促销优惠同时使用 会员折扣
                        continue;
                    }
                    ...
                    if($row['enough']>0&&$p==1)
                    {
                        /*if($good['optionid']>0&&$good['type']!=4)
                        {
                            ...
                            $sql = "select  marketprice  from " . tablename('vending_machine_goods_option') ;
                            $sql.=" where uniacid=:uniacid and id =:id order by id desc LIMIT 1 "; //类型+最低消费+示使用
    ...
    function payResult($logno) {
        ...
        $log = pdo_fetch('SELECT * FROM ' . tablename('vending_machine_coupon_log') . ' WHERE `logno`=:logno and `uniacid`=:uniacid  limit 1', array(':uniacid' => $_W['uniacid'], ':logno' => $logno));
        if (empty($log)) {
        ...
        if (empty($log['status'])) {
            ...
            $update['status'] = 1;
            pdo_update('vending_machine_coupon_log', $update, array('id' => $log['id']));
    ...
    function sendMessage($coupon, $send_total, $member, $account = null) {
        ...
        if(strexists($siteroot,'/addons/vending_machine/')){
            $replace_str =  array("/addons/vending_machine/");
    ...
    function useRechargeCoupon($log) {
        ...
        $data = pdo_fetch('select id,openid,couponid,used from ' . tablename('vending_machine_coupon_data') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $log['couponid'], ':uniacid' => $_W['uniacid']));
        if (empty($data)) {
            return;
        }
        if (!empty($data['used'])) {
        ...
        $coupon = pdo_fetch('select enough,backcredit,backmoney,backredpack,couponname from ' . tablename('vending_machine_coupon') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $data['couponid'], ':uniacid' => $_W['uniacid']));
        if (empty($coupon)) {
            return;
        }
        ...
        pdo_update('vending_machine_coupon_data', array('used' => 1, 'usetime' => time(), 'ordersn' => $log['logno']), array('id' => $data['id']));
    ...
    function consumeCouponCount($openid, $money = 0, $merch_array,$goods_array) {
        ...
        //平台户优惠券
        $sql = "select d.id,d.couponid,c.enough,c.merchid,c.limitgoodtype,c.limitgoodcatetype,c.limitgoodcateids,c.limitgoodids,c.limitdiscounttype  from " . tablename('vending_machine_coupon_data') . " d";
        $sql.=" left join " . tablename('vending_machine_coupon') . " c on d.couponid = c.id";
        ...
        $sql.=" and ((c.enough>0 and c.enough<=:money) or c.enough=0) ";
        $sql.=" and (   (c.timelimit = 0 and ( c.timedays=0 or c.timedays*86400 + d.gettime >=unix_timestamp() ) )  or  (c.timelimit =1 and c.timestart<={$time} && c.timeend>={$time})) order by d.gettime desc"; //有效期
        $list = pdo_fetchall($sql, $param);
        if (!empty($merch_array)) {
            foreach ($merch_array as $key => $value) {
                ...
                if ($merchid > 0) {
                    ...
                    $param[':merchid'] = $merchid;
                    $param[':money'] = $money;
                    $sql = "select d.id,d.couponid,c.enough,c.merchid,c.limitgoodtype,c.limitgoodcatetype,c.limitgoodcateids,c.limitgoodids  from " . tablename('vending_machine_coupon_data') . " d";
                    $sql.=" left join " . tablename('vending_machine_coupon') . " c on d.couponid = c.id";
    ...
    function rechargeCouponCount($openid, $money = 0) {
        ...
        $time = time();
        $sql = "select count(*) from " . tablename('vending_machine_coupon_data') . " d "
    ...
    function useConsumeCoupon($orderid = 0) {
        ...
        $order = pdo_fetch('select ordersn,createtime,couponid from ' . tablename('vending_machine_order') . ' where id=:id and status>=0 and uniacid=:uniacid limit 1', array(':id' => $orderid, ':uniacid' => $_W['uniacid']));
        if (empty($order)) {
            return;
        }
        ...
        pdo_update('vending_machine_coupon_data', array('used' => 1, 'usetime' => $order['createtime'], 'ordersn' => $order['ordersn']), array('id' => $order['couponid']));
    ...
    function returnConsumeCoupon($order) {
        global $_W;
        if (!is_array($order)) {
            $order = pdo_fetch('select id,openid,ordersn,createtime,couponid,status,finishtime from ' . tablename('vending_machine_order') . ' where id=:id and status>=0 and uniacid=:uniacid limit 1', array(':id' => intval($order), ':uniacid' => $_W['uniacid']));
        }
        ...
        if (!empty($coupon['used']) && ($coupon['ordersn'] == $order['ordersn'])) {
            pdo_update('vending_machine_coupon_data', array('used' => 0, 'usetime' => 0, 'ordersn' => ''), array('id' => $order['couponid']));
    ...
    function backConsumeCoupon($orderid) {
        ...
        if (!is_array($orderid)) {
            $order = pdo_fetch('select id,openid,ordersn,createtime,couponid,couponmerchid,status,finishtime,`virtual`,isparent,parentid,coupongoodprice  from ' . tablename('vending_machine_order') . ' where id=:id and status>=0 and couponid >0 and uniacid=:uniacid limit 1', array(':id' => intval($orderid), ':uniacid' => $_W['uniacid']));
        }
        ...
        if ($isparent == 1 || $parentid != 0) {
            ...
            if ($isparent == 1) {
                ...
                if ($couponmerchid > 0) {
                    //使用了多商户优惠券
                    $sql = 'select id,openid,ordersn,createtime,couponid,couponmerchid,status,finishtime,`virtual`,isparent,parentid from ' . tablename('vending_machine_order') . ' where parentid=:parentid and couponmerchid=:couponmerchid and status>=0 and uniacid=:uniacid limit 1';
        ...
        if ($parentid != 0 && $couponmerchid == 0) {
            if ($all_done == 1) {
                $sql = 'select id,openid,ordersn,createtime,couponid,couponmerchid,status,finishtime,`virtual`,isparent,parentid from ' . tablename('vending_machine_order') . ' where id=:id and status>=0 and uniacid=:uniacid limit 1';
        ...
        if ($canback) {
            ...
            if($parentid>0)
            {
                $ordermoney = pdo_fetchcolumn('select coupongoodprice from ' . tablename('vending_machine_order') . ' where id=:id and status>=0 and couponid >0 and uniacid=:uniacid limit 1', array(':id' => intval($parentid), ':uniacid' => $_W['uniacid']));
            }else
            {
            ...
            if($ordermoney==0)
            {
                $sql = 'select ifnull( sum(og.realprice),0) from ' . tablename('vending_machine_order_goods') . ' og ';
                $sql .= ' left join ' . tablename('vending_machine_order') . ' o on';
            ...
            pdo_update('vending_machine_coupon_data', array('back' => 1, 'backtime' => time()), array('id' => $order['couponid']));
    ...
    function getCoupon($couponid = 0) {
        ...
        return pdo_fetch('select * from ' . tablename('vending_machine_coupon') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $couponid, ':uniacid' => $_W['uniacid']));
    }

    function getCouponByDataID($dataid = 0) {
        global $_W;
        $data = pdo_fetch('select id,openid,couponid,used,back,backtime,ordersn from ' . tablename('vending_machine_coupon_data') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $dataid, ':uniacid' => $_W['uniacid']));
        if (empty($data)) {
            return false;
        }
        $coupon = pdo_fetch('select * from ' . tablename('vending_machine_coupon') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $data['couponid'], ':uniacid' => $_W['uniacid']));
        if (empty($coupon)) {
            return false;
        }
    ...
    function setCoupon($row, $time, $withOpenid = true) {
        ...
        if ($withOpenid) {
            ...
            if ($row['getmax'] > 0) {
                $gets = pdo_fetchcolumn('select count(*) from ' . tablename('vending_machine_coupon_data') . ' where couponid=:couponid and openid=:openid and uniacid=:uniacid and gettype=1 limit 1', array(':couponid' => $row['id'], ':openid' => $openid, ':uniacid' => $_W['uniacid']));
    ...
    function addtaskdata($orderid)
    {
        ...
        $order = pdo_fetch('select id,openid,price  from ' . tablename('vending_machine_order') . ' where id=:id   and uniacid=:uniacid limit 1', array(':id' => intval($orderid), ':uniacid' => $_W['uniacid']));
        ...
        {
            $price = $order['price'];
            $sendtasks = pdo_fetch('select id,couponid,sendnum,num,sendpoint  from ' . tablename('vending_machine_coupon_sendtasks') . '
            ...
            if(!empty($sendtasks))
            {
                ...
                pdo_insert('vending_machine_coupon_taskdata', $data);

                $num = intval($sendtasks['num'])-intval($sendtasks['sendnum']);
                pdo_update('vending_machine_coupon_sendtasks', array('num' => $num), array('id' => $sendtasks['id']));
            }
        }
        ...
        if($pdata['isopengoodssendtask'] == 1)
        {
            ...
            from '. tablename('vending_machine_coupon_goodsendtask').' gst
            inner join ' . tablename('vending_machine_order_goods') . ' og on og.goodsid =gst.goodsid  and (orderid=:orderid or parentorderid=:orderid)
            ...
            foreach($goodssendtasks as $task)
            {
                if ($task['starttime'] <= TIMESTAMP && $task['endtime'] >= TIMESTAMP){
                    ...
                    pdo_update('vending_machine_coupon_goodsendtask', array('num' => $num), array('id' => $task['taskid']));
                }
            }
    ...
    function sendcouponsbytask($orderid) {
        ...
        if (!is_array($orderid)) {
            $order = pdo_fetch('select id,openid,ordersn,createtime,status,finishtime,`virtual`,isvirtualsend,isparent,parentid  from ' . tablename('vending_machine_order') . ' where id=:id and status>=0  and uniacid=:uniacid limit 1', array(':id' => intval($orderid), ':uniacid' => $_W['uniacid']));
        }
        if (empty($order)) {
            return;
        }
        ...
        //判断是否是记次/时商品
        if($order['status'] == 2){
            $sql = 'SELECT goodsid FROM '.tablename('vending_machine_order_goods').' WHERE uniacid=:uniacid AND orderid=:orderid ';
            $goodsids = pdo_fetchall($sql,array(':uniacid'=>$_W['uniacid'],':orderid'=>$order['id']));
            foreach($goodsids as $gidk => $gidv){
                $gtype = pdo_fetch('SELECT type FROM '.tablename('vending_machine_goods').' WHERE uniacid=:uniacid AND id=:id',array(':uniacid'=>$_W['uniacid'],':id'=>$gidv['goodsid']));
                if($gtype['type'] == 5){
        ...
        if ($isonlyverifygoods || $order['status'] == 1)
        {
            ...
            if($parentid>0)
            {
                $num = pdo_fetchcolumn('select 1 from '.tablename('vending_machine_order') .'
    ...
    function getOrderSendCoupons($orderid,$sendpoint,$tasktype,$openid)
    {
        ...
        if($sendpoint ==2)
        {
            $taskdata = pdo_fetchall('select id, couponid, sendnum  from ' . tablename('vending_machine_coupon_taskdata') . '
    ...
    function posterbylist($list ,$openid,$gettype)
    {
        ...
        foreach($list  as $taskdata)
        {
            ...
            // 通过优惠券id查找商户id
            $merchid = pdo_fetchcolumn("select merchid from " . tablename('vending_machine_coupon') . ' where id=:couponid limit 1', array(':couponid' => $taskdata['couponid']));
            ...
            for ($i = 1; $i <= $couponnum; $i++) {
                ...
                pdo_insert('vending_machine_coupon_sendshow', $data);
            }

            pdo_update('vending_machine_coupon_taskdata', array('status' => 1), array('id' => $taskdata['id']));
        }
    ...
    function posterbyusesendtask($couponid,$openid)
    {
        ...
        $list = pdo_fetchall('select  *  from ' . tablename('vending_machine_coupon_usesendtasks') . '
        ...
        foreach($list  as $taskdata)
        {
            ...
            pdo_update('vending_machine_coupon_usesendtasks', array('num' => $num2), array('id' => $taskdata['id']));
        }
    ...
    function getCoupons() {
        ...
        return pdo_fetchall('select * from ' . tablename('vending_machine_coupon') . ' where  (timelimit = 0  or  (timelimit =1 and timestart<={$time} && timeend>={$time})) and uniacid=:uniacid', array(':uniacid' => $_W['uniacid']));
    }
    ...
    function getInfo(){
        ...
        $osql = 'SELECT `id` FROM '.tablename('vending_machine_order').$condition;
        $order = pdo_fetchall($osql,$paramso);
        //根据订单判断是否是新用户
        if (empty($order)) {

            $sql2 = 'SELECT * FROM '.tablename('vending_machine_sendticket').' WHERE uniacid = '.intval($_W['uniacid']);
            $ticket = pdo_fetch($sql2);
            if ($ticket['status'] == 1) {
                if ($ticket['expiration'] == 1) {
                    if (TIMESTAMP > $ticket['endtime']) {
                        $status = array('status' => 0);
                        pdo_update('vending_machine_sendticket',$status,array('id' => $ticket['id']));
                        return false;
                    } else {
...
//                                dump($cpinfo);exit;
    ...
    function getCoupon_new($cpid){
        ...
        if(is_array($cpids)){
            ...
            foreach ($cpids as $cpk => $cpv) {
                $cpsql = 'SELECT * FROM '.tablename('vending_machine_coupon').' WHERE uniacid = '.intval($_W['uniacid']).' AND id = '.intval($cpv);
            ...
            $cpsql = 'SELECT * FROM '.tablename('vending_machine_coupon').' WHERE uniacid = '.intval($_W['uniacid']).' AND id = '.intval($cpid);
    ...
    function insertDraw($openid,$cpinfo){
        ...
        $drawsql = 'SELECT * FROM '.tablename('vending_machine_sendticket_draw').' WHERE  openid = :openid AND uniacid = :uniacid ';
        $drawparpams = array(
            ':uniacid' => intval($_W['uniacid']),
            ':openid' => trim($openid),
        );
        ...
        if (empty($drawdata)) {
            ...
            $insert = pdo_insert('vending_machine_sendticket_draw',$data);
            return $insert;
        } else {
            return false;
        }
    ...
    function sendTicket($openid, $couponid,$gettype=0) {
        ...
        $log = pdo_insert('vending_machine_coupon_log', $couponlog);
        ...
        $data = pdo_insert('vending_machine_coupon_data', $data);
        $did = pdo_insertid();
    ...
    function activity($money){
        global $_W;
        $sql = 'SELECT * FROM '.tablename('vending_machine_sendticket_share').' WHERE uniacid = '.intval($_W['uniacid']).' AND status = 1 AND (enough = '.$money.' OR enough <= '.$money.') AND (expiration = 0 OR (expiration = 1 AND endtime >= '.TIMESTAMP.')) ORDER BY enough DESC,createtime DESC LIMIT 1';
    ...
    function getCanGetCouponNum($merchid=0){
        ...
        $sql = "select id,timelimit,coupontype,timedays,timestart,timeend,couponname,enough,backtype,deduct,discount,backmoney,backcredit,backredpack,bgcolor,thumb,credit,money,getmax,merchid,total as t,tagtitle,settitlecolor,titlecolor  from " . tablename('vending_machine_coupon');
    ...
    function sethasnewcoupon($openid,$hasnewcoupon=0){
        global $_W;
        pdo_update('vending_machine_member', array('hasnewcoupon'=>$hasnewcoupon), array('openid'=>$openid,'uniacid'=>$_W['uniacid']));
    }
