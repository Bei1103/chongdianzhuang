...
class Wxcard_ComModel extends ComModel {
...
//        }
    ...
    public function wxCardUpdateQuantity($card_id) {
        ...
        if(!empty($id)){
            //平台户优惠券
            $sql ="select id,uniacid, card_id from " . tablename('vending_machine_wxcard') ;
        ...
        pdo_update('vending_machine_wxcard',$data,array('id' => $id));
    ...
    function consumeWxCardCount($openid,  $merch_array,$goods_array) {
        ...
        //平台户优惠券
        $sql ="select id,uniacid, card_id,least_cost,reduce_cost,merchid,limitgoodtype,limitgoodcatetype,limitgoodcateids,limitgoodids  from " . tablename('vending_machine_wxcard') ;
    ...
    function getAvailableWxcards($type, $money = 0, $merch_array,$goods_array=array()){
        ...
        //平台户优惠券
        $sql ="select id,uniacid,card_type,logo_url,title, card_id,least_cost,reduce_cost,discount,merchid,limitgoodtype,limitgoodcatetype,limitgoodcateids,limitgoodids,datetype,end_timestamp,fixed_term  from " . tablename('vending_machine_wxcard') ;
        $sql.="  where uniacid=:uniacid and merchid=0 and card_id in ({$card_id})  order by id desc";
        $cardlist = pdo_fetchall($sql, $param);
    ...
    public function ActivateMembercardbyopenid($openid)
    {
        ...
        $sql="select *  from " . tablename('vending_machine_member') . " where  openid=:openid  and uniacid=:uniacid  limit 1";
        ...
        if(empty($member['level'])){
            ...
            $level = pdo_fetchcolumn('select levelname from ' . tablename('vending_machine_member_level') . ' where id=:id limit 1', array(':id' => $member['level']));
        }
    ...
    public function ActivateVerifygoodCard($id,$card_id,$code,$openid)
    {
        ...
        $sql='select vg.*,c.card_id  from ' . tablename('vending_machine_verifygoods') . '   vg
	 ...
	 inner  join ' . tablename('vending_machine_goods_cards') . ' c on c.id = g.cardid
	 ...
	 and  ((vg.limittype=0   and vg.limitdays * 86400 + vg.starttime >=unix_timestamp() )or ( vg.limittype=1   and vg.limitdate >=unix_timestamp() ))  and  vg.used =0 and (vg.activecard=0 or vg.activecard is null) and g.cardid>0 and vg.id=:id ';
        ...
        $verifygoodlogs = pdo_fetchall('select *  from ' . tablename('vending_machine_verifygoods_log') . '    where verifygoodsid =:id  ', array(':id' => $id));
        ...
        {
            pdo_update("vending_machine_verifygoods",array('activecard'=>1,'cardcode'=>$code),array('id'=>$id));
            return true;
        }
    }
    ...
    public function updateusercardbyvarifygoodid($id)
    {
        ...
        $sql='select vg.*,c.card_id,vg.cardcode  from ' . tablename('vending_machine_verifygoods') . '   vg
	 inner join ' . tablename('vending_machine_order_goods') . ' og on vg.ordergoodsid = og.id
	 inner join ' . tablename('vending_machine_goods') . ' g on og.goodsid = g.id
	 inner  join ' . tablename('vending_machine_goods_cards') . ' c on c.id = g.cardid
    ...
    {
        global $_W;
        $sql='select g.id  from ' . tablename('vending_machine_goods') . ' g
         inner join  ' . tablename('vending_machine_goods_cards') . '  c on c.id = g.cardid  where g.uniacid=:uniacid and c.card_id=:card_id';
    ...
    public function updateMemberInfoWithWxcard($openid){
        ...
        $result = pdo_update('vending_machine_member', $arr, array('openid' => $openid, 'uniacid' => $_W['uniacid']));
        return $result;
    }
