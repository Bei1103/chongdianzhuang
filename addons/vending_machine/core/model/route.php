...
class Route_Model {
    ...
    function run($isweb = true)
    {
        ...
        // autoload
        if (version_compare(VENDING_MACHINE_VERSION, '3.14.0', '>=')) {
            @include_once VENDING_MACHINE_PATH . 'vendor/autoload.php';
        }
        
        require_once IA_ROOT . "/addons/vending_machine/core/inc/page.php";
        if ($isweb) {
            require_once VENDING_MACHINE_CORE . "inc/page_web.php";
            require_once VENDING_MACHINE_CORE . "inc/page_web_com.php";
        } else{
            require_once VENDING_MACHINE_CORE . "inc/page_mobile.php";
            require_once VENDING_MACHINE_CORE . "inc/page_mobile_login.php";
        }
        ...
        $method = "main";
        $root = $isweb ? VENDING_MACHINE_CORE_WEB : VENDING_MACHINE_CORE_MOBILE;
        ...
        if(strexists($_W['siteurl'] ,"web/merchant.php")) {
            ...
            $isplugin = !empty($r) && is_dir(VENDING_MACHINE_PLUGIN . $routes[0]);
        }


        if ($isplugin) {
            if ($isweb) {
                require_once VENDING_MACHINE_CORE . "inc/page_web_plugin.php";
                
            } else {

                require_once VENDING_MACHINE_CORE . "inc/page_mobile_plugin.php";
                require_once VENDING_MACHINE_CORE . "inc/page_mobile_plugin_login.php";
                require_once VENDING_MACHINE_CORE . "inc/page_mobile_plugin_pf.php";
            }

            $_W['plugin'] = $routes[0];
            $root = VENDING_MACHINE_PLUGIN . $routes[0] . "/core/" . ( $isweb ? "web" : "mobile") . "/";
            if($isMerch){
                $_W['plugin'] ="merch";
                $root = VENDING_MACHINE_PLUGIN .  "merch/core/web/manage/";
            } else if($isNewstore){
                $_W['plugin'] ="newstore";
                $root = VENDING_MACHINE_PLUGIN .  "newstore/core/web/manage/";
            }
            else{
            ...
            require_once VENDING_MACHINE_CORE . "inc/page_system.php";
        }
