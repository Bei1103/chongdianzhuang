...
class Member_Model {
    ...
    public function getInfo($openid = '')
    {
        ...
        $uid = intval($openid);
        if (strlen($openid)>11) {
            $info = pdo_fetch('select * from ' . tablename('vending_machine_member') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            if (empty($info)) {
                ...
                if (!empty($condition)) {
                    $info = pdo_fetch('select * from ' . tablename('vending_machine_member') . ' where ' . $condition . '  and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
                    if (!empty($info)) {
            ...
            $info = pdo_fetch('select * from ' . tablename('vending_machine_member') . ' where id=:id  and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $uid));
        }
        if (!empty($info['uid'])) {
            //会员余额积分信息
            load()->model('mc');
     ...
     * 获取会员信息
     */
    public function getMember($openid = '',$type=0)//0微信 1支付宝
    {
        global $_W;
        if($type==1){
            $info = pdo_fetch('select * from ' . tablename('vending_machine_member') . " where alipay_user_id=:alipay_user_id and uniacid=:uniacid and alipay_user_id!='' limit 1", array(':uniacid' => $_W['uniacid'], ':alipay_user_id' => $openid));
        }else {
            $uid = (int)$openid;
            if (strlen($openid)>11) {
                $info = pdo_fetch('select * from ' . tablename('vending_machine_member') . ' where  openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
                ...
                $info = pdo_fetch('select * from ' . tablename('vending_machine_member') . ' where id=:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $openid));
            }
        }
        ...
        if (!empty($info)) {
            ...
            if(strpos($info['avatar'],'132132')){
                ...
                pdo_update('vending_machine_member', $upgrade2, array('id' => $info['id']));
            }
        ...
        if (empty($info['uid'])) {
            ...
            if ($followed) {
                ...
                if (!empty($uid)) {
                    ...
                    if (!empty($upgrade)) {
                        pdo_update('vending_machine_member', $upgrade, array('id' => $info['id']));
                    }
                }
            }
        }
    ...
    public function getMobileMember($mobile) {
        global $_W;
        $info = pdo_fetch('select * from ' . tablename('vending_machine_member') . ' where mobile=:mobile and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':mobile' => $mobile));
        if (!empty($info)){
            $info = $this->updateCredits($info);
        }
        return $info;
    ...
    public function setCredit($openid = '', $credittype = 'credit1', $credits = 0, $log = array()) {
        ...
        if (!empty($uid)) {
            ...
            //如果未关注
            $value = pdo_fetchcolumn("SELECT {$credittype} FROM " . tablename('vending_machine_member') . " WHERE  uniacid=:uniacid and openid=:openid limit 1", array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            $newcredit = $credits + $value;
            if ($newcredit <= 0) {
                $newcredit = 0;
            }
            pdo_update('vending_machine_member', array($credittype => $newcredit), array('uniacid' => $_W['uniacid'], 'openid' => $openid));
        ...
        //新增商城积分(余额)记录表
        $member_log_table_flag = pdo_tableexists('vending_machine_member_credit_record');
        if($member_log_table_flag){
            $log_data['openid'] = $openid;
            $log_data['presentcredit'] = $a;
            pdo_insert('vending_machine_member_credit_record', $log_data);
        }
...
//        }
    ...
    public function getCredit($openid = '', $credittype = 'credit1') {
        ...
        if (!empty($uid)) {
            ...
            //如果未关注
            $item = pdo_fetch("SELECT {$credittype} FROM " . tablename('vending_machine_member') . " WHERE openid=:openid and uniacid=:uniacid limit 1", array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            if(empty($item)){
                $item = pdo_fetch("SELECT {$credittype} FROM " . tablename('vending_machine_member') . " WHERE openid_wa=:openid and uniacid=:uniacid limit 1", array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }
    ...
    public function getCredits($openid = '', $credittypes = array('credit1', 'credit2')) {
        ...
        if (!empty($uid)) {
            ...
            $item = pdo_fetch("SELECT {$types} FROM " . tablename('vending_machine_member') . " WHERE openid=:openid and uniacid=:uniacid limit 1", array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            if(empty($item)){
                $item = pdo_fetch("SELECT {$types} FROM " . tablename('vending_machine_member') . " WHERE openid_wa=:openid and uniacid=:uniacid limit 1", array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }
            //如果未关注
//            return pdo_fetch("SELECT {$types} FROM " . tablename('vending_machine_member') . " WHERE  (openid=:openid or openid_wa=:openid) and uniacid=:uniacid limit 1", array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            if(empty($item)){
	...
	public function checkMember() {
		...
		$member = array();
		$shopset = m('common')->getSysset(array('shop','wap','pay'));
		$openid = $_W['openid'];
        ...
        if(empty($openid) && !VENDING_MACHINE_DEBUG&&is_alipay()){//支付宝内

            if($shopset['pay']['alipay']==1&&!empty($shopset['pay']['alipay_id'])){

                $auth_code=trim($_GPC['auth_code']);
                if(!empty($auth_code)) {
                    $alipayclient=p('alipay');
                }
                if(!empty($auth_code)){
                    $result = $alipayclient->get_token($auth_code);
                    if(is_error($result)){
                        if($_W['ispost']){
                    ...
                    if(!empty($user_id)){
                        $member=$this->getMember($user_id,1);
                        if(empty($member)){
                            $info=$alipayclient->get_info($result['access_token']);
                            if(is_error($info)){
                                if($_W['ispost']){
                                    show_json(0,$info['message']);
                                }else{
                ...
                }/*elseif(!empty($accessToken)&&!empty($accessToken['token']) && $accessToken['expire'] > TIMESTAMP){
                    $info=Factory::base()->oauth()->info($accessToken['token']);
                    if(!empty($info->userId)){
                        $member=$this->getMember($info->userId,1);
                    }
                }*/

                if(empty($member)&&!empty($info)&&!empty($info['user_id'])){
                    //exit();
                    $member = array(
                        'uniacid' => $_W['uniacid'],
                        'openid' => md5('alipay:'.$info['user_id']),
                        'nickname' => !empty($info['nick_name']) ? $info['nick_name'] : '',
                        'avatar' => !empty($info['avatar']) ? $info['avatar'] : '',
                        'gender' => $info['gender']=='f'?1:$info['gender']=='m'?1: '-1',
                        'province' => !empty($info['province']) ?$info['province'] : '',
                        'city' => !empty($info['city']) ?$info['city'] : '',
                        'createtime' => time(),
                        'status' => 0,
                        'alipay_user_id'=>$info['user_id'],
                    );

                    pdo_insert('vending_machine_member', $member);
                    $member['id'] = pdo_insertid();
                    $this->deleterepeat($member['openid']);
                ...
                if(!empty($member['id'])) {
                    $_SESSION['openid'] = $_W['openid']=$openid=$member['openid'];
                }
                if(empty($openid)) {
                    $payments = pdo_fetch("SELECT * FROM ".tablename('vending_machine_payment')." WHERE uniacid=:uniacid AND id=:id",array(':uniacid'=>$_W['uniacid'],':id'=>$shopset['pay']['alipay_id']));
                    $payments=iunserializer($payments['alipay_sec']);
                    ...
                    $oauth_url = "https://openauth.alipay.com/oauth2/publicAppAuthorize.htm?app_id={$payments['appid']}&scope=auth_user&redirect_uri=" . urlencode($url);
        ...
        if(empty($openid) && !VENDING_MACHINE_DEBUG&&is_h5app()&&!empty($_GPC['alipay_user_id'])){//app内支付宝跳转
            $member=$this->getMember(trim($_GPC['alipay_user_id']),1);
            if(!empty($member)) {
                $_SESSION['openid'] = $_W['openid']=$openid=$member['openid'];
            }
            if(empty($member)){
        ...
        //app内
        if(is_h5app()&&!empty($_GPC['openid'])){//app内微信跳转
		...
		//如果你把下面这个if注释了,你就倒霉了
		if (empty($openid) && !VENDING_MACHINE_DEBUG) {
		...
		if (empty($member) && !empty($openid)) {
			...
			if (!empty($upgrade)) {
				pdo_update('vending_machine_member', $upgrade, array('id' => $member['id']));
			}
            $this->deleterepeat($openid);
		}
		//分销商
    ...
    function getLevels($all=true) {
        ...
        return pdo_fetchall('select * from ' . tablename('vending_machine_member_level') . ' where uniacid=:uniacid'.$condition.' order by level asc', array(':uniacid' => $_W['uniacid']));
    }
    ...
    function getLevel($openid) {
        ...
        if (!empty($member) && !empty($member['level'])) {
            $level = pdo_fetch('select * from ' . tablename('vending_machine_member_level') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $member['level'], ':uniacid' => $_W['uniacid']));
            if (!empty($level)) {
                return $level;
            }
        }
    ...
    function getOneGoodsLevel($openid, $goodsid) {
        ...
        $data = array();
        $levels = pdo_fetchall('select * from ' . tablename('vending_machine_member_level') . ' where uniacid=:uniacid and buygoods=1 and level and level > :level order by level asc', array(':uniacid' => $uniacid, ':level' => $level));
    ...
    function getGoodsLevel($openid, $orderid) {
        ...
        $order_goods = pdo_fetchall("select goodsid from " . tablename('vending_machine_order_goods') . " where orderid=:orderid and uniacid=:uniacid", array(':uniacid' => $uniacid, ':orderid' => $orderid));
    ...
    function upgradeLevel($openid, $orderid = 0, $status = 3) {
        ...
        $level = false;
        if (empty($leveltype)) {
            /*$ordermoney = pdo_fetchcolumn('select ifnull( sum(og.realprice),0) from ' . tablename('vending_machine_order_goods') . ' og '
            ...
            //根据订单支付金额来判断   -- yqj --
            $ordermoney = pdo_fetchcolumn('select sum(price) from ' . tablename('vending_machine_order') . " where uniacid=:uniacid and openid=:openid and status=:status limit 1", array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid'], ':status' => $status));
            $ordermoney = floatval($ordermoney);
            $level = pdo_fetch('select * from ' . tablename('vending_machine_member_level') . " where uniacid=:uniacid  and enabled=1 and {$ordermoney} >= ordermoney and ordermoney>0  order by level desc limit 1", array(':uniacid' => $_W['uniacid']));
        } else if ($leveltype == 1) {
            $ordercount = pdo_fetchcolumn('select count(*) from ' . tablename('vending_machine_order') . ' where openid=:openid and status=:status and uniacid=:uniacid ', array(':uniacid' => $_W['uniacid'], ':openid' => $member['openid'],':status' => $status));
            $level = pdo_fetch('select * from ' . tablename('vending_machine_member_level') . " where uniacid=:uniacid and enabled=1 and {$ordercount} >= ordercount and ordercount>0  order by level desc limit 1", array(':uniacid' => $_W['uniacid']));
        }
        ...
        if ($canupgrade) {
            //会员升级
            pdo_update('vending_machine_member', array('level' => $level['id']), array('id' => $member['id']));
    ...
    function upgradeLevelByLevelId($openid, $LevelID) {
        ...
        $level = pdo_fetch('select *  from ' . tablename('vending_machine_member_level') . " where uniacid=:uniacid and enabled=1 and id=:id",  array(':uniacid' => $_W['uniacid'],':id' =>$LevelID));
    ...
    function getGroups() {
        global $_W;
        return pdo_fetchall('select * from ' . tablename('vending_machine_member_group') . ' where uniacid=:uniacid order by id asc', array(':uniacid' => $_W['uniacid']));
    }
    ...
    function setGroups($openid,$group_ids,$reason = ''){
        ...
        $old_group_ids = pdo_getcolumn('vending_machine_member',$condition,'groupid');
        ...
        pdo_update('vending_machine_member', array('groupid'=>$group_ids), $condition);
        foreach ($diff_ids as $groupid){
            pdo_insert('vending_machine_member_group_log',
    ...
    public function checkMemberFromPlatform($openid = '',$acc='') {
        ...
        if (empty($member)) {
            ...
            pdo_update('vending_machine_member', $member, array('id' => $member['id']));
    ...
    public function checkMemberSNS($sns){
        ...
        if (empty($member)){
            ...
            pdo_insert('vending_machine_member', $data);
            if(method_exists(m('member'),'memberRadisCountDelete')) {
                m('member')->memberRadisCountDelete(); //清除会员统计radis缓存
            }
            return pdo_insertid();
        ...
        elseif(empty($member['bindsns']) || $member['bindsns']==$sns){
            pdo_update('vending_machine_member', $data, array('id'=>$member['id'], 'uniacid'=>$_W['uniacid']));
            return $member['id'];
        }
    }
    ...
    public function memberRadisCountDelete(){
        ...
        if($open_redis){ //判断redisshi
            $redis = redis();
            $keysArr = $redis -> keys("vending_{$_W['uniacid']}_member*"); //查询出所有会员统计的缓存的key值
    ...
    public function deleterepeat($openid = '')
    {
        ...
        if (!empty($uid)) {
            $result = pdo_fetchall("SELECT id,openid,createtime FROM ".tablename('vending_machine_member')." WHERE uniacid=:uniacid and openid=:openid order by createtime DESC",array('uniacid' => $_W['uniacid'],'openid' => $uid));
            $count = count($result);
            if ($count > 1){
                pdo_delete('vending_machine_member',array('id'=>$result['0']['id'],'uniacid'=>$_W['uniacid']));
            }
        }
