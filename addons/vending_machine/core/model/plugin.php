...
class Plugin_Model {
    ...
    public function exists($pluginName = '') {
        ...
        $dbplugin = pdo_fetchall('select * from ' . tablename('vending_machine_plugin') . ' where identity=:identyty limit  1', array(':identity' => $pluginName));
        if (empty($dbplugin)) {
            return false;
        }
        return true;
    ...
    public function getAll($iscom = false,$status='') {
        ...
        if ($iscom) {
            ...
            if (empty($plugins)) {
                $plugins = pdo_fetchall('select * from ' . tablename('vending_machine_plugin') . ' where iscom=1 and deprecated=0 '.$status.' order by displayorder asc');
            ...
            if (empty($plugins)) {
                $plugins = pdo_fetchall('select * from ' . tablename('vending_machine_plugin') . ' where iscom=0 and deprecated=0 '.$status.' order by displayorder asc');
                $plugins = $this->arrayUnique($plugins);
    ...
    public function refreshCache($status='',$iscom = false) {
        ...
        $com = pdo_fetchall('select * from ' . tablename('vending_machine_plugin') . ' where iscom=1 and deprecated=0 '.$status.' order by displayorder asc');
    ...
    public function getList($status='') {
        ...
        if(file_exists($filename)){
            $item = pdo_fetch('select  plugins from ' . tablename('vending_machine_perm_plugin') . ' where acid=:acid limit 1', array(':acid' => $_W['uniacid']));
            ...
            foreach ($plugins as $key => $value){
                if(!strstr($item['plugins'],$value['identity']) && !strstr($setting['plugin'],$value['identity']) && !strstr($setting['com'],$value['identity'])){
                    $plugin = pdo_fetch("SELECT max(permendtime) as permendtime FROM " . tablename('vending_machine_system_grant_log') . " 
            ...
            $item = pdo_fetch('select plugins from ' . tablename('vending_machine_perm_plugin') . ' where acid=:acid limit 1', array(':acid' => $acid['acid']));
            $setting = pdo_fetch("select * from ".tablename('vending_machine_system_plugingrant_setting')." where 1 = 1 limit 1 ");

            foreach ($plugins as $key => $value){
                if(!strstr($item['plugins'],$value['identity']) && !strstr($setting['plugin'],$value['identity']) && !strstr($setting['com'],$value['identity'])){
                    $plugin = pdo_fetchcolumn("SELECT count(1) FROM " . tablename('vending_machine_system_plugingrant_log') . "
                    WHERE `identity` = '".$value['identity']."' and uniacid = ".$acid['uniacid']." and isperm = 1 and `month` = 0 ");
                    if($plugin > 0){
                        $plugin = pdo_fetch("SELECT max(permendtime) as permendtime,`month`,isperm FROM " . tablename('vending_machine_system_plugingrant_log') . "
                        WHERE `identity` = '".$value['identity']."' and uniacid = ".$acid['uniacid']." and isperm = 1 and `month` = 0 ");
                    }else{
                        $plugin = pdo_fetch("SELECT max(permendtime) as permendtime,`month`,isperm FROM " . tablename('vending_machine_system_plugingrant_log') . " 
    ...
    public function loadModel($pluginname = '') {
        ...
        static $_model;
        if (!$_model) {
            $modelfile = IA_ROOT . '/addons/vending_machine/plugin/' . $pluginname . "/core/model.php";
    ...
    public function getConfig($pluginname = '') {
        ...
        $config_file = $moduleroot = IA_ROOT . "/addons/vending_machine/plugin/".$pluginname."/config.php";
    ...
    public function permission($pluginName, $uniacid = 0)
    {
        ...
        // 系统给的所有可用插件
        $allPlugins = pdo_fetchall('select * from ' . tablename('vending_machine_plugin') . ' where iscom=0 and deprecated=0 and status=1 order by displayorder asc');
        $allPlugins = array_column($allPlugins, 'identity');

        if($permset) {
            $plugins = pdo_fetchcolumn("SELECT plugins FROM " . tablename('vending_machine_perm_plugin') . " WHERE acid =:id limit 1", array(':id' => $id));
