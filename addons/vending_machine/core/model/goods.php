...
class Goods_Model {
    ...
    public function getOption($goodsid = 0, $optionid = 0) {
        global $_W;
        return pdo_fetch("select * from " . tablename('vending_machine_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid Limit 1', array(':id' => $optionid, ':uniacid' => $_W['uniacid'], ':goodsid' => $goodsid));
    }
    ...
    public function getOptionPirce($goodsid = 0, $optionid = 0) {
        global $_W;
        return pdo_fetchcolumn("select marketprice from " . tablename('vending_machine_goods_option') . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid', array(':id' => $optionid, ':uniacid' => $_W['uniacid'], ':goodsid' => $goodsid));
    }
    ...
    public function getList($args = array()) {
        ...
        if (!$random) {
            ...
            FROM " . tablename('vending_machine_goods') . " where  {$condition} GROUP BY id ORDER BY {$order} {$orderby} LIMIT " . ($page - 1) * $pagesize . ',' . $pagesize;
            $total = pdo_fetchcolumn("select count(*) from " . tablename('vending_machine_goods') . " where  {$condition} ",$params);
        } else {
            ...
            FROM " . tablename('vending_machine_goods') . " where  {$condition}  GROUP BY id ORDER BY rand() LIMIT " . $pagesize;
            $total  = $pagesize;
        }
        ...
        foreach($list as $lk => $lv){
            ...
            if($lv['hasoption']==1) {
                $pricemax = array();
                $options = pdo_fetchall('select * from ' . tablename('vending_machine_goods_option') . ' where goodsid=:goodsid and                               uniacid=:uniacid order by displayorder asc', array(':goodsid' => $lv['id'], ':uniacid' => $_W['uniacid']));
...
//                    秒杀
            ...
            if($lv['type'] == 3){
                $vsql = 'SELECT * FROM '.tablename('vending_machine_virtual_type').' WHERE uniacid = '.intval($_W['uniacid']).' AND id = '.intval($lv['virtual']);
        ...
        g.minprice = (select min(marketprice) from ".tablename('vending_machine_goods_option')." where goodsid = ".$lv['id']."),
        g.maxprice = (select max(marketprice) from ".tablename('vending_machine_goods_option')." where goodsid = ".$lv['id'].")
        where g.id = ".$lv['id']." and g.hasoption=1";
                    pdo_query($sql);
                } else {
                    $sql = "update ".tablename('vending_machine_goods')." set minprice = marketprice,maxprice = marketprice where id = ".$lv['id']." and hasoption=0;";
                    pdo_query($sql);
                }
                $goods_price = pdo_fetch("select minprice,maxprice from " . tablename('vending_machine_goods') . " where id=:id and uniacid=:uniacid limit 1", array(':id' => $lv['id'], ':uniacid' => $_W['uniacid']));
...
//                $pricemax = array();
//                $options = pdo_fetchall('select * from ' . tablename('vending_machine_goods_option') . ' where goodsid=:goodsid and                               uniacid=:uniacid order by displayorder asc', array(':goodsid' => $value['id'], ':uniacid' => $_W['uniacid']));
//                foreach($options as $k=>$v){
//                    array_push($pricemax, $v['marketprice']);
//                }
...
//        }
    ...
    public function getCommission($goods,$level,$set)
    {
        ...
        if (!empty($goods) && $goods['hasoption']) {
            $option = pdo_fetchall('select * from ' . tablename('vending_machine_goods_option') . ' where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc', array(':goodsid' => $goods['id'], ':uniacid' => $_W['uniacid']));
        }
    ...
    public function getListbyCoupon($args = array()) {
        ...
        $table =tablename('vending_machine_goods').' g';
        if($couponid>0)
        {
            $data = pdo_fetch('select c.*  from ' . tablename('vending_machine_coupon_data') . '  cd inner join  ' . tablename('vending_machine_coupon') . ' c on cd.couponid = c.id  where cd.id=:id and cd.uniacid=:uniacid and coupontype =0  limit 1', array(':id' => $couponid, ':uniacid' => $_W['uniacid']));
    ...
    public function getTotals() {
        global $_W;
        return array(
            "sale" => pdo_fetchcolumn('select count(1) from ' . tablename('vending_machine_goods') . " where status > 0 and checked=0 and deleted=0 and total>0 and  type !=30 and uniacid=:uniacid", array(':uniacid' => $_W['uniacid'])),
            "out" => pdo_fetchcolumn('select count(1) from ' . tablename('vending_machine_goods') . " where status > 0 and deleted=0 and total=0 and type !=30 and uniacid=:uniacid", array(':uniacid' => $_W['uniacid'])),
            "stock" => pdo_fetchcolumn('select count(1) from ' . tablename('vending_machine_goods') . " where (status=0 or checked=1) and deleted=0 and uniacid=:uniacid and type !=30", array(':uniacid' => $_W['uniacid'])),
            "cycle" => pdo_fetchcolumn('select count(1) from ' . tablename('vending_machine_goods') . " where deleted=1 and uniacid=:uniacid and type !=30", array(':uniacid' => $_W['uniacid'])),
            "verify" => pdo_fetchcolumn('select count(1) from ' . tablename('vending_machine_goods') . " where deleted=0  and uniacid=:uniacid and type !=30 and merchid>0 and checked=1", array(':uniacid' => $_W['uniacid'])),
        );
    }
    ...
    public function getComments($goodsid = '0', $args = array()) {
        ...
        $sql = "SELECT id,nickname,headimgurl,content,images FROM " . tablename('vending_machine_goods_comment') . " where 1 {$condition} ORDER BY createtime desc LIMIT " . ($page - 1) * $pagesize . ',' . $pagesize;
        $list = pdo_fetchall($sql, $params);
        foreach ($list as &$row) {
    ...
    public function isFavorite($id = '') {
        ...
        $count = pdo_fetchcolumn('select count(*) from ' . tablename('vending_machine_member_favorite') . " where goodsid=:goodsid and deleted=0 and openid=:openid and uniacid=:uniacid limit 1", array(':goodsid' => $id, ':openid' => $_W['openid'], ':uniacid' => $_W['uniacid']));
        return $count > 0;
    }
    ...
    public function addHistory($goodsid = 0) {
        ...
        //浏览总数
        pdo_query("update " . tablename('vending_machine_goods') . " set viewcount=viewcount+1 where id=:id and uniacid='{$_W[uniacid]}' ", array(":id" => $goodsid));

        //浏览记录
        $history = pdo_fetch('select id,times from ' . tablename('vending_machine_member_history') . ' where goodsid=:goodsid and uniacid=:uniacid and openid=:openid limit 1'
        ...
        if (empty($history)) {
            ...
            pdo_insert('vending_machine_member_history', $history);
        } else {
            pdo_update('vending_machine_member_history', array('deleted' => 0, 'times' => $history['times'] + 1), array('id' => $history['id']));
        }
    }
...
//        如果是O2O的购物车
        ...
        $count = pdo_fetchcolumn('select sum(total) from ' . tablename('vending_machine_member_cart') . " where uniacid=:uniacid and openid=:openid ".$sqlcondition." and deleted=0 limit 1", $paras);
        return $count;
    }
    ...
    public function getSpecThumb($specs) {
        ...
        if (!empty($specid)) {
            $spec = pdo_fetch("select thumb from " . tablename('vending_machine_goods_spec_item') . " "
    ...
    public function getOptions($goods)
    {
        ...
        if (!empty($goods) && $goods['hasoption']) {
            ...
            foreach($specs as &$spec) {
                $spec['items'] = pdo_fetchall('select * from '.tablename('vending_machine_goods_spec_item')." where specid=:specid order by displayorder asc",array(':specid'=>$spec['id']));
            }
            unset($spec);
            $options = pdo_fetchall('select * from ' . tablename('vending_machine_goods_option') . ' where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc', array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));
        }
...
//            if((!empty($showlevels)&&in_array($member['level'], $showlevels)) || (!empty($showgroups) && in_array($member['groupid'], $showgroups)) || (empty($showlevels) && empty($showgroups))){
    ...
    public function canBuyAgain($goods)
    {
        ...
        $order_goods = pdo_fetchall("SELECT id,orderid FROM ".tablename('vending_machine_order_goods')." WHERE uniacid=:uniaicd AND openid=:openid AND goodsid=:goodsid {$condition}",array(':uniaicd'=>$_W['uniacid'],':openid'=>$_W['openid'],':goodsid'=>$id),'orderid');

        if (empty($order_goods)){
            return false;
        }
        $order = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename('vending_machine_order')." WHERE uniacid=:uniaicd AND status>=:status AND id IN (".implode(',',array_keys($order_goods)).")",array(':uniaicd'=>$_W['uniacid'],':status'=>(empty($goods['buyagain_condition'])?'1':'3')));
        return !empty($order);
    }
    ...
    {
        global $_W;
        $order_goods = pdo_fetchall("SELECT id,goodsid FROM ".tablename('vending_machine_order_goods')." WHERE uniacid=:uniaicd AND openid=:openid AND canbuyagain = 1 AND orderid <> :orderid",array(':uniaicd'=>$_W['uniacid'],':openid'=>$_W['openid'],'orderid'=>$orderid),'goodsid');
        if (empty($order_goods)){
            return false;
        }
        pdo_query('UPDATE '.tablename('vending_machine_order_goods')." SET `canbuyagain`='0' WHERE uniacid=:uniacid AND goodsid IN (".implode(',',array_keys($order_goods)).")",array(':uniacid'=>$_W['uniacid']));
    }
    ...
    public function createcode($parameter) {
        global $_W;
        $path = IA_ROOT . "/addons/vending_machine/data/goodscode/" . $_W['uniacid'] . "/";
        ...
        $img = $_W['siteroot'] . "addons/vending_machine/data/goodscode/" . $_W['uniacid'] . "/" . $file;
...
//        if (!strexists($imgurl,str_replace(array('https://','http://'),'',$_W['siteroot']))){
    ...
    public function mergeText($target, $data, $text,$center=false) {
        $font = IA_ROOT . "/addons/vending_machine/static/fonts/msyh.ttf";
