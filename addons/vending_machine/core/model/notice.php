...
class Notice_Model
{
    ...
    public function sendMerchSuccess($id)
    {
        ...
        $merch = pdo_get('vending_machine_merch_reg', ['id' => $id, 'uniacid' => $_W['uniacid']]);
        $template = pdo_get('vending_machine_member_message_template_default', ['typecode' => 'march_type_success']);
...
//        m('message')->sendTplnotice($merch['openid'], $template['templateid'], $msg,'',$account);
    ...
    public function sendMerchFail($id)
    {
        ...
        $template = pdo_get('vending_machine_member_message_template_default', ['typecode' => 'march_type_fail']);
        $this->sendNotice(array(
            "openid" => $merch['openid'],
            'tag' => 'march_type_fail',
            'default' => $msg,
...
//        m('message')->sendTplnotice($merch['openid'], $template['templateid'], $msg,'',$account);
    ...
    public function sendTeamMessage($orderid = '0', $delRefund = false)
    {
        ...
        $order = pdo_fetch('select * from ' . tablename('vending_machine_groups_order') . ' where id=:id limit 1', array(':id' => $orderid));
        if (empty($order)) {
            return;
        }
        $openid = $order['openid'];
        ...
        $order_goods = pdo_fetch('select * from ' . tablename('vending_machine_groups_goods') . ' where uniacid=:uniacid and id=:id ', array(':uniacid' => $_W['uniacid'], ':id' => intval($order['goodid'])));
        ...
        if ($delRefund == true) {
            ...
            if ($order['status'] == 1) {
                ...
                if ($order['success'] == 1) {
                    //拼团成功通知
                    $order = pdo_fetchall('select * from ' . tablename('vending_machine_groups_order') . ' where teamid = :teamid and success = 1 and status = 1 ', array(':teamid' => $order['teamid']));
    ...
    public function sendGroupsNotice(array $params)
    {
        ...
        if ($tm['is_advanced']) {
            ...
            //高级模式
            if (!empty($templateid)) {
                $advanced_template = pdo_fetch('select * from ' . tablename('vending_machine_member_message_template') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $templateid, ':uniacid' => $_W['uniacid']));
    ...
    public function sendOrderWillCancelMessage($orderid = '0', $closedaytimes)
    {
        ...
        $order = pdo_fetch('select * from ' . tablename('vending_machine_order') . ' where id=:id limit 1', array(':id' => $orderid));
        if (empty($order)) {
            return;
        }
        $openid = $order['openid'];
        ...
        $order_goods = pdo_fetchall("select g.id,g.title,og.realprice,og.total,og.price,og.optionname as optiontitle,g.noticeopenid,g.noticetype from " . tablename('vending_machine_order_goods') . " og "
        ...
        if (!empty($order['storeid'])) {
            if ($order['merchid'] > 0) {
                $store = pdo_fetch('select * from ' . tablename('vending_machine_merch_store') . " where id=:id and uniacid=:uniacid and merchid = :merchid limit 1", array(':id' => $order['storeid'], ':uniacid' => $_W['uniacid'], ':merchid' => $order['merchid']));
            } else {
                $store = pdo_fetch('select * from ' . tablename('vending_machine_store') . " where id=:id and uniacid=:uniacid limit 1", array(':id' => $order['storeid'], ':uniacid' => $_W['uniacid']));
            }
        }
        ...
        if (!empty($order['address'])) {
            ...
            if ((is_array($address) && empty($address)) || !is_array($address)) {
                ...
                if ((is_array($address) && empty($address)) || !is_array($address)) {
                    $address = pdo_fetch('select id,realname,mobile,address,province,city,area from ' . tablename('vending_machine_member_address') . ' where id=:id and uniacid=:uniacid limit 1'
        ...
        if ($order['status'] == 0) {
            ...
            pdo_update('vending_machine_order', array("willcancelmessage" => 1), array('id' => $orderid));
    ...
    public function sendOrderMessage($orderid = '0', $delRefund = false, $raid = null)
    {
        ...
        $order_goods = pdo_fetchall("select g.id,g.title,og.realprice,og.total,og.price,og.optionname as optiontitle,g.noticeopenid,g.noticetype,og.sendtype,og.expresscom,og.expresssn,og.sendtime from " . tablename('vending_machine_order_goods') . " og "
            . " left join " . tablename('vending_machine_goods') . " g on g.id=og.goodsid "
            . " where $scondition and og.uniacid=:uniacid ", $param);
        $goods = '';
        $goodsname = '';
        ...
        if ($delRefund) {
            ...
            if (empty($order['refundid']) && $order['refundstate'] == 3){
                //如果是单品维权 TODO
                $order['single_refund'] =  pdo_fetch('select id from ' . tablename('vending_machine_order_single_refund') . ' where orderid=:id limit 1', array(':id' => $order[id]));
            }
            ...
            if (!empty($order['refundid']) || !empty($order['single_refund'])) {
                ...
                $refund = pdo_fetch('select * from ' . tablename('vending_machine_order_refund') . ' where id=:id limit 1', array(':id' => $order['refundid']));

                //如果维权表查找不到,去单品维权表查找
                if (empty($refund)) {
                    $refund = pdo_fetch('select * from ' . tablename('vending_machine_order_single_refund') . ' where id=:id limit 1', array(':id' => $order['single_refund']['id']));
                }
                if (empty($refund)) {
                    return;
                }
                ...
                if ($refund['status'] == 0) {
                    ...
                    if ($refund['rtype'] == 2 || $refund['rtype'] == 1) {
                        ...
                        if ($order['merchid'] > 0) {
                            ...
                            if (empty($raid)) {
                                //卖家收货地址
                                $salerefund = pdo_fetch('select * from ' . tablename('vending_machine_refund_address') . ' where uniacid=:uniacid and isdefault=1 limit 1', array(':uniacid' => $_W['uniacid']));
                            } else {
                                //卖家收货地址
                                $salerefund = pdo_fetch('select * from ' . tablename('vending_machine_refund_address') . ' where uniacid=:uniacid and id=' . $raid . ' limit 1', array(':uniacid' => $_W['uniacid']));
                            }
...
//						));
            ...
            $item = pdo_fetch("select og.trade_time,g.title,s.storename,s.mobile,p.nickname,s.id as storeid  from " . tablename('vending_machine_order_goods') . "  og
				 ...
				 left join " . tablename('vending_machine_store') . " s  on  o.storeid = s.id
				 left join " . tablename('vending_machine_newstore_people') . " p  on  og.peopleid = p.id
            ...
            if (empty($notice['o2o_snorder_close_advanced'])) {
                ...
                $salers = pdo_fetchall('select *   from ' . tablename('vending_machine_saler') . " sr  inner join " . tablename("vending_machine_store") . " s on s.id = sr.storeid  where  s.id=:id and sr.getnotice =1", array(":id" => $item['storeid']));
...
//                        'uniacid'=>$_W['uniacid'],
//                    );
//                    pdo_insert('vending_machine_virtual_send_log', $insert);
...
//{{first.DATA}}业务类型：{{keyword1.DATA}}处理状态：{{keyword2.DATA}}处理内容：{{keyword3.DATA}}{{remark.DATA}}
    ...
    public function sendMemberLogMessage($log_id = '', $channel = 0, $isback = false)
    {
        ...
        $log_info = pdo_fetch('select * from ' . tablename('vending_machine_member_log') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $log_id, ':uniacid' => $_W['uniacid']));
    ...
    public function sendStockWarnMessage($goodsid, $optionid)
    {
        ...
        $goods = pdo_fetch("select  * from " . tablename('vending_machine_goods') . " where id=:id and uniacid=:uniacid", array(":id" => $goodsid, ":uniacid" => $_W['uniacid']));
        ...
        if (!empty($optionid)) {
            $option = pdo_fetch("select  *  from " . tablename('vending_machine_goods_option') . " where id=:id and uniacid=:uniacid and goodsid = :goodsid", array(':uniacid' => $_W['uniacid'], ':goodsid' => $goodsid, ':id' => $optionid));
...
//					));
    ...
    public function sendNotice(array $params)
    {
        ...
        if (!empty($templateid)) {
            $template = pdo_fetch('select * from ' . tablename('vending_machine_member_message_template') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $templateid, ':uniacid' => $_W['uniacid']));
            if (!empty($template)) {
                ...
                if (is_error($ret)) {
                    //默认模板消息
                    $templatetype = pdo_fetch('select templateid  from ' . tablename('vending_machine_member_message_template_default') . ' where typecode=:typecode and uniacid=:uniacid  limit 1', array(':typecode' => $tag, ':uniacid' => $_W['uniacid']));
            ...
            pdo_insert('vending_message_mass_sign', $insert);*/
        }
        return $ret;
    }
    ...
    function settemplateidbyback($tag, $uniacid = 0)
    {
        ...
        //默认模板消息
        $templatetype = pdo_fetch('select `name`,templatecode,content  from ' . tablename('vending_machine_member_message_template_type') . ' where typecode=:typecode  limit 1', array(':typecode' => $tag));
        ...
        foreach ($result['template_list'] as $key => $value) {
            ...
            //pdo_update('vending_machine_member_message_template_type', array('content'=>$value['content']), array('typecode' =>$tag));
            ...
            if (str_replace("\n", "", $value['content']) == $content) {
                ...
                //默认模板消息
                $defaulttemp = pdo_fetch('select 1  from ' . tablename('vending_machine_member_message_template_default') . ' where typecode=:typecode and uniacid=:uniacid  limit 1',
                    array(':typecode' => $tag, ':uniacid' => $uniacid));
                if (empty($defaulttemp)) {
                    pdo_insert('vending_machine_member_message_template_default',
                        array('typecode' => $tag, 'uniacid' => $uniacid, 'templateid' => $value['template_id']));
                } else {
                    pdo_update('vending_machine_member_message_template_default', array('templateid' => $value['template_id']),
                        array('typecode' => $tag, 'uniacid' => $uniacid));
                }
    ...
    public function sendVerifygoodMessage($verifygoodlogid)
    {
        ...
        $item = pdo_fetch('select vgl.*,vg.limitnum,s.storename,sa.salername,sa.openid as saleropenid,o.openid,g.title,o.ordersn  from ' . tablename('vending_machine_verifygoods_log') . '   vgl
		...
		inner  join ' . tablename('vending_machine_verifygoods') . ' vg on vg.id = vgl.verifygoodsid
		inner  join ' . tablename('vending_machine_order_goods') . ' og on vg.ordergoodsid = og.id
		inner  join ' . tablename('vending_machine_order') . ' o on o.id = og.orderid
		left  join ' . tablename('vending_machine_goods') . ' g on og.goodsid = g.id
	  where  vgl.id =:id  and vgl.uniacid=:uniacid  limit 1', array(':id' => $verifygoodlogid, ':uniacid' => $_W['uniacid']));


        $verifygoodlogs = pdo_fetchall('select *  from ' . tablename('vending_machine_verifygoods_log') . '    where verifygoodsid =:id  ', array(':id' => $item['verifygoodsid']));
        $verifynum = 0;
