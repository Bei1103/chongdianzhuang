...
class Bind_Model {
    ...
    public function update($mid=0, $arr=array()){
        ...
        pdo_update('vending_machine_member', $arr, array('id'=>$mid,'uniacid'=>$_W['uniacid']));
    }
    ...
    public function merge($a=array(), $b=array()){
        ...
        if(!empty($arr) && is_array($arr)){
            pdo_update('vending_machine_member', $arr, array('id'=>$b['id']));
        }

        // 2. 分销信息
        pdo_update('vending_machine_commission_apply', array('mid' => $b['id']), array('uniacid' => $_W['uniacid'], 'mid' => $a['id']));
        //订单上级
        pdo_update('vending_machine_order',array('agentid'=>$b['id']),array('agentid'=>$a['id']));
        //会员上级
        pdo_update('vending_machine_member',array('agentid'=>$b['id']),array('agentid'=>$a['id']));
        ...
        // 删除用户A
        pdo_delete('vending_machine_member', array('id' => $a['id'], 'uniacid' => $_W['uniacid']));
        if(method_exists(m('member'),'memberRadisCountDelete')) {
            m('member')->memberRadisCountDelete();  //清除会员统计radis缓存
        }
        // 0. 替换所有数据表里的openid
        $tables = pdo_fetchall("SHOW TABLES like '%_vending_machine_%'");
        ...
        // 插入合并日志
        pdo_insert("vending_machine_member_mergelog", array(
            'uniacid'=>$_W['uniacid'],
            'mergetime'=>time(),
            'openid_a'=>$a['openid'],
            'openid_b'=>$b['openid'],
    ...
    public function mergeforuniacid($a=array(), $b=array()){
        ...
        // 2. 分销信息
        pdo_update('vending_machine_commission_apply', array('mid' => $b['id']), array('mid' => $a['id']));
        //订单上级
        pdo_update('vending_machine_order',array('agentid'=>$b['id']),array('agentid'=>$a['id']));
        //会员上级
        pdo_update('vending_machine_member',array('agentid'=>$b['id']),array('agentid'=>$a['id']));
        ...
        //pdo_update('vending_machine_member',array('credit1'=>0,'credit2'=>0), array('openid' => $a['openid']));
        ...
        foreach ($tables as $k => $v) {
            ...
            // 更新表中 含有 openid、uniacid的表
            if (pdo_fieldexists($tablename, 'openid') && pdo_fieldexists($tablename, 'uniacid')) {
                if($tablename !='vending_machine_member')
                {
                    pdo_update($tablename, array('openid' => $b['openid']), array('uniacid' => $_W['uniacid'], 'openid' => $a['openid']));
                }
            }
