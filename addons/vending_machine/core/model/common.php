...
class Common_Model {
    ...
    public function getSetData($uniacid = 0) {
        ...
        if (empty($set)) {
            $set = pdo_fetch("select * from " . tablename('vending_machine_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));
            if (empty($set)) {
                $set = array();
            }
    ...
    public function updateSysset($values, $uniacid = 0) {
        ...
        $setdata = pdo_fetch("select * from " . tablename('vending_machine_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));

        if (empty($setdata)) {
            $res = pdo_insert('vending_machine_sysset', array('sets' => iserializer($values), 'uniacid' => $uniacid));
            ...
            $res = pdo_update('vending_machine_sysset', array('sets' => iserializer($sets)), array('id' => $setdata['id']));
            if ($res) {
                $setdata['sets'] = $sets;
            }
        }
    ...
    public function deleteSysset($key, $uniacid = 0) {
        ...
        $setdata = pdo_fetch("select id, sets from " . tablename('vending_machine_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));
        if (!empty($setdata)) {
            $sets = iunserializer($setdata['sets']);
            $sets = is_array($sets) ? $sets : array();
            ...
            pdo_update('vending_machine_sysset', array('sets' => iserializer($sets)), array('id' => $setdata['id']));
        }
    ...
    public function updatePluginset($values, $uniacid = 0) {
        ...
        $setdata = pdo_fetch("select * from " . tablename('vending_machine_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));
        if (empty($setdata)) {
            $res = pdo_insert('vending_machine_sysset', array('plugins' => iserializer($values), 'uniacid' => $uniacid));
            ...
            $res = pdo_update('vending_machine_sysset', array('plugins' => iserializer($plugins)), array('id' => $setdata['id']));
            if ($res) {
    ...
    public function alipay_build($params, $alipay = array(), $type = 0, $openid = '') {
        ...
        if($pay['alipay_id']>0){
            ...
            //$config['notify_url'] = $_W['siteroot'] . "addons/vending_machine/payment/alipay/notify.php";
            if (empty($type)) {
                //购物
                $config['notify_url'] = $_W['siteroot'] . "addons/vending_machine/payment/alipay/notify.php";
            ...
            if (empty($type)) {
                //购物
                $set['notify_url'] = $_W['siteroot'] . "addons/vending_machine/payment/alipay/notify.php";
    ...
    public function publicAliPay($params = array(),$return = null)
    {
        ...
        $url = VENDING_MACHINE_DEBUG ? 'https://openapi.alipaydev.com/gateway.do' : 'https://openapi.alipay.com/gateway.do';
        if ($return !== null){
            return $public;
        }
...
//        }
    ...
    function public_build($isapp=false)
    {
        ...
        if (!empty($set['weixin_id']) && $isapp==false){
            $payments = pdo_fetch("SELECT * FROM ".tablename('vending_machine_payment')." WHERE uniacid=:uniacid AND id=:id",array(':uniacid'=>$_W['uniacid'],':id'=>$set['weixin_id']));
            if (empty($payments)){
                error(-1,'支付参数不存在!');
            }
            $payments['is_new'] = 1;
    ...
    function wechat_build($params, $wechat, $type = 0) {
        ...
        if ($payment['is_new'] == 1) {
            ...
            }elseif ($payment['type'] == 1||$payment['type']==5&&!empty($payment['appid'])&&!empty($payment['mch_id'])){
                $params['openid'] = isset($params['user']) ? trim($params['user']) : trim($_W['openid']);
    ...
    function wechat_jspay($params,$wechat,$type=0)
    {
        ...
        $package['device_info'] = "vending_machine";
        ...
        $package['notify_url'] = $_W['siteroot'] . "addons/vending_machine/payment/wechat/notify.php";
    ...
    function wechat_child_build($params, $wechat, $type = 0) {
        ...
        $package['nonce_str'] = random(32);
        $package['body'] = trim($params['title']);
        $package['device_info'] = isset($params['device_info'])?"vending_machine:".$params['device_info']:"vending_machine";
        ...
        $package['notify_url'] = empty($params['notify_url']) ? $_W['siteroot'] . "addons/vending_machine/payment/wechat/notify.php" : $params['notify_url'];
        $package['trade_type'] = 'JSAPI';
        ...
        * */
        if(strpos($params['openid'],'sns_wa_')!==false){
            ...
            if (!empty($set['weixin_id'])){
                $payment = pdo_fetch("SELECT * FROM ".tablename('vending_machine_payment')." WHERE uniacid=:uniacid AND id=:id",array(':uniacid'=>$_W['uniacid'],':id'=>$set['weixin_id']));
                if (empty($payment)){
                    error(-1,'支付参数不存在!');
                }
                $payment['is_new'] = 1;
            }else{
                $payment = pdo_fetch("SELECT * FROM ".tablename('vending_machine_payment')." WHERE uniacid=:uniacid AND `type`='0'",array(':uniacid'=>$_W['uniacid']));
            }
            if (empty($payment)) {
                $payment = array();
        ...
        $package['act_name'] = mb_strcut($params['act_name'],0,32); //活动名称
        ...
        if (is_array($certs)) {
            ...
            $certfile = IA_ROOT . "/addons/vending_machine/cert/" . random(128);
            file_put_contents($certfile, $certs['cert']);
            $keyfile = IA_ROOT . "/addons/vending_machine/cert/" . random(128);
            ...
            if(!empty($certs['root']))
            {
                $rootfile = IA_ROOT . "/addons/vending_machine/cert/" . random(128);
...
//  }
    ...
    public function createNO($table, $field, $prefix) {
        ...
        while (1) {
            $count = pdo_fetchcolumn('select count(*) from ' . tablename('vending_machine_' . $table) . " where {$field}=:billno limit 1", array(':billno' => $billno));
            if ($count <= 0) {
                break;
            }
            $billno = date('YmdHis') . random(6, true);
    ...
    public function getSec($uniacid = 0) {
        ...
        $set = pdo_fetch("select sec from " . tablename('vending_machine_sysset') . ' where uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));
        if (empty($set)) {
            $set = array();
        }
        //是否启用新版支付宝
        ...
        if($data['alipay_id']>0){
            $paymentalis = pdo_fetch("SELECT alipay_sec FROM ".tablename('vending_machine_payment')." WHERE id = :id and uniacid=:uniacid and paytype = 1 ",array(':uniacid'=>$_W['uniacid'],':id'=>$data['alipay_id']));
    ...
    public function paylog($log = '') {
        ...
        if (!empty($openpaylog)) {
            $path = IA_ROOT . "/addons/vending_machine/data/paylog/" . $_W['uniacid'] . "/" . date('Ymd');
            if (!is_dir($path)) {
                load()->func('file');
                @mkdirs($path, '0777');
            }
    ...
    public function getAreas()
    {
        ...
        if (!empty($new_area)) {
            $file = IA_ROOT . "/addons/vending_machine/static/js/dist/area/AreaNew.xml";
        } else {
            $file = IA_ROOT . "/addons/vending_machine/static/js/dist/area/Area.xml";
        }
    ...
    public function getWechats() {
        return pdo_fetchall("SELECT  a.uniacid,a.name FROM " . tablename('vending_machine_sysset') . " s  "
    ...
    public function getCopyright($ismanage = false) {
        ...
        if(!empty($merchid)){
            $merch = pdo_fetch("select logo from ".tablename('vending_machine_merch_user')." where id = :id and uniacid = :uniacid ",array(":id"=>$merchid,":uniacid"=>$_W['uniacid']));
            if(!empty($merch['logo'])) {
