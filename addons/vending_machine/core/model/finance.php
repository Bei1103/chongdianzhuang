        ...
        if (empty($paytype)) {
            ...
            if(strpos($openid,'sns_wa_')!==false){
                ...
                if (!empty($set['weixin_id'])){
                    ...
                    $payment = pdo_fetch("SELECT * FROM ".tablename('vending_machine_payment')." WHERE uniacid=:uniacid AND  type=0  AND paytype=0 AND sub_appid=:appid",array(':uniacid'=>$_W['uniacid'],':appid'=>$_W['account']['key']));
                }
                ...
                if (empty($payment)) {
                    ...
                    $certs = $sec;
                }elseif($payment['type']==5&&!empty($payment['appid'])&&!empty($payment['mch_id'])){
                    $certs = array(
                        'cert' => $payment['sub_cert_file'],
                        'key' => $payment['sub_key_file'],
                    );
                }else{
                    $certs = array(
...
//        }
        ...
        if ($payment['is_new'] == 1 && !$app){
            ...
            switch ($payment['type'])
            {
                ...
                case '4':
                    ...
                    break;
                case '5'://支付分
                    if(!empty($payment['appid'])&&!empty($payment['mch_id'])) {
                        $wechat = $sub_wechat;
                    }
            }
            unset($sub_wechat);
    ...
    public function wxapp_refund($openid, $out_trade_no, $out_refund_no, $totalmoney, $refundmoney=0, $app=false,$refund_account = false)
    {
        ...
        if (is_array($certs)) {
            ...
            $certfile = IA_ROOT . "/addons/vending_machine/cert/" . random(64);
            file_put_contents($certfile, $certs['cert']);
            $keyfile = IA_ROOT . "/addons/vending_machine/cert/" . random(64);
            file_put_contents($keyfile, $certs['key']);
            ...
            if(! empty($certs['root']))
            {
                $rootfile = IA_ROOT . "/addons/vending_machine/cert/" . random(64);
                file_put_contents($rootfile, $certs['root']);
                $extras['CURLOPT_CAINFO'] = $rootfile;
            }
        } else {
    ...
    public function AlipayRefund($params,$batch_no,$config)
    {
        ...
        $parameter = array(
            ...
            "notify_url"	=> isset($params['notify_url']) ? $params['notify_url'] : $_W['siteroot'] . "addons/vending_machine/payment/alipay/notify.php",
        ...
        foreach ($dates as $date) {
            ...
            if ($datatype) {
                ...
                foreach ($dc_arr as $key => $value) {
                    if ($key != 0 && $key < $len-3) {
                        if (!strstr($value, 'vending_machine')) {
    ...
    private function downloadday($date, $wechat, $type)
    {
        ...
        $pars['nonce_str'] = random(8);
        $pars['device_info'] = "vending_machine";
        $pars['bill_date'] = $date;
        $pars['bill_type'] = $type;
        ksort($pars, SORT_STRING);
        $string1 = '';
