...
//            $address_area = $address['area'];
    ...
    function getDefaultDispatch($merchid = 0) {
        ...
        $sql = 'select * from ' . tablename('vending_machine_dispatch') . ' where isdefault=1 and uniacid=:uniacid and merchid=:merchid and enabled=1 Limit 1';
    ...
    function getNewDispatch($merchid = 0) {
        ...
        $sql = 'select * from ' . tablename('vending_machine_dispatch') . ' where uniacid=:uniacid and merchid=:merchid and enabled=1 order by id desc Limit 1';
        $params = array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid);
        $data = pdo_fetch($sql, $params);
    ...
    function getOneDispatch($id) {
        ...
        if ($id==0)
        {
            $sql = 'select * from ' . tablename('vending_machine_dispatch') . ' where isdefault=1 and uniacid=:uniacid and enabled=1 Limit 1';
        }
        else
        {
            $sql = 'select * from ' . tablename('vending_machine_dispatch') . ' where id=:id and uniacid=:uniacid and enabled=1 Limit 1';
            $params[':id'] = $id;
        }
        $data = pdo_fetch($sql, $params);
        return $data;
