...
class Verifygoods_Model {
    ...
    public function createverifygoods($orderid) {
        ...
        $verifygoods = pdo_fetchall('select  *   from  ' . tablename('vending_machine_verifygoods') . ' where  orderid=:orderid ', array(':orderid' => $orderid));
...
//                        }
//                    }
                    pdo_insert("vending_machine_verifygoods",$data);
                }
            }
...
//            }
    ...
    public function getCanUseVerifygoods($openid)
    {
        global $_W;
        $time = time();
        $sql = 'select vg.*,g.title,g.subtitle,g.thumb,c.card_id  from ' . tablename('vending_machine_verifygoods') . '   vg
                 inner join ' . tablename('vending_machine_order_goods') . ' og on vg.ordergoodsid = og.id
                 left  join ' . tablename('vending_machine_order') . ' o on vg.orderid = o.id
                 left  join ' . tablename('vending_machine_order_refund') . ' orf on o.refundid = orf.id
                 inner join ' . tablename('vending_machine_goods') . ' g on og.goodsid = g.id
                 left  join ' . tablename('vending_machine_goods_cards') . ' c on c.id = g.cardid
    ...
    public function search($verifycode ){
        ...
        global $_W,$_GPC;
        $verifygood = pdo_fetch('select *  from ' . tablename('vending_machine_verifygoods') . " where uniacid=:uniacid and  verifycode=:verifycode and invalid =0  limit 1 ", array(':uniacid' => $_W['uniacid'], ':verifycode' => $verifycode));
        ...
        $saler = pdo_fetch('select * from ' . tablename('vending_machine_saler') . ' where  status=1  and  openid=:openid and uniacid=:uniacid limit 1', array(
    ...
    public function allow($verifycode, $times){
        ...
        $order = pdo_fetch('select * from ' . tablename('vending_machine_order') . ' where id=:id and uniacid=:uniacid  limit 1', array(':id' => $verifygood['orderid'], ':uniacid' => $verifygood['uniacid']));
        if (empty($order)) {
        ...
        // 核销商品详情
        $verifygoods_detail = pdo_fetch('select vg.*,g.id as goodsid ,g.title,g.subtitle,g.thumb,vg.storeid  from ' . tablename('vending_machine_verifygoods') . '   vg
		 inner join ' . tablename('vending_machine_order_goods') . ' og on vg.ordergoodsid = og.id
		 inner join ' . tablename('vending_machine_goods') . ' g on og.goodsid = g.id
    ...
    public function complete($verifycode, $times, $remarks){
        ...
        pdo_insert("vending_machine_verifygoods_log",$data);

        $logid = pdo_insertid();

        m('notice')->sendVerifygoodMessage($logid);

        pdo_query("update ".tablename("vending_machine_verifygoods")." set used=:used , verifycode=null ,codeinvalidtime=0 where id=:id",array(":id"=>$verifygood['id'],":used"=>$used));
        ...
        if($isonlyverifygood)
        {
            $status = pdo_fetchcolumn('select status  from ' . tablename('vending_machine_order') . " where uniacid=:uniacid and id=:id  limit 1 ", array(':uniacid' => $_W['uniacid'], ':id' => $verifygood['orderid']));
            //TODO
            $verifytypexzx = pdo_fetchall('select verifytype  from ' . tablename('vending_machine_order') . " where uniacid=:uniacid and id=:id  limit 1 ", array(':uniacid' => $_W['uniacid'], ':id' => $verifygood['orderid']));
...
//
    ...
    {
        global $_W;
        $sql = 'select  COUNT(1)  from ' . tablename('vending_machine_verifygoods') . '   vg
                 inner join ' . tablename('vending_machine_order_goods') . ' og on vg.ordergoodsid = og.id
                 left  join ' . tablename('vending_machine_order') . ' o on vg.orderid = o.id
                 left  join ' . tablename('vending_machine_order_refund') . ' orf on o.refundid = orf.id
                 inner join ' . tablename('vending_machine_goods') . ' g on og.goodsid = g.id
