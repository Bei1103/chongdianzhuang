...
class Shop_Model {
	...
	public function getCategory($refresh = false) {
		...
		if (empty($allcategory) || $refresh) {
			$parents = array();
			$children = array();
			$category = pdo_fetchall("SELECT * FROM " . tablename('vending_machine_category') . " WHERE uniacid =:uniacid AND enabled=1 ORDER BY parentid ASC, displayorder DESC",array(':uniacid'=>$_W['uniacid']));
	...
	public function getFullCategory($fullname = false, $enabled = false) {
		...
		$allcategory = array();
		$sql = "SELECT * FROM " . tablename('vending_machine_category') . " WHERE uniacid=:uniacid ";
		if ($enabled) {
			$sql.=" AND enabled=1";
		}
	...
	public function getAllCategory($refresh = false) {
		...
		if (empty($allcategory) || $refresh) {
			$allcategory = pdo_fetchall("SELECT id,parentid,uniacid,name,thumb FROM " . tablename('vending_machine_category') . " WHERE uniacid = '{$_W['uniacid']}'",array(),'id');
