...
class Util_Model {
    ...
    function get_area_config_data($uniacid = 0){
        ...
        $sql = 'select * from '. tablename('vending_machine_area_config').' where uniacid=:uniacid limit 1';
    ...
    function location($lat, $lng){
        ...
        if (empty($key)) {
            $key = '6cd9844ce05d2707aa079cf7f0df43e2';
        }
    ...
    private function getExpressBird($express, $expresssn,$mobile='', $express_set) {
        ...
        if (!empty($express_set['express_bird_cache']) && $express_set['express_bird_cache']>0) {
            ...
            $cache = pdo_fetch("SELECT * FROM".tablename("vending_machine_express_cache")."WHERE express=:express AND expresssn=:expresssn LIMIT 1", array('express'=>$express, 'expresssn'=>$expresssn));
        ...
        $expressInfo =  pdo_fetch("SELECT * FROM".tablename("vending_machine_express")."WHERE express=:express LIMIT 1", array('express'=>$express));
...
//            }
        ...
        if($express_set['express_bird_cache']>0 && !empty($list)){
            // 更新缓存
            if(empty($cache)){
                pdo_insert("vending_machine_express_cache", array('expresssn'=>$expresssn, 'express'=>$express, 'lasttime'=>time(), 'datas'=>iserializer($list)));
            }else{
                pdo_update("vending_machine_express_cache", array('lasttime'=>time(), 'datas'=>iserializer($list)), array('id'=>$cache['id']));
            }
        }
