...
//        }
        ...
        if(p('merch')){
            ...
            $_sql = "select iscredit from".tablename('vending_machine_merch_user')."where 1 {$_condition}";
    ...
    public function init()
    {
        ...
        if(!$this->merch){
            $arr['notice'] = pdo_fetchall("SELECT * FROM " . tablename('vending_machine_system_copyright_notice') . " ORDER BY displayorder DESC,createtime DESC LIMIT 5" );
            if(p('commission')) {
                if ($this->cv('commission.apply.view1')) {
                    $arr['commission1'] = pdo_fetchcolumn("select count(1) from" . tablename('vending_machine_commission_apply') . " a "
                ...
                if ($this->cv('commission.apply.view2')) {
                    $arr['commission2'] = pdo_fetchcolumn("select count(1) from" . tablename('vending_machine_commission_apply') . " a "
                        . " left join " . tablename('vending_machine_member') . " m on m.uid = a.mid"
                        . " left join " . tablename('vending_machine_commission_level') . " l on l.id = m.agentlevel"
            ...
            if($this->cv('shop.comment')){
                $arr['comment'] = pdo_fetchcolumn("SELECT COUNT(1) FROM " . tablename('vending_machine_order_comment') . "c LEFT JOIN ". tablename('vending_machine_goods') ." g ON g.id=c.goodsid WHERE (c.checked=1 OR c.replychecked=1) AND c.deleted=0 AND c.uniacid=:uniacid AND g.merchid=:merchid", array(':uniacid'=>$_W['uniacid'], ':merchid'=>0));
            }
        }else{
            $arr['notice']='none';
            if($this->cv('shop.comment')){
                $arr['comment'] = pdo_fetchcolumn("SELECT COUNT(1) FROM " . tablename('vending_machine_order_comment') . "c LEFT JOIN ". tablename('vending_machine_goods') ." g ON g.id=c.goodsid WHERE (c.checked=1 OR c.replychecked=1) AND c.deleted=0 AND c.uniacid=:uniacid AND g.merchid=:merchid", array(':uniacid'=>$_W['uniacid'], ':merchid'=>$_W['merchid']));
            }
        }

        if(!empty($arr['funbar']['open'])){
            $funbardata = pdo_fetch('select * from '. tablename('vending_machine_funbar').' where uid=:uid and uniacid=:uniacid limit 1', array(':uid'=>$_W['uid'], ':uniacid'=>$_W['uniacid']));
    ...
    protected function getOrderTotal($status = 0)
    {
        ...
        $total = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('vending_machine_order') . " WHERE ". $condition, $params);
        return $total;
    }
