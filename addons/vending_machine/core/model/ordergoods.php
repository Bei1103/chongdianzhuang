    ...
    function setStock($goods)
    {
        ...
        if (!empty($goods['optionid'])) {
            ...
            if (!empty($option) && $option['stock'] != -1) {
                ...
                if (!empty($stock)){
                    pdo_update('vending_machine_goods_option', array('stock' => $stock), array('uniacid' => $_W['uniacid'], 'goodsid' => $goods['goodsid'], 'id' => $goods['optionid']));
                }
            }
        }
        ...
        if (!empty($totalstock)){
            pdo_update('vending_machine_goods', array('total' => $totalstock), array('uniacid' => $_W['uniacid'], 'id' => $goods['goodsid']));
        }
