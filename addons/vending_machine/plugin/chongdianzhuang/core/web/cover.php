...
class Cover_Page extends PluginWebPage {

    
    function main() {
        global $_W,$_GPC;
 
        $rule = pdo_fetch("select * from " . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => "vending_machine区域代理中心入口设置"));
        if (!empty($rule)) {
        ...
        if ($_W['ispost']) {
            ...
            if(!empty($keyword)){
                if($keyword['name']!='vending_machine区域代理中心入口设置'){
                    show_json(0, '关键字已存在!');
                }
            }
            if (!empty($rule)) {
            ...
            $rule_data = array(
                'uniacid' => $_W['uniacid'],
                'name' => 'vending_machine区域代理中心入口设置',
                'module' => 'cover',
                'displayorder' => 0,
            ...
            $cover_data = array(
                ...
                'url' => mobileUrl('chongdianzhuang')
            );
            pdo_insert('cover_reply', $cover_data);

            plog('chongdianzhuang.cover.edit', "修改区域代理中心入口设置");
			
            show_json(1);
        }
        $url = mobileUrl('chongdianzhuang',null,true);
