...
class Agent_Page extends PluginWebPage {
    ...
    function main() {
        ...
        if ($_GPC['export'] == '1') {
            ...
            plog('chongdianzhuang.agent.export', '导出区域代理商数据');
            foreach ($list as &$row) {
    ...
    function delete() {
        ...
        foreach ($members as $member) {
            ...
            plog('chongdianzhuang.agent.delete', "取消区域代理资格 <br/>区域代理信息:  ID: {$member['id']} /  {$member['openid']}/{$member['nickname']}/{$member['realname']}/{$member['mobile']}");
        }
    ...
    function check() {
        ...
        foreach ($members as $member) {
            ...
            if ($status == 1) {
                ...
                plog('chongdianzhuang.aagent.check', "审核代理 <br/>代理信息:  ID: {$member['id']} /  {$member['openid']}/{$member['nickname']}/{$member['realname']}/{$member['mobile']}");
                ...
                plog('chongdianzhuang.agent.check', "取消审核 <br/>代理信息:  ID: {$member['id']} /  {$member['openid']}/{$member['nickname']}/{$member['realname']}/{$member['mobile']}");
            }
        }
        show_json(1, array('url' => referer()));
    }
    ...
    function aagentblack() {
        ...
        foreach ($members as $member) {
            ...
            if ($aagentblack == 1) {
                ...
                plog('chongdianzhuang.agent.aagentblack', "设置黑名单 <br/>区域代理信息:  ID: {$member['id']} /  {$member['openid']}/{$member['nickname']}/{$member['realname']}/{$member['mobile']}");
            } else {
                pdo_update('vending_machine_member', array('isaagent' => 1, 'aagentstatus' => 1, 'aagentblack' => 0), array('id' => $member['id']));
                plog('chongdianzhuang.agent.aagentblack', "取消黑名单 <br/>区域代理信息:  ID: {$member['id']} /  {$member['openid']}/{$member['nickname']}/{$member['realname']}/{$member['mobile']}");
            }
        }
        show_json(1, array('url' => referer()));
    }
