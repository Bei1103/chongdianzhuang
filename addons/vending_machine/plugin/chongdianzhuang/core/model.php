...
define('TM_chongdianzhuang_PAY', 'TM_chongdianzhuang_PAY'); //分红打款
define('TM_chongdianzhuang_UPGRADE', 'TM_chongdianzhuang_UPGRADE'); //升级
define('TM_chongdianzhuang_BECOME', 'TM_chongdianzhuang_BECOME'); //成为代理商通知

if (!class_exists('chongdianzhuangModel')) {

    class chongdianzhuangModel extends PluginModel
    {
        ...
        function getLevels($all = true, $default = false)
        {
            global $_W;
            if ($all) {
                $levels = pdo_fetchall('select * from ' . tablename('vending_machine_chongdianzhuang_level') . ' where uniacid=:uniacid order by id asc', array(':uniacid' => $_W['uniacid']));
            } else {
                $levels = pdo_fetchall('select * from ' . tablename('vending_machine_chongdianzhuang_level') . ' where uniacid=:uniacid and (ordermoney>0 or bonusmoney>0) order by bonus asc', array(':uniacid' => $_W['uniacid']));
            }

            if ($default) {
                $default = array(
                    'id' => '0',
                    "levelname" => empty($_S['chongdianzhuang']['levelname']) ? '默认等级' : $_S['chongdianzhuang']['levelname'],
                    'bonus1' => $_W['shopset']['chongdianzhuang']['bonus1'],
                    'bonus2' => $_W['shopset']['chongdianzhuang']['bonus2'],
                    'bonus3' => $_W['shopset']['chongdianzhuang']['bonus3']
                );
        ...
        function getBonus($openid = '', $params = array())
        {
            ...
            if (in_array('ok', $params)) {
                $ret['ok'] = pdo_fetchcolumn('select ifnull(sum(paymoney1),0)+ifnull(sum(paymoney2),0)  + ifnull(sum(paymoney3),0)  from ' . tablename('vending_machine_chongdianzhuang_billp') . ' where openid=:openid and status=1 and uniacid=:uniacid limit 1',
                    array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }
            if (in_array('ok1', $params)) {
                $ret['ok1'] = pdo_fetchcolumn('select ifnull(sum(paymoney1),0)  from ' . tablename('vending_machine_chongdianzhuang_billp') . ' where openid=:openid and status=1 and uniacid=:uniacid limit 1',
                    array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }
            if (in_array('ok2', $params)) {
                $ret['ok2'] = pdo_fetchcolumn('select ifnull(sum(paymoney2),0)   from ' . tablename('vending_machine_chongdianzhuang_billp') . ' where openid=:openid and status=1 and uniacid=:uniacid limit 1',
                    array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }
            if (in_array('ok3', $params)) {
                $ret['ok3'] = pdo_fetchcolumn('select ifnull(sum(paymoney3),0)   from ' . tablename('vending_machine_chongdianzhuang_billp') . ' where openid=:openid and status=1 and uniacid=:uniacid limit 1',
                    array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }

            if (in_array('lock', $params)) {
                $ret['lock'] = pdo_fetchcolumn('select ifnull(sum(paymoney1),0)+ifnull(sum(paymoney2),0)  + ifnull(sum(paymoney3),0)  from ' . tablename('vending_machine_chongdianzhuang_billp') . ' where openid=:openid and status<>1 and uniacid=:uniacid limit 1',
                    array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }
            if (in_array('lock1', $params)) {
                $ret['lock1'] = pdo_fetchcolumn('select ifnull(sum(paymoney1),0)  from ' . tablename('vending_machine_chongdianzhuang_billp') . ' where openid=:openid and status<>1 and uniacid=:uniacid limit 1',
                    array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }
            if (in_array('lock2', $params)) {
                $ret['lock2'] = pdo_fetchcolumn('select ifnull(sum(paymoney2),0)   from ' . tablename('vending_machine_chongdianzhuang_billp') . ' where openid=:openid and status<>1 and uniacid=:uniacid limit 1',
                    array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }
            if (in_array('lock3', $params)) {
                $ret['lock3'] = pdo_fetchcolumn('select ifnull(sum(paymoney3),0)  from ' . tablename('vending_machine_chongdianzhuang_billp') . ' where openid=:openid and status<>1 and uniacid=:uniacid limit 1',
                    array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }


            if (in_array('total', $params)) {
                $ret['total'] = pdo_fetchcolumn('select ifnull(sum(paymoney1),0)+ifnull(sum(paymoney2),0)  + ifnull(sum(paymoney3),0)  from ' . tablename('vending_machine_chongdianzhuang_billp') . ' where openid=:openid and uniacid=:uniacid limit 1',
                    array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }
            if (in_array('total1', $params)) {
                $ret['total1'] = pdo_fetchcolumn('select ifnull(sum(paymoney1),0)  from ' . tablename('vending_machine_chongdianzhuang_billp') . ' where openid=:openid and uniacid=:uniacid limit 1',
                    array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }
            if (in_array('total2', $params)) {
                $ret['total2'] = pdo_fetchcolumn('select ifnull(sum(paymoney2),0)   from ' . tablename('vending_machine_chongdianzhuang_billp') . ' where openid=:openid and uniacid=:uniacid limit 1',
                    array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }
            if (in_array('total3', $params)) {
                $ret['total3'] = pdo_fetchcolumn('select ifnull(sum(paymoney3),0)  from ' . tablename('vending_machine_chongdianzhuang_billp') . ' where openid=:openid and uniacid=:uniacid limit 1',
                    array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }
            return $ret;
        ...
        function sendMessage($openid = '', $data = array(), $message_type = '')
        {
            ...
            if ($message_type == TM_chongdianzhuang_PAY && empty($usernotice['chongdianzhuang_pay'])) {
                if($tm['is_advanced']){
                    if($tm['chongdianzhuang_pay_close_advanced']){
                        return false;
                    }
            ...
            } else if ($message_type == TM_chongdianzhuang_UPGRADE && empty($usernotice['chongdianzhuang_upgrade'])) {
                if($tm['is_advanced']){
                    if($tm['chongdianzhuang_upgrade1_close_advanced'] && $tm['chongdianzhuang_upgrade2_close_advanced'] && $tm['chongdianzhuang_upgrade3_close_advanced']){
                        return false;
                    }
                    ...
                    $message = '';
                    $upgradetitle = '';
                    if ($data['aagenttype'] == 1 && empty($tm['chongdianzhuang_upgrade1_close_advanced'])) {
                        $message = $tm['upgrade1'];
                        $tag = 'chongdianzhuang_upgrade1';
                        $upgradetitle = empty($tm['upgradetitle1']) ? '省级代理等级升级通知' : $tm['upgradetitle1'];
                    } elseif ($data['aagenttype'] == 2 && empty($tm['chongdianzhuang_upgrade2_close_advanced'])) {
                        $tag = 'chongdianzhuang_upgrade2';
                        $message = $tm['upgrade2'];
                        $upgradetitle = empty($tm['upgradetitle2']) ? '市级代理等级升级通知' : $tm['upgradetitle2'];
                    } elseif ($data['aagenttype'] == 3 && empty($tm['chongdianzhuang_upgrade3_close_advanced'])) {
                        $tag = 'chongdianzhuang_upgrade3';
                        $message = $tm['upgrade3'];
            ...
            } else if ($message_type == TM_chongdianzhuang_BECOME && empty($usernotice['chongdianzhuang_become'])) {
                if($tm['is_advanced']){
                    if($tm['chongdianzhuang_become_close_advanced']){
                        return false;
                    }
                    if ($data['aagenttype'] == 1) {
                        $aagenttype = "省级代理";
        ...
        protected function sendNotice($touser, $tm, $tag, $datas, $member, $msg)
        {
            ...
            if (!empty($tm['is_advanced']) && !empty($tm[$tag])) {
                //高级模式
                $advanced_template = pdo_fetch('select * from ' . tablename('vending_machine_member_message_template') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $tm[$tag], ':uniacid' => $_W['uniacid']));
        ...
        function getLevel($openid)
        {
            ...
            $level = pdo_fetch('select * from ' . tablename('vending_machine_chongdianzhuang_level') . ' where uniacid=:uniacid and id=:id limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $member['aagentlevel']));
            return $level;
        }
        ...
        function upgradeLevelByOrder($openid)
        {
            ...
            if ($leveltype == 4 || $leveltype == 5) { //自购升级
                ...
                $orders = pdo_fetch('select sum(og.realprice) as ordermoney,count(distinct og.orderid) as ordercount from ' . tablename('vending_machine_order') . ' o '
                    . ' left join  ' . tablename('vending_machine_order_goods') . ' og on og.orderid=o.id '
                ...
                if ($leveltype == 4) {
                    ...
                    //自购订单金额
                    $newlevel = pdo_fetch('select * from ' . tablename('vending_machine_chongdianzhuang_level') . " where uniacid=:uniacid  and {$ordermoney} >= ordermoney and ordermoney>0  order by ordermoney desc limit 1", array(':uniacid' => $_W['uniacid']));
                    if (empty($newlevel)) {
                        return;
                    }
                    ...
                    //自购订单数量
                    $newlevel = pdo_fetch('select * from ' . tablename('vending_machine_chongdianzhuang_level') . " where uniacid=:uniacid  and {$ordercount} >= ordercount and ordercount>0  order by ordercount desc limit 1", array(':uniacid' => $_W['uniacid']));
                    if (empty($newlevel)) {
                        return;
                    }
                    if (!empty($oldlevel['id'])) {
                ...
                //代理商升级
                pdo_update('vending_machine_member', array('aagentlevel' => $newlevel['id']), array('id' => $m['id']));
                ...
                foreach ($agents as $agent) {
                    ...
                    //代理商升级
                    pdo_update('vending_machine_member', array('aagentlevel' => $newlevel['id']), array('id' => $agent['id']));
        ...
        function upgradeLevelByAgent($openid)
        {
            ...
            if ($leveltype == 6 || $leveltype == 8) {
                ...
                foreach ($agents as $agent) {
                    ...
                    if ($leveltype == 6) {
                        //下级总人数（代理商+非代理商）
                        $downs1 = pdo_fetchall('select id from ' . tablename('vending_machine_member') . ' where agentid=:agentid and uniacid=:uniacid ', array(':agentid' => $m['id'], ':uniacid' => $_W['uniacid']), 'id');
                        $downcount += count($downs1);
                        if (!empty($downs1)) {
                            $downs2 = pdo_fetchall('select id from ' . tablename('vending_machine_member') . ' where agentid in( ' . implode(',', array_keys($downs1)) . ') and uniacid=:uniacid', array(':uniacid' => $_W['uniacid']), 'id');
                            $downcount += count($downs2);
                            if (!empty($downs2)) {
                                $downs3 = pdo_fetchall('select id from ' . tablename('vending_machine_member') . ' where agentid in( ' . implode(',', array_keys($downs2)) . ') and uniacid=:uniacid', array(':uniacid' => $_W['uniacid']), 'id');
                                $downcount += count($downs3);
                            }
                        }
                        $newlevel = pdo_fetch('select * from ' . tablename('vending_machine_chongdianzhuang_level') . " where uniacid=:uniacid  and {$downcount} >= downcount and downcount>0  order by downcount desc limit 1", array(':uniacid' => $_W['uniacid']));
                    } else if ($leveltype == 8) {
                ...
                if ($leveltype == 7) {
                    //一级下级人数（代理商+非代理商）
                    $downcount = pdo_fetchcolumn('select count(*) from ' . tablename('vending_machine_member') . ' where agentid=:agentid and uniacid=:uniacid ', array(':agentid' => $m['id'], ':uniacid' => $_W['uniacid']));
                    $newlevel = pdo_fetch('select * from ' . tablename('vending_machine_chongdianzhuang_level') . " where uniacid=:uniacid  and {$downcount} >= downcount and downcount>0  order by downcount desc limit 1", array(':uniacid' => $_W['uniacid']));
                } else if ($leveltype == 9) {
                    //一级团队人数（代理商）
        ...
        function upgradeLevelByCommissionOK($openid)
        {
            ...
            $newlevel = pdo_fetch('select * from ' . tablename('vending_machine_chongdianzhuang_level') . " where uniacid=:uniacid  and {$commissionmoney} >= commissionmoney and commissionmoney>0  order by commissionmoney desc limit 1", array(':uniacid' => $_W['uniacid']));
        ...
        function upgradeLevelByBonus($openid)
        {
            ...
            $newlevel = pdo_fetch('select * from ' . tablename('vending_machine_chongdianzhuang_level') . " where uniacid=:uniacid  and {$bonusmoney['ok']} >= bonusmoney and bonusmoney>0  order by bonusmoney desc limit 1", array(':uniacid' => $_W['uniacid']));
        ...
        function getBonusData($year = 0, $month = 0, $week = 0,$openid = '')
        {
            ...
            //历史记录
            $bill = pdo_fetch('select * from ' . tablename('vending_machine_chongdianzhuang_bill') . ' where uniacid=:uniacid and `year`=:years and `month`=:months and `week`=:week limit 1',
            ...
            //找出符合条件的订单
            $orders = pdo_fetchall("select id,openid,price,addressid, address from " . tablename('vending_machine_order') . " where uniacid={$_W['uniacid']} and addressid<>0 and status=3 and ischongdianzhuang=0 and finishtime + {$settletimes}>= {$starttime} and  finishtime + {$settletimes}<={$endtime}", array(), 'id');
            ...
            //所有区域代理商
            $aagents = pdo_fetchall('select m.id,m.openid,m.aagentlevel,m.aagenttype,m.aagentprovinces,m.aagentcitys,m.aagentareas, l.bonus1,l.bonus2,l.bonus3 from ' . tablename('vending_machine_member') . ' m '
                . '  left join ' . tablename('vending_machine_chongdianzhuang_level') . ' l on l.id = m.aagentlevel '
            ...
            foreach ($orders as $o) {
                ...
                foreach ($aagents as &$a) {
                    ...
                    if (!is_array($address)) {
                        $address = pdo_fetch('select * from ' . tablename('vending_machine_member_address') . ' where id=:id limit 1', array(':id' => $o['addressid']));
                    }
        ...
        function getTotals()
        {
            global $_W;
            return array(
                'total0' => pdo_fetchcolumn('select count(*) from ' . tablename('vending_machine_chongdianzhuang_bill') . ' where uniacid=:uniacid and status=0 limit 1', array(':uniacid' => $_W['uniacid'])),
                'total1' => pdo_fetchcolumn('select count(*) from ' . tablename('vending_machine_chongdianzhuang_bill') . ' where uniacid=:uniacid and status=1 limit 1', array(':uniacid' => $_W['uniacid'])),
                'total2' => pdo_fetchcolumn('select count(*) from ' . tablename('vending_machine_chongdianzhuang_bill') . ' where uniacid=:uniacid and status=2  limit 1', array(':uniacid' => $_W['uniacid']))
            );
        }
    }
